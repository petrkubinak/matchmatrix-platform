...
Fix automation execution queue dedup V1

CO TO JE:
- Oprava duplicity řádků ve view:
  ops.v_automation_execution_queue

PROČ:
- Jeden sport/entity/provider mohl mít více planner řádků.
- JOIN pak vracel duplicity.
- V16 panel musí dostat pouze 1 nejlepší execution row.

K ČEMU TO JE:
- čistý execution queue output
- správný scheduler view
- stabilní automation dashboard
- přesné provider routing rozhodování
...
planner_ranked AS (
    SELECT
        ...
        ROW_NUMBER() OVER (
            PARTITION BY provider, sport_code, entity
            ORDER BY
                CASE
                    WHEN status IN ('running') THEN 1
                    WHEN status IN ('pending') THEN 2
                    WHEN status IN ('ready') THEN 3
                    WHEN status IN ('done') THEN 4
                    ELSE 9
                END,
