/*
MATCHMATRIX SQL 106_E V2

Soubor:
C:\MatchMatrix-platform\sql\106_E_create_v_provider_routing_master_v2.sql

Co to je:
Centrální provider routing master view pro MatchMatrix orchestration layer.

K čemu to je:
Sjednocuje provider coverage, runtime audit, sport completion audit,
people audit a provider sport matrix do jednoho orchestration pohledu.
...
- failover kandidáti
- scheduler
- později AI orchestration

Jak se to použije na webu/aplikaci:
Interní OPS vrstva. Slouží k rozhodování:
- co lze spustit
- co je blokované
- kde je fallback
- kde chybí endpoint / worker / provider
- co čeká na PRO
*/

CREATE OR REPLACE VIEW ops.v_provider_routing_master_V2 AS
...
base AS (
    SELECT
        ...
        CASE
            ...
            WHEN coverage_status ILIKE 'runtime_tested%' THEN 'CONFIRMED_PROVIDER'
            WHEN coverage_status ILIKE 'tech_ready%' THEN 'RUNNABLE_PROVIDER'
...
ranked AS (
    SELECT
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY
                CASE
                    WHEN b.candidate_provider = b.primary_provider THEN 0
                    WHEN b.candidate_provider = b.fallback_provider THEN 1
                    ELSE 2
                END,
                b.provider_priority NULLS LAST,
                b.fetch_priority NULLS LAST,
                b.merge_priority NULLS LAST,
                b.candidate_provider
        ) AS routing_rank
...
SELECT
    ...
    CASE
        WHEN provider_gap = 'OK'
         AND routing_candidate IS TRUE
         AND (
                runtime_state ILIKE 'confirmed%'
             OR runtime_state ILIKE 'runtime_tested%'
             OR runtime_state ILIKE 'ready%'
             OR runtime_state ILIKE 'done%'
             OR coverage_status ILIKE 'confirmed%'
             OR coverage_status ILIKE 'runtime_tested%'
         )
        THEN TRUE
    ...
    CASE
        ...
        WHEN provider_gap = 'GAP_NO_PRIMARY_PROVIDER'
        THEN 'Chybí primary provider.'

        WHEN provider_gap = 'GAP_NO_FALLBACK_PROVIDER'
        THEN 'Chybí fallback provider.'

        WHEN provider_gap = 'GAP_NO_ENDPOINT'
        THEN 'Provider nemá endpoint.'

        WHEN provider_gap = 'GAP_NO_WORKER'
        THEN 'Chybí worker script.'

        WHEN provider_gap = 'GAP_PROVIDER_BLOCKED'
        THEN 'Primary provider je blokovaný.'

        WHEN provider_gap = 'GAP_PROVIDER_PLANNED_ONLY'
        THEN 'Provider je pouze planned.'

        WHEN provider_gap = 'GAP_NOT_PRODUCTION_READY'
        THEN 'Není production ready.'

        ELSE 'Routing není připraven pro automatizaci.'
