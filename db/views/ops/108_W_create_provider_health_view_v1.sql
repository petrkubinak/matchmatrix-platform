...
agg AS (
    ...
    SELECT
        ...
        SUM(
            CASE
                WHEN LOWER(COALESCE(parse_status, '')) NOT IN
                ('parsed', 'done', 'ok', 'success', 'failed', 'error', 'pending', 'empty')
                THEN 1 ELSE 0
            END
        ) AS other_status_payloads,        
...
SELECT
    ...
    CASE
        ...
        WHEN other_status_payloads > 0
             AND other_status_payloads::numeric / total_payloads >= 0.30
            THEN 'HIGH'
