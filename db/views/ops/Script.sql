...
CO TO JE:
- Vyhodnocení kvality planner targetů.

K ČEMU TO JE:
- Odhaluje ligy/sezóny které:
    - opakovaně vrací empty data
    - mají warning pressure
    - způsobují planner overload
...
parsed AS (
    ...
    SELECT
        ...
        CASE
            WHEN command_text ~ '--league-id[ =][0-9]+'
            THEN regexp_replace(
                command_text,
                '.*--league-id[ =]([0-9]+).*',
                '\1'
