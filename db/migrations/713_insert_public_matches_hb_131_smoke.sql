-- 713_insert_public_matches_hb_131_smoke.sql
-- Cíl:
-- Prvni smoke merge HB fixtures do public.matches
-- pro:
...
src AS (
    ...
    WHERE f.provider = 'api_handball'
      ...
      AND f.external_league_id = '131'
      AND f.season = '2024'
),
mapped AS (
    SELECT
        24881::int AS league_id,         -- Champions League canonical league
        hs.sport_id,
        ...
        s.fixture_date,
        CASE
            WHEN UPPER(COALESCE(s.status_text, '')) IN ('FT', 'AET', 'PEN') THEN 'FINISHED'
        ...
        CASE
            WHEN UPPER(COALESCE(s.status_text, '')) IN ('FT', 'AET', 'PEN')
...
-- kontrola 1: kolik HB matches je po merge
SELECT
    COUNT(*) AS hb_matches_total
FROM public.matches
WHERE ext_source = 'api_handball';

-- kontrola 2: ukazka HB matches
SELECT
    m.id,
    m.league_id,
