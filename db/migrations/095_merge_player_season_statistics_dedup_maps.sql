-- ============================================================================
-- 095_merge_player_season_statistics_dedup_maps.sql
-- Cíl:
--   Finální merge season-level player stats
--   s deduplikovanými provider mapami
-- ============================================================================
...
src AS (
    ...
    JOIN public.sports sp
      ON sp.code = 'FB'
    JOIN ppm_u
      ON ppm_u.provider = s.provider
     AND ppm_u.provider_player_id = s.player_external_id
    JOIN tpm_u
      ON tpm_u.provider = s.provider
     AND tpm_u.provider_team_id = s.team_external_id
    JOIN lpm_u
      ON lpm_u.provider = s.provider
     AND lpm_u.provider_league_id = s.external_league_id
    WHERE lower(s.sport_code) = 'football'
    GROUP BY
        ppm_u.player_id,
        tpm_u.team_id,
        sp.id,
