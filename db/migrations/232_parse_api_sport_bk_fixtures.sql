-- ============================================================================
-- MatchMatrix
-- Parse latest api_sport basketball fixtures(games) payload -> staging.stg_provider_fixtures
-- File: C:\MatchMatrix-platform\db\migrations\2026-03-24_parse_api_sport_bk_fixtures.sql
-- ============================================================================
...
norm AS (
    SELECT
        i.provider,
        i.sport_code,
        COALESCE(i.item_json -> 'id' ->> 'id', i.item_json ->> 'id') AS external_fixture_id,
        COALESCE(i.item_json -> 'league' ->> 'id', i.item_json ->> 'league') AS external_league_id,
        COALESCE(i.item_json -> 'league' ->> 'season', i.item_json ->> 'season') AS season,
        COALESCE(i.item_json -> 'teams' -> 'home' ->> 'id', i.item_json -> 'home' ->> 'id') AS home_team_external_id,
        COALESCE(i.item_json -> 'teams' -> 'away' ->> 'id', i.item_json -> 'away' ->> 'id') AS away_team_external_id,
        COALESCE(
            NULLIF(i.item_json -> 'date' ->> 'date', ''),
        ...
        COALESCE(
            ...
            i.item_json -> 'scores' -> 'home' ->> 'points',
            i.item_json -> 'scores' -> 'home' ->> 'score'
        ) AS home_score,
        COALESCE(
            i.item_json -> 'scores' -> 'away' ->> 'total',
            i.item_json -> 'scores' -> 'away' ->> 'points',
