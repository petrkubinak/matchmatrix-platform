-- ============================================================================
-- 097_merge_player_season_stats_final_business_dedup.sql
-- Cíl:
--   Finální merge player season stats
--   s poslední deduplikací na business key:
...
src AS (
    SELECT
        ...
        MAX(passes_total) AS passes_total,
        MAX(passes_key) AS passes_key,
        MAX(passes_accuracy) AS passes_accuracy,
        MAX(tackles_total) AS tackles_total,
        MAX(tackles_blocks) AS tackles_blocks,
        MAX(tackles_interceptions) AS tackles_interceptions,
        MAX(duels_total) AS duels_total,
        MAX(duels_won) AS duels_won,
        MAX(dribbles_attempts) AS dribbles_attempts,
        ...
        MAX(penalty_won) AS penalty_won,
        MAX(penalty_committed) AS penalty_committed,
        MAX(penalty_scored) AS penalty_scored,
        MAX(penalty_missed) AS penalty_missed,
        MAX(penalty_saved) AS penalty_saved
    FROM src_raw
    GROUP BY
        player_id,
        team_id,
