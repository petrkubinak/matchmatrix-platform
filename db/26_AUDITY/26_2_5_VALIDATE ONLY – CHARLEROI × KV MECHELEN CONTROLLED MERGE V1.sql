...
REVIEW CASE 2
VALIDATE ONLY – CHARLEROI × KV MECHELEN CONTROLLED MERGE V1
===============================================================================
...
ŘÍZENÉ ŘEŠENÍ:
1. Kanonický zápas dostane oficiální kontumační výsledek 0:5.
2. Výsledek 1:0 při přerušení se zachová v provenienci.
3. Identita football_data_uk se přesune jako sekundární.
4. match_features a mm_match_ratings se převedou.
5. Historická duplicita 7569 se zkušebně odstraní.
6. Vše se na konci vrátí pomocí ROLLBACK.
===============================================================================
*/
...
CREATE TEMP TABLE mm_be_case2_target_match_snapshot
ON COMMIT DROP
AS
SELECT
    to_jsonb(match)
...
BEGIN
    ...
    FROM mm_be_case2_target_match_snapshot;
    ...
    IF v_review_cases <> 2 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Případů k revizi je % místo 2.',
            v_review_cases;
    END IF;
...
BEGIN
    ...
    RAISE NOTICE
        'OK MATCH: Kanonický výsledek zkušebně nastaven na 0:5 / FINISHED.';
END
$update_match$;
...
BEGIN
    ...
    SET
        ...
        updated_at = clock_timestamp(),
        updated_by = 'BELGIUM_REVIEW_CASE_2_VALIDATE'
    ...
    FOR column_record IN
        ...
        WHERE table_object.relkind IN ('r', 'p')
          ...
          (
              ...
              WHERE fk.schema_name = namespace_object.nspname
                AND fk.table_name = table_object.relname
                AND fk.fk_column = column_object.attname
          )
...
-------------------------------------------------------------------------------
-- 17. ZKUŠEBNÍ ODSTRANĚNÍ DUPLICITY
-------------------------------------------------------------------------------
...
BEGIN
    ...
    RAISE NOTICE
        'OK DELETE: Historická duplicita 7569 zkušebně odstraněna.';
END
$delete_historical$;
...
BEGIN
    ...
    FROM public.matches match
    CROSS JOIN mm_be_case2_target_match_snapshot snapshot
    WHERE match.id = 344421
      AND (
          to_jsonb(match)
              - ARRAY[
    ...
    IF v_review_cases <> 1 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Zbývajících review případů je % místo 1.',
            v_review_cases;
    END IF;
...
SELECT
    'VALIDATE_ONLY_REVIEW_CASE_2_OK – OFICIÁLNÍ VÝSLEDEK 0:5 A ÚPLNÉ SLOUČENÍ OVĚŘENO'
...
SELECT
    CASE
        ...
        THEN
            'ROLLBACK_OK – REVIEW CASE 2 VRÁCEN DO PŮVODNÍHO STAVU'
