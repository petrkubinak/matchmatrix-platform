...
VALIDATE ONLY – DOWNSTREAM REFERENCE TRANSFER V1
===============================================================================

CO:
- Zkušebně převede 927 vazeb v public.match_features.
- Zkušebně převede 927 řádků v public.mm_match_ratings.
- U ratingů sjednotí dimenze podle cílového kanonického zápasu:
    match_id,
    league_id,
    kickoff,
    home_team_id,
...
- public.match_provider_map se nemění.
- Na konci proběhne povinný ROLLBACK.
===============================================================================
*/
...
-------------------------------------------------------------------------------
-- 2. OCHRANA PROTI SOUBĚŽNÝM ZMĚNÁM
-------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------
-- 3. MIGRAČNÍ PLÁN VČETNĚ CÍLOVÝCH DIMENZÍ
-------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------
-- 5. SNAPSHOT DAT, KTERÁ SE NESMĚJÍ ZMĚNIT
-------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------
-- 6. POVINNÉ KONTROLY PŘED ZKUŠEBNÍ MIGRACÍ
-------------------------------------------------------------------------------
...
DECLARE
    ...
    v_ratings_consistent_previous bigint;
...
BEGIN
    ...
    SELECT COUNT(*)
    INTO v_ratings_consistent_previous
    FROM public.mm_match_ratings ratings
    JOIN mm_be_downstream_transfer_plan plan
      ON ratings.match_id = plan.previous_match_id
    ...
    IF v_null_target_dimensions <> 0 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: % cílových zápasů nemá úplné dimenze.',
            v_null_target_dimensions;
    END IF;
    ...
    IF v_features_previous <> 927
       OR v_features_target <> 0 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: match_features previous %, target %.',
            v_features_previous,
            v_features_target;
    END IF;
    ...
    IF v_ratings_previous <> 927
       OR v_ratings_target <> 0 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: mm_match_ratings previous %, target %.',
            v_ratings_previous,
            v_ratings_target;
    END IF;

    IF v_ratings_consistent_previous <> 927 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: S původním zápasem je konzistentních pouze % ratingů.',
            v_ratings_consistent_previous;
    END IF;

    IF v_feature_snapshot_rows <> 927
       OR v_rating_snapshot_rows <> 927 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: snapshot features %, ratings %.',
            v_feature_snapshot_rows,
            v_rating_snapshot_rows;
    END IF;

    RAISE NOTICE
        'OK PRECHECK: plán 927, features 927/0, ratings 927/0, ratingové dimenze 927.';
END
$precheck$;

-------------------------------------------------------------------------------
-- 7. ZKUŠEBNÍ PŘEVOD public.match_features
-------------------------------------------------------------------------------

DO $transfer_features$
DECLARE
...
BEGIN
    ...
    IF v_updated_rows <> 927 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: match_features převedeno % místo 927.',
            v_updated_rows;
    END IF;

    RAISE NOTICE
        'OK FEATURES TRANSFER: Zkušebně převedeno 927 řádků.';
END
$transfer_features$;

-------------------------------------------------------------------------------
-- 8. ZKUŠEBNÍ PŘEVOD public.mm_match_ratings
-------------------------------------------------------------------------------
...
BEGIN
    ...
    IF v_updated_rows <> 927 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: mm_match_ratings převedeno % místo 927.',
            v_updated_rows;
    END IF;

    RAISE NOTICE
        'OK RATINGS TRANSFER: Zkušebně převedeno a sjednoceno 927 řádků.';
END
$transfer_ratings$;

-------------------------------------------------------------------------------
-- 9. ÚPLNÁ KONTROLA PO ZKUŠEBNÍ MIGRACI
-------------------------------------------------------------------------------
...
BEGIN
    ...
    IF v_ratings_previous <> 0
       ...
       OR v_ratings_aligned <> 927 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: ratings previous %, target %, aligned %.',
            v_ratings_previous,
            v_ratings_target,
            v_ratings_aligned;
    END IF;
    ...
    IF v_feature_payload_mismatches <> 0 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Změněno % feature payloadů.',
            v_feature_payload_mismatches;
    END IF;

    IF v_rating_payload_mismatches <> 0 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Změněno % ratingových payloadů.',
    ...
    IF v_features_total <> v_baseline_features_total
       ...
       OR v_features_orphans <> v_baseline_features_orphans THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Změněn souhrn match_features.';
    END IF;
    ...
    IF v_ratings_total <> v_baseline_ratings_total
       ...
       OR v_ratings_orphans <> v_baseline_ratings_orphans THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Změněn souhrn mm_match_ratings.';
    END IF;
...
-------------------------------------------------------------------------------
-- 10. KONTROLNÍ SOUHRN PŘED ROLLBACKEM
-------------------------------------------------------------------------------
...
SELECT
    'VALIDATE_ONLY_DOWNSTREAM_TRANSFER_OK – 927 VAZEB V OBOU TABULKÁCH OVĚŘENO'
...
-------------------------------------------------------------------------------
-- 12. KONTROLA OBNOVENÍ PŮVODNÍHO STAVU
-------------------------------------------------------------------------------
...
SELECT
    CASE
        ...
        THEN
            'ROLLBACK_OK – DOWNSTREAM TABULKY VRÁCENY DO PŮVODNÍHO STAVU'
        ELSE
