...
APPLY – DELETE 927 HISTORICAL DUPLICATE MATCHES V1
===============================================================================

CO:
- Trvale odstraní 927 historických zápasů football_data_uk,
  jejichž providerové identity a downstream data již byly převedeny
  na kanonické zápasy API-Football.

ZACHOVÁ:
- 1284 unikátních historických zápasů,
- 3 případy k ruční revizi,
- 960 zápasů API-Football,
- 121908 providerových identit,
- všechny downstream tabulky a jejich řádky.

OČEKÁVANÝ VÝSLEDEK:
- public.matches celkem: 120984
- belgická historie football_data_uk: 1287
    - 1284 unikátních zápasů
    - 3 případy k revizi
- belgické API-Football zápasy: 960
- původní duplicitní zápasy: 0

BEZPEČNOST:
- Opakovaná kontrola všech FK vazeb.
- Opakovaná kontrola měkkých match_id vazeb.
- Kontrola, že DELETE nezměnil žádnou downstream tabulku.
- Při jakékoliv odchylce je celá transakce automaticky zrušena.
===============================================================================
*/
...
-------------------------------------------------------------------------------
-- 4. KATALOG FOREIGN KEY VAZEB NA public.matches(id)
-------------------------------------------------------------------------------
...
BEGIN
    ...
    RAISE NOTICE
        'OK LOCKS: public.matches, providerová mapa a všechny FK tabulky uzamčeny.';
END
$lock_fk_tables$;

-------------------------------------------------------------------------------
-- 6. SNAPSHOT VÝCHOZÍHO STAVU
-------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------
-- 7. SNAPSHOT POČTŮ VŠECH FK TABULEK
-------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------
-- 8. AUDIT DECLAROVANÝCH FK VAZEB
-------------------------------------------------------------------------------
...
BEGIN
    ...
    IF v_previous_matches <> 927
       OR v_target_matches <> 927 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: previous matches %, target matches %.',
            v_previous_matches,
            v_target_matches;
    END IF;
    ...
    IF v_provider_identities_previous <> 0 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: Na původních zápasech je % providerových identit.',
            v_provider_identities_previous;
    END IF;

    IF v_provider_identities_target <> 1854 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: Na cílových zápasech je % identit místo 1854.',
    ...
    IF v_ratings_previous <> 0
       OR v_ratings_target <> 927 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: ratings previous %, target %.',
            v_ratings_previous,
            v_ratings_target;
    END IF;
    ...
    IF v_fk_references <> 0 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: Deklarované FK obsahují % vazeb.',
            v_fk_references;
    END IF;

    IF v_soft_references <> 0 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: Měkké vazby obsahují % odkazů.',
            v_soft_references;
    END IF;

    IF v_review_cases <> 3 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: Případů k revizi existuje % místo 3.',
            v_review_cases;
    END IF;
...
-------------------------------------------------------------------------------
-- 11. TRVALÉ ODSTRANĚNÍ 927 HISTORICKÝCH DUPLICIT
-------------------------------------------------------------------------------
...
BEGIN
    ...
    IF v_deleted_rows <> 927 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: Odstraněno % zápasů místo 927.',
            v_deleted_rows;
    END IF;

    RAISE NOTICE
        'OK DELETE APPLY: Trvale připraveno odstranění 927 historických duplicit.';
END
$apply_delete$;
...
BEGIN
    ...
    LOOP
        ...
        IF v_after_count <> table_record.row_count THEN
            RAISE EXCEPTION
                'APPLY_FAILED: Tabulka %.% změnila počet z % na %.',
                table_record.schema_name,
                table_record.table_name,
...
BEGIN
    ...
    IF v_total_matches <> 120984 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: Celkový počet zápasů je % místo 120984.',
            v_total_matches;
    END IF;
    ...
    IF v_history_matches <> 1287 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: Historických belgických zápasů je % místo 1287.',
            v_history_matches;
    END IF;

    IF v_api_matches <> 960 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: API belgických zápasů je % místo 960.',
            v_api_matches;
    END IF;

    IF v_review_cases <> 3 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: Review případy % místo 3.',
            v_review_cases;
    END IF;

    IF v_provider_rows <> v_baseline_provider_rows
       OR v_provider_matches <> v_baseline_provider_matches
       OR v_provider_previous <> 0
       OR v_provider_orphans <> 0 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: Providerová mapa změnila nebo ztratila integritu.';
    END IF;

    IF v_features_rows <> v_baseline_features_rows
       OR v_features_target <> 927 THEN
        RAISE EXCEPTION
            'APPLY_FAILED: match_features změnila stav.';
    END IF;
    ...
    IF v_ratings_rows <> v_baseline_ratings_rows
       ...
       OR v_ratings_orphans <> v_baseline_ratings_orphans THEN
        RAISE EXCEPTION
            'APPLY_FAILED: mm_match_ratings změnila stav.';
    END IF;

    RAISE NOTICE
        'OK POSTCHECK: matches 120984, previous 0, target 927, history 1287, API 960, review 3, provider mapa a downstream tabulky beze změny.';
END
$postcheck$;
...
SELECT
    'APPLY_VALIDATED – 927 HISTORICKÝCH DUPLICIT PŘIPRAVENO K COMMIT'
...
SELECT
    CASE
        WHEN total_matches = 120984
         AND previous_matches = 0
         AND target_matches = 927
         AND history_matches = 1287
         AND api_matches = 960
         AND review_matches = 3
         AND provider_orphans = 0
         AND target_features = 927
         AND target_ratings = 927
        THEN
            'APPLY_OK – 927 HISTORICKÝCH DUPLICIT TRVALE ODSTRANĚNO'
        ELSE
