...
REVIEW CASE 3
VALIDATE ONLY – ANTWERP × BEERSCHOT CONTROLLED MERGE V1
===============================================================================
...
ŘÍZENÉ ŘEŠENÍ:
1. Kanonický zápas dostane oficiální výsledek 5:0.
2. Skóre 4:0 při přerušení zůstane v provenienci.
3. API-Football zůstane jedinou primární identitou.
4. football_data_uk se přesune jako sekundární identita.
5. match_features a mm_match_ratings se převedou.
6. Historická duplicita 6865 se zkušebně odstraní.
7. Na konci proběhne povinný ROLLBACK.
===============================================================================
*/
...
CREATE TEMP TABLE mm_be_case3_plan
ON COMMIT DROP
AS
SELECT
...
WHERE historical.id = 6865
  AND target.id = 317481;

-------------------------------------------------------------------------------
-- 4. KATALOG FOREIGN KEY VAZEB
-------------------------------------------------------------------------------

CREATE TEMP TABLE mm_be_case3_fk_catalog
ON COMMIT DROP
AS
SELECT
...
CREATE TEMP TABLE mm_be_case3_baseline
ON COMMIT DROP
AS
SELECT
...
CREATE TEMP TABLE mm_be_case3_target_snapshot
ON COMMIT DROP
AS
SELECT
    to_jsonb(match)
...
WHERE match.id = 317481;

CREATE TEMP TABLE mm_be_case3_feature_snapshot
ON COMMIT DROP
AS
SELECT
    to_jsonb(features)
...
WHERE features.match_id = 6865;

CREATE TEMP TABLE mm_be_case3_rating_snapshot
ON COMMIT DROP
AS
SELECT
    to_jsonb(ratings)
...
FROM public.mm_match_ratings ratings
WHERE ratings.match_id = 6865;

-------------------------------------------------------------------------------
-- 8. AUDIT DALŠÍCH FK VAZEB
-------------------------------------------------------------------------------

CREATE TEMP TABLE mm_be_case3_fk_audit
(
...
BEGIN
    ...
    LOOP
        ...
        INSERT INTO mm_be_case3_fk_audit
        VALUES
        (
            fk_record.schema_name,
            fk_record.table_name,
...
BEGIN
    ...
    WHERE id = 6865
      AND league_id = 4
      AND kickoff::date = DATE '2024-09-29'
      AND home_team_id = 964
      AND away_team_id = 980
      AND home_score = 5
      AND away_score = 0
      AND status = 'FINISHED'
    ...
    WHERE id = 317481
      AND league_id = 20853
      AND kickoff::date = DATE '2024-09-29'
      AND home_team_id = 13172
      AND away_team_id = 12515
      AND home_score = 4
      AND away_score = 0
      AND status = 'FINISHED'
    ...
    WHERE match_id = 6865
      ...
      AND provider_match_id =
          '4|2425|29/09/2024|Antwerp|Beerschot VA'
      AND is_primary = true
    ...
    FROM public.match_provider_map
    WHERE match_id = 317481
      AND provider = 'api_football'
      AND provider_match_id = '1213268'
      AND is_primary = true
      AND mapping_status = 'ACTIVE';
    ...
    FROM mm_be_case3_target_snapshot;

    SELECT COUNT(*)
    INTO v_feature_snapshot
    FROM mm_be_case3_feature_snapshot;

    SELECT COUNT(*)
    INTO v_rating_snapshot
    FROM mm_be_case3_rating_snapshot;
    ...
    IF v_historical_features <> 1
       ...
       OR v_target_ratings <> 0 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Downstream data nejsou v očekávaném stavu.';
    END IF;
    ...
    IF v_target_snapshot <> 1
       ...
       OR v_rating_snapshot <> 1 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Snapshoty nejsou úplné.';
    END IF;

    IF v_review_cases <> 1 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Poslední review případ neexistuje právě jednou.';
    END IF;

    RAISE NOTICE
        'OK PRECHECK: zápasy, identity, features 1/0, ratings 1/0, další FK 0 a review případ 1.';
END
$precheck$;
...
BEGIN
    ...
    IF v_updated_rows <> 1 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Kanonický zápas nebyl aktualizován.';
    END IF;

    RAISE NOTICE
        'OK MATCH: Kanonický výsledek zkušebně nastaven na 5:0 / FINISHED.';
END
$update_match$;
...
BEGIN
    ...
    SET
        metadata =
            ...
            jsonb_build_object(
                ...
                jsonb_build_object(
                    ...
                    'official_statistics_retained',
                    true,

                    'matchmatrix_review_case',
                    'BELGIUM_REVIEW_CASE_3',
        ...
        updated_at = clock_timestamp(),
        updated_by = 'BELGIUM_REVIEW_CASE_3_VALIDATE'
    ...
    IF v_updated_rows <> 1 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Kanonická identita nebyla aktualizována.';
    END IF;
END
...
BEGIN
    ...
    IF v_updated_rows <> 1 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Historická identita nebyla přesunuta.';
    END IF;

    RAISE NOTICE
        'OK IDENTITY: Historická identita zkušebně převedena jako sekundární.';
END
$move_identity$;
...
BEGIN
    ...
    IF v_updated_rows <> 1 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: match_features nebyla převedena.';
    END IF;

    RAISE NOTICE
        'OK FEATURES: Zkušebně převeden 1 řádek.';
END
$move_features$;
...
BEGIN
    ...
    IF v_updated_rows <> 1 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: mm_match_ratings nebyla převedena.';
    END IF;

    RAISE NOTICE
        'OK RATINGS: Zkušebně převeden a rozměrově sjednocen 1 řádek.';
END
$move_ratings$;
...
BEGIN
    ...
    LOOP
        ...
        IF v_reference_rows <> 0 THEN
            RAISE EXCEPTION
                'VALIDATION_FAILED: %.% stále obsahuje % FK vazeb.',
                fk_record.schema_name,
                fk_record.table_name,
                v_reference_rows;
        END IF;
    ...
    RAISE NOTICE
        'OK FK CHECK: Na zápas 6865 nezůstala žádná FK vazba.';
END
$remaining_fk_check$;
...
BEGIN
    FOR column_record IN
        ...
        WHERE table_object.relkind IN ('r', 'p')
          ...
          (
              SELECT 1
              FROM mm_be_case3_fk_catalog fk
    ...
    LOOP
        ...
        IF v_reference_rows <> 0 THEN
            RAISE EXCEPTION
                'VALIDATION_FAILED: %.% stále obsahuje % měkkých vazeb.',
                column_record.schema_name,
                column_record.table_name,
                v_reference_rows;
        END IF;
    ...
    RAISE NOTICE
        'OK SOFT CHECK: Na zápas 6865 nezůstala žádná měkká vazba.';
END
$remaining_soft_check$;

-------------------------------------------------------------------------------
-- 17. ZKUŠEBNÍ ODSTRANĚNÍ HISTORICKÉ DUPLICITY
-------------------------------------------------------------------------------
...
BEGIN
    ...
    IF v_deleted_rows <> 1 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Historický zápas nebyl odstraněn.';
    END IF;

    RAISE NOTICE
        'OK DELETE: Historická duplicita 6865 zkušebně odstraněna.';
END
$delete_historical$;

-------------------------------------------------------------------------------
-- 18. ÚPLNÁ KONTROLA PO MIGRACI
-------------------------------------------------------------------------------
...
BEGIN
    ...
    FROM public.matches match
    CROSS JOIN mm_be_case3_target_snapshot snapshot
    WHERE match.id = 317481
      AND (
          to_jsonb(match)
              - ARRAY[
    ...
    FROM public.match_features features
    CROSS JOIN mm_be_case3_feature_snapshot snapshot
    WHERE features.match_id = 317481
      AND (
          to_jsonb(features)
              - ARRAY[
                  'match_id',
    ...
    FROM public.mm_match_ratings ratings
    CROSS JOIN mm_be_case3_rating_snapshot snapshot
    WHERE ratings.match_id = 317481
      AND (
          to_jsonb(ratings)
              - ARRAY[
    ...
    IF v_target_identities <> 2
       ...
       OR v_history_resolution <> 1 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Providerové identity nebo provenance nejsou správné.';
    END IF;
    ...
    IF v_target_payload_mismatch <> 0
       ...
       OR v_rating_payload_mismatch <> 0 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Neočekávaná změna zachovávaných dat.';
    END IF;
    ...
    IF v_review_cases <> 0 THEN
        RAISE EXCEPTION
            'VALIDATION_FAILED: Review případ stále existuje.';
    END IF;
    ...
    RAISE NOTICE
        'OK POSTCHECK: 5:0, dvě identity, jedna primární, downstream data převedena, duplicita odstraněna a review případy 0.';
END
$postcheck$;

-------------------------------------------------------------------------------
-- 19. SOUHRN PŘED ROLLBACKEM
-------------------------------------------------------------------------------
...
SELECT
    'VALIDATE_ONLY_REVIEW_CASE_3_OK – OFICIÁLNÍ VÝSLEDEK 5:0 A ÚPLNÉ SLOUČENÍ OVĚŘENO'
...
-------------------------------------------------------------------------------
-- 21. KONTROLA OBNOVENÍ PŮVODNÍHO STAVU
-------------------------------------------------------------------------------
...
SELECT
    CASE
        ...
        THEN
            'ROLLBACK_OK – REVIEW CASE 3 VRÁCEN DO PŮVODNÍHO STAVU'
