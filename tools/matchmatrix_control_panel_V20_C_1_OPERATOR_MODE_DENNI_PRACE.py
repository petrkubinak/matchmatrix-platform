"""
MATCHMATRIX CONTROL PANEL V20.C.1 OPERATOR MODE DENNI PRACE
========================================================
...
V20.C.1:
- DENNÍ PRÁCE převedena do OPERATOR MODE.
- Ze záložky jsou odstraněny provozně málo užitečné bloky STRUČNÝ STAV PC2 a PC2 MASTER ROADMAP.
- Přidány bloky PRŮBĚH POSLEDNÍHO BĚHU a VÝSLEDEK / CO DĚLAT DÁL.
- Cíl: spustit, sledovat průběh, vidět výsledek, dostat srozumitelnou opravu a pokračovat.
"""
...
# V20.C.1 - DENNÍ PRÁCE OPERATOR MODE
COLUMN_LABELS.update({
    "operator_block": "Blok",
    "operator_status": "Stav",
    "current_action": "Aktuální akce",
    "progress_pct": "Průběh %",
    "progress_bar": "Graficky",
    "processed_count": "Zpracováno",
    "inserted_count": "Nové",
    "updated_count": "Aktualizováno",
    "skipped_count": "Přeskočeno",
    "error_count": "Chyby",
    "eta_text": "ETA",
    "what_happened": "Co se stalo",
    "what_to_do": "Co dělat",
    "next_safe_action": "Další bezpečný krok",
    "operator_note": "Poznámka pro operátora",
})
...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V20.C.1 - DENNÍ PRÁCE OPERATOR MODE"
        )
    ...
    def build_ui(self):
        ...
        tk.Label(
            header,
            text="▰ MATCHMATRIX ŘÍDICÍ CENTRUM V20.C.1",
            bg="#07040d",
            fg=PINK,
...
# =========================================================
        ...
        tab_pc2_command.rowconfigure(0, weight=1)
        tab_pc2_command.rowconfigure(1, weight=2)
        tab_pc2_command.rowconfigure(2, weight=1)
        tab_pc2_command.rowconfigure(3, weight=1)
        ...
        self.pc2_button_tree = self.create_section(
            tab_pc2_command, "🏠 DENNÍ PRÁCE – CO SPUSTIT TEĎ", 0, 0, 2
        )

        self.pc2_queue_tree = self.create_section(
            tab_pc2_command, "🧭 DETAIL FRONTY / STAVY", 1, 0, 2
        )

        self.pc2_run_monitor_tree = self.create_section(
            tab_pc2_command, "📊 PRŮBĚH POSLEDNÍHO BĚHU", 2, 0
        )

        self.pc2_actions_tree = self.create_section(
            tab_pc2_command, "✅ MOŽNÉ AKCE NAD VYBRANÝM ŘÁDKEM", 2, 1
        )

        self.pc2_result_tree = self.create_section(
            tab_pc2_command, "✅ VÝSLEDEK / CO DĚLAT DÁL", 3, 0, 2
        )

        # V20.C.1: staré bloky STRUČNÝ STAV PC2 a PC2 MASTER ROADMAP už v DENNÍ PRÁCI nezobrazujeme.
        # Data zůstávají dostupná v PŘEHLED / ROADMAPA. Alias necháváme kvůli bezpečné zpětné kompatibilitě.
        self.pc2_kpi_tree = self.pc2_run_monitor_tree
        self.pc2_roadmap_tree = self.pc2_result_tree
...
Další termín: {h.get('next_target_date') or '-'}"""
    ...
    def load_pc2_operator_run_monitor(self):
        """
        V20.C.1 - OPERATOR RUN MONITOR

        CO TO JE:
        - Srozumitelný operátorský přehled poslední / aktuální PC2 akce.

        K ČEMU TO JE:
        - V DENNÍ PRÁCI ukáže průběh, výsledek, chybu a doporučený další krok.
        - Nevyžaduje zatím nové monitorovací tabulky; čte existující PC2 command queue.

        KDE TO UVIDÍME:
        - DENNÍ PRÁCE -> PRŮBĚH POSLEDNÍHO BĚHU
        - DENNÍ PRÁCE -> VÝSLEDEK / CO DĚLAT DÁL

        JAK SE TO VYUŽIJE:
        - Operátor hned vidí, jestli pokračovat, retry, blokovat nebo zkontrolovat log.
        """
        ...
        WITH latest AS (
            ...
            ORDER BY
                COALESCE(last_started_at, last_finished_at, updated_at) DESC NULLS LAST,
                id DESC
            LIMIT 1
        )
        SELECT
            'AKTUÁLNÍ / POSLEDNÍ AKCE' AS operator_block,
            command_id,
            sport_code,
            sport_name,
            target_layer,
            command_title AS current_action,
            run_status AS operator_status,
            CASE
                WHEN run_status = 'DONE' THEN 100
                WHEN run_status = 'FAILED' THEN 100
                WHEN run_status = 'READY_TO_RUN' THEN 0
                WHEN run_status = 'RUNNING' THEN 50
                ELSE 25
            END AS progress_pct,
            CASE
                WHEN run_status = 'DONE' THEN '████████████████████ 100 %'
                WHEN run_status = 'FAILED' THEN '████████████████████ CHYBA'
                WHEN run_status = 'READY_TO_RUN' THEN '░░░░░░░░░░░░░░░░░░░░ 0 %'
                WHEN run_status = 'RUNNING' THEN '██████████░░░░░░░░░░ 50 %'
                ELSE '█████░░░░░░░░░░░░░░░ 25 %'
            END AS progress_bar,
            COALESCE(last_result, 'Zatím bez výsledku.') AS what_happened,
            CASE
                WHEN run_status = 'DONE' THEN 'Vše vypadá dokončené. Můžeš pokračovat další READY akcí.'
                WHEN run_status = 'FAILED' THEN 'Zkontroluj chybu. Použij RETRY / READY, TEST nebo BLOKOVAT podle detailu.'
                WHEN run_status = 'READY_TO_RUN' THEN 'Akce je připravená. Vyber řádek a spusť vybranou akci.'
                WHEN run_status = 'RUNNING' THEN 'Akce běží. Vyčkej na dokončení a sleduj log.'
                ELSE 'Ověř stav řádku a podle potřeby obnov PC2 data.'
            END AS what_to_do,
            CASE
                WHEN run_status = 'DONE' THEN 'POKRAČOVAT VE FRONTĚ'
                WHEN run_status = 'FAILED' THEN 'RETRY / TEST / BLOKOVAT'
                WHEN run_status = 'READY_TO_RUN' THEN 'SPUSTIT VYBRANOU AKCI'
                ELSE 'OBNOVIT PC2 DATA'
            END AS next_safe_action,
            last_started_at,
            last_finished_at,
            updated_at
        FROM latest;
        ...
        WITH recent AS (
            ...
            FROM ops.v_pc2_run_command_queue_v2
            ORDER BY
                COALESCE(last_finished_at, last_started_at, updated_at) DESC NULLS LAST,
                id DESC
            LIMIT 8
        )
        SELECT
            ...
            command_title AS current_action,
            CASE
                WHEN run_status = 'DONE' THEN 'HOTOVO'
                WHEN run_status = 'FAILED' THEN 'CHYBA'
                WHEN run_status = 'READY_TO_RUN' THEN 'PŘIPRAVENO'
                ELSE run_status
            END AS what_happened,
            COALESCE(last_result, 'Bez detailu výsledku.') AS operator_note,
            CASE
                WHEN run_status = 'DONE' THEN 'Pokračuj další připravenou akcí.'
                WHEN run_status = 'FAILED' THEN 'Otevři detail, zkontroluj log a použij RETRY nebo BLOKOVAT.'
                WHEN run_status = 'READY_TO_RUN' THEN 'Můžeš spustit, pokud je worker ověřený.'
                ELSE 'Obnov data a ověř stav.'
            END AS what_to_do,
            last_started_at,
            last_finished_at,
            updated_at
        ...
        try:
            self.populate_tree(self.pc2_run_monitor_tree, db_query(latest_sql))
            self.populate_tree(self.pc2_result_tree, db_query(result_sql))
        except Exception as e:
            fallback = [{
                "operator_block": "CHYBA PANELU",
                "operator_status": "CHYBA",
                "what_happened": str(e),
                "what_to_do": "Zkontroluj view ops.v_pc2_run_command_queue_v2 a obnov panel.",
                "next_safe_action": "OBNOVIT PC2 DATA",
            }]
            self.populate_tree(self.pc2_run_monitor_tree, fallback)
            self.populate_tree(self.pc2_result_tree, fallback)
    ...
    def load_pc2_command_center(self):
        ...
        self.load_pc2_operator_run_monitor()

        try:
            self.after(300, lambda: self.populate_tree(self.pc2_actions_tree, db_query(actions_sql)))
        except Exception:
            self.populate_tree(self.pc2_actions_tree, db_query(actions_sql))
