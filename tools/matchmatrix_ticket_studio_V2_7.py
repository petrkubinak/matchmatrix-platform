...
# ============================================================
# MATCHMATRIX - TICKET STUDIO V2.6
# ------------------------------------------------------------
# V2.6:
# - lighter visual grid inspired by web cards
# - compact rows / pill buttons / no hard row lines
# - synced horizontal header + body scroll
# - softer alternating row shades
# - prepared hooks for team hover actions
# ============================================================
...
PILL_BG = "#2A1C57"
PILL_BG_ALT = "#322165"
PANEL_LINE_SOFT = "#2E2058"
PANEL_GLOW = "#1D133B"
TEXT_SOFT = "#CDBEF3"

FONT_GRID_COMPACT = ("Segoe UI", 8)
FONT_GRID_TEAM = ("Segoe UI", 8, "bold")
FONT_PILL = ("Segoe UI", 8, "bold")
FONT_HEADER_COMPACT = ("Segoe UI", 8, "bold")


class TicketStudioV26(TicketStudioV25):
    GRID_BREAKPOINT = 1380
    TABLET_BREAKPOINT = 1020
    DESKTOP_BREAKPOINT = 1500
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.team_menu = None
        super().__init__(root)
        self.root.title("MatchMatrix Ticket Studio V2.6")
    ...
    def build_header(self):
        ...
        self.title_var = tk.StringVar(value="MatchMatrix Ticket Studio V2.6")
        self.subtitle_var = tk.StringVar(value="odlehčený grid • kompaktní řádky • pill akce • sync header")
        self.viewport_var = tk.StringVar(value="layout: desktop | zápasy: grid")

        tk.Label(left, textvariable=self.title_var, bg=BG, fg=TEXT, font=FONT_TITLE, anchor="w").grid(row=0, column=0, sticky="w")
        self.header_hint = tk.Label(left, textvariable=self.subtitle_var, bg=BG, fg=MUTED, font=FONT, anchor="w", justify="left")
        self.header_hint.grid(row=1, column=0, sticky="ew", pady=(2, 0))

        self.viewport_label = tk.Label(self.header, textvariable=self.viewport_var, bg=BG, fg=YELLOW, font=FONT_SMALL, anchor="e")
        self.viewport_label.grid(row=0, column=1, rowspan=2, sticky="ne", padx=(12, 0))

        def resize_header(_event=None):
            total = max(self.header.winfo_width(), 300)
            wrap = max(220, total - 340)
            self.header_hint.configure(wraplength=wrap)
        self.header.bind("<Configure>", resize_header)
        self.root.after(100, resize_header)

    def build_center_panel(self, parent):
        parent.grid_rowconfigure(1, weight=1)
        parent.grid_columnconfigure(0, weight=1)

        top = tk.Frame(parent, bg=CARD, highlightthickness=1, highlightbackground=PANEL_LINE_SOFT)
        top.grid(row=0, column=0, sticky="nsew", pady=(0, 8))
        tk.Label(top, text="Nabídka zápasů", bg=CARD, fg=TEXT, font=FONT_SECTION).pack(anchor="w", padx=12, pady=(10, 4))
        self.center_hint_var = tk.StringVar(value="široké zobrazení: odlehčený grid")
        tk.Label(top, textvariable=self.center_hint_var, bg=CARD, fg=MUTED, font=FONT_SMALL).pack(anchor="w", padx=12, pady=(0, 10))

        self.grid_box = tk.Frame(parent, bg=CARD, highlightthickness=1, highlightbackground=PANEL_LINE_SOFT)
        self.grid_box.grid(row=1, column=0, sticky="nsew")
        self.grid_box.grid_rowconfigure(1, weight=1)
        self.grid_box.grid_columnconfigure(0, weight=1)
        ...
        self.grid_total_width = sum(col[1] for col in self.grid_columns) + 18

        self.header_canvas = tk.Canvas(self.grid_box, bg=CARD, height=30, highlightthickness=0, bd=0)
        self.header_canvas.grid(row=0, column=0, sticky="ew")
        self.match_header = tk.Frame(self.header_canvas, bg=CARD)
        self.header_window = self.header_canvas.create_window((0, 0), window=self.match_header, anchor="nw")
        self.build_match_header(self.match_header)
        ...
        self.match_canvas = tk.Canvas(body, bg=CARD, highlightthickness=0, bd=0)
        self.match_scroll_y = tk.Scrollbar(body, orient="vertical", command=self.match_canvas.yview)
        self.match_scroll_x = tk.Scrollbar(self.grid_box, orient="horizontal", command=self.sync_x_scroll)
        ...
        self.match_window = self.match_canvas.create_window((0, 0), window=self.match_inner, anchor="nw")
        self.match_canvas.configure(yscrollcommand=self.match_scroll_y.set, xscrollcommand=self._on_body_xscroll)
        ...
        def on_canvas_resize(event):
            if self.current_match_mode == "grid":
                ...
                width = max(event.width, 320)
            self.match_canvas.itemconfigure(self.match_window, width=width)
            self.header_canvas.configure(height=30)
            self.header_canvas.itemconfigure(self.header_window, width=max(width, self.grid_total_width))
            self.header_canvas.configure(scrollregion=(0, 0, max(width, self.grid_total_width), 30))

        self.match_canvas.bind("<Configure>", on_canvas_resize)
        self.header_canvas.bind("<Configure>", lambda e: self.header_canvas.itemconfigure(self.header_window, height=30))
    ...
    def build_match_header(self, parent):
        for widget in parent.winfo_children():
            widget.destroy()
        parent.configure(height=30, width=self.grid_total_width)

        x = 0
        for title, width, anchor in self.grid_columns:
            lbl = tk.Label(parent, text=title, bg=CARD, fg=YELLOW, font=FONT_HEADER_COMPACT, anchor=anchor)
            lbl.place(x=x + 2, y=5, width=width - 4, height=20)
            x += width

    def _on_body_xscroll(self, first, last):
        self.match_scroll_x.set(first, last)
        try:
    ...
    def _pill_button(self, parent, text, x, y, width, height, command, enabled=True, subtle=False):
        bg = PILL_BG_ALT if subtle else PILL_BG
        fg = TEXT if enabled else MUTED
        btn = tk.Button(
            parent,
            text=text,
            bg=bg if enabled else parent["bg"],
            fg=fg,
            activebackground=PILL_BG_ALT,
            activeforeground=TEXT,
            font=FONT_PILL,
            relief="flat",
            bd=0,
    ...
    def _row_bg(self, idx: int) -> str:
        return ROW_BG_A if idx % 2 == 0 else ROW_BG_B

    def _attach_row_hover(self, row_frame: tk.Frame, widgets: list[tk.Widget], idx: int):
            ...
            for w in widgets:
                try:
                    if isinstance(w, tk.Frame):
                        if str(w.cget("bg")) in {ROW_BG_A, ROW_BG_B, ROW_BG_HOVER}:
                            w.configure(bg=bg)
                    elif isinstance(w, tk.Label):
                        if str(w.cget("bg")) in {base_bg, ROW_BG_HOVER}:
                            w.configure(bg=bg)
                except Exception:
                    pass
    ...
    def show_team_hover_menu(self, event, row: dict, team_side: str):
        try:
        ...
        team_name = str(row.get("home_team" if team_side == "home" else "away_team", "?"))
        self.team_menu = tk.Menu(self.root, tearoff=0, bg=CARD_2, fg=TEXT, activebackground=PILL_BG, activeforeground=TEXT)
        self.team_menu.add_command(label=f"{team_name}")
        self.team_menu.add_separator()
        self.team_menu.add_command(label="Forma (připravit)")
        self.team_menu.add_command(label="Tabulka (připravit)")
        self.team_menu.add_command(label="H2H (připravit)")
        self.team_menu.add_command(label="Poslední zápasy (připravit)")
        try:
            self.team_menu.tk_popup(event.x_root + 8, event.y_root + 8)
        finally:
            try:
                self.team_menu.grab_release()
            except Exception:
                pass
    ...
    def render_match_rows(self):
        ...
        if self.current_match_mode == "grid":
            ...
            self.header_canvas.grid_remove()
            self.match_scroll_x.grid_remove()
            self.match_inner.configure(width=max(self.match_canvas.winfo_width(), 320))
    ...
    def render_match_row(self, row: dict, row_index: int):
        bg_row = self._row_bg(row_index)
        wrap = tk.Frame(self.match_inner, bg=bg_row, width=self.grid_total_width, height=self.row_height)
        ...
        kickoff = tk.Label(wrap, text=self.fmt_kickoff(row.get("kickoff")), bg=bg_row, fg=TEXT_SOFT, font=FONT_GRID_COMPACT, anchor="w")
        kickoff.place(x=x + 4, y=4, width=82, height=18)
        widgets.append(kickoff)
        x += 86

        league = tk.Label(wrap, text=str(row.get("league_name", "?")), bg=bg_row, fg=MUTED, font=FONT_GRID_COMPACT, anchor="w")
        league.place(x=x + 4, y=4, width=150, height=18)
        widgets.append(league)
        x += 154

        home = tk.Label(wrap, text=str(row.get("home_team", "?")), bg=bg_row, fg=TEXT, font=FONT_GRID_TEAM, anchor="w", cursor="hand2")
        home.place(x=x + 4, y=4, width=142, height=18)
        home.bind("<Enter>", lambda e, r=row: self.show_team_hover_menu(e, r, "home"))
        widgets.append(home)
        x += 146

        away = tk.Label(wrap, text=str(row.get("away_team", "?")), bg=bg_row, fg=TEXT, font=FONT_GRID_TEAM, anchor="w", cursor="hand2")
        away.place(x=x + 4, y=4, width=142, height=18)
        away.bind("<Enter>", lambda e, r=row: self.show_team_hover_menu(e, r, "away"))
        widgets.append(away)
        x += 146

        for code, odd_value, width, market in [
            ("1", row.get("odd_1"), 48, "H2H"),
            ("X", row.get("odd_x"), 48, "H2H"),
            ("2", row.get("odd_2"), 48, "H2H"),
            ("1X", dc_odds["1X"], 52, "DC"),
            ("12", dc_odds["12"], 52, "DC"),
            ("X2", dc_odds["X2"], 52, "DC"),
        ]:
            self._pill_button(
                ...
                width - 6,
                20,
                command=lambda m=market, c=code, o=odd_value, r=row: self.add_fixed_pick(r, m, c, o),
                enabled=bool(odd_value),
                subtle=(market == "DC"),
            )
            x += width

        for text, width in (("—", 60), ("—", 58), ("—", 52), ("—", 52)):
            lbl = tk.Label(wrap, text=text, bg=bg_row, fg=MUTED, font=FONT_GRID_COMPACT, anchor="center")
            lbl.place(x=x, y=4, width=width, height=18)
            widgets.append(lbl)
            x += width

        for block_index, width in ((1, 34), (2, 34), (3, 34)):
            self._pill_button(
                wrap,
                self.block_label(block_index),
                x + 2,
                3,
                width - 6,
                20,
                command=lambda bi=block_index, r=row: self.add_to_block(r, bi),
                subtle=True,
            )
            x += width

        self._attach_row_hover(wrap, widgets, row_index)

    def render_match_card(self, row: dict, row_index: int):
        bg_row = self._row_bg(row_index)
        card = tk.Frame(self.match_inner, bg=bg_row)
        card.pack(fill="x", padx=8, pady=6)
        card.grid_columnconfigure(0, weight=1)
        ...
        home_away = tk.Label(top, text=f"{row.get('home_team', '?')} vs {row.get('away_team', '?')}", bg=bg_row, fg=TEXT, font=FONT_BOLD, anchor="w")
        home_away.grid(row=0, column=0, sticky="w")
        tk.Label(top, text=self.fmt_kickoff(row.get("kickoff")), bg=bg_row, fg=YELLOW, font=FONT_SMALL, anchor="e").grid(row=0, column=1, sticky="e")
        tk.Label(top, text=str(row.get("league_name", "?")), bg=bg_row, fg=MUTED, font=FONT_SMALL, anchor="w").grid(row=1, column=0, columnspan=2, sticky="w", pady=(2, 0))
        ...
        dc_odds = self.compute_double_chance_odds(row.get("odd_1"), row.get("odd_x"), row.get("odd_2"))
        primary = [("1", row.get("odd_1"), "H2H"), ("X", row.get("odd_x"), "H2H"), ("2", row.get("odd_2"), "H2H")]
        secondary = [("1X", dc_odds["1X"], "DC"), ("12", dc_odds["12"], "DC"), ("X2", dc_odds["X2"], "DC")]

        for col, (code, odd_value, market) in enumerate(primary):
            box = tk.Frame(odds_wrap, bg=bg_row)
            box.grid(row=0, column=col, sticky="ew", padx=2, pady=2)
            tk.Label(box, text=code, bg=bg_row, fg=YELLOW if odd_value else MUTED, font=FONT_SMALL).pack(pady=(2, 0))
            tk.Button(box, text=self.fmt_odds(odd_value), bg=PILL_BG, fg=TEXT if odd_value else MUTED, font=FONT_SMALL, relief="flat", bd=0, state="normal" if odd_value else "disabled", command=lambda m=market, c=code, o=odd_value, r=row: self.add_fixed_pick(r, m, c, o)).pack(fill="x", padx=4, pady=(2, 4))

        for col, (code, odd_value, market) in enumerate(secondary):
            box = tk.Frame(odds_wrap, bg=bg_row)
            box.grid(row=1, column=col, sticky="ew", padx=2, pady=2)
            tk.Label(box, text=code, bg=bg_row, fg=YELLOW if odd_value else MUTED, font=FONT_SMALL).pack(pady=(2, 0))
            tk.Button(box, text=self.fmt_odds(odd_value), bg=PILL_BG_ALT, fg=TEXT if odd_value else MUTED, font=FONT_SMALL, relief="flat", bd=0, state="normal" if odd_value else "disabled", command=lambda m=market, c=code, o=odd_value, r=row: self.add_fixed_pick(r, m, c, o)).pack(fill="x", padx=4, pady=(2, 4))
        ...
        for col in range(4):
            info.grid_columnconfigure(col, weight=1)
        for col, label in enumerate(("Pred: —", "Forma: —", "Tab: —", "H2H: —")):
            tk.Label(info, text=label, bg=CARD, fg=MUTED, font=FONT_SMALL, padx=6, pady=5).grid(row=0, column=col, sticky="ew", padx=2, pady=2)
        ...
        actions.grid_columnconfigure(2, weight=1)
        for col, block_index in enumerate((1, 2, 3)):
            tk.Button(actions, text=f"Blok {self.block_label(block_index)}", bg=PILL_BG_ALT, fg=TEXT, font=FONT_SMALL, relief="flat", bd=0, command=lambda bi=block_index, r=row: self.add_to_block(r, bi)).grid(row=0, column=col, sticky="ew", padx=3)
    ...
    def _apply_responsive_state(self):
        ...
        if mode_changed:
            self.current_match_mode = target_match_mode
            self.center_hint_var.set("kompaktní zobrazení: karty" if target_match_mode == "cards" else "široké zobrazení: odlehčený grid")
            self.render_match_rows()

        if target_layout == "mobile":
            self.subtitle_var.set("mobile režim • sekce pod sebou • karty a kompaktní akce")
        else:
            self.subtitle_var.set("odlehčený grid • kompaktní řádky • pill akce • sync header")
        self.viewport_var.set(f"layout: {self.current_layout_mode} | zápasy: {self.current_match_mode} | {width}x{height}")



# ============================================================
# MATCHMATRIX - TICKET STUDIO V2.7 STABLE
# ------------------------------------------------------------
# V2.7 STABLE:
# - fixed desktop layout only
# - no responsive rebuilds on resize
# - optimized compact grid for faster rendering
# - softer row shading, no hard row borders
# - synced horizontal header + body scroll
# - simple team hover menu hook
# ============================================================

ROW_BG_S_A = "#150F2C"
ROW_BG_S_B = "#1A1233"
ROW_BG_S_HOVER = "#221845"
PILL_BG_S = "#2A1C57"
PILL_BG_S_ALT = "#332166"
TEXT_SOFT_S = "#CDBEF3"
FONT_TITLE_S = ("Segoe UI", 15, "bold")
FONT_GRID_S = ("Segoe UI", 8)
FONT_GRID_TEAM_S = ("Segoe UI", 8, "bold")
FONT_HEADER_S = ("Segoe UI", 8, "bold")
FONT_PILL_S = ("Segoe UI", 8, "bold")


class TicketStudioV27(TicketStudioV24):
    def __init__(self, root: tk.Tk):
        self.row_height = 28
        self.team_menu = None
        super().__init__(root)
        self.root.title("MatchMatrix Ticket Studio V2.7 STABLE")
        try:
    ...
    def build_header(self):
        ...
        tk.Label(
            self.header,
            text="MatchMatrix Ticket Studio V2.7 STABLE",
            bg=BG,
            fg=TEXT,
            font=FONT_TITLE_S,
            anchor="w",
        ...
        tk.Label(
            self.header,
            text="desktop full-screen • stabilní rychlá verze • kompaktní grid • bez živého přepínání layoutu",
            bg=BG,
            fg=MUTED,
            font=FONT,
            anchor="w",
        ...
        self.viewport_var = tk.StringVar(value="desktop | stable grid")
        tk.Label(
            self.header,
            textvariable=self.viewport_var,
            bg=BG,
        ...
        def update_viewport(_event=None):
            self.viewport_var.set(f"desktop | stable grid | {self.root.winfo_width()}x{self.root.winfo_height()}")

        self.root.bind("<Map>", update_viewport, add="+")
        self.root.bind("<Configure>", self._on_root_configure_stable, add="+")

    def _on_root_configure_stable(self, event):
    ...
    def build_ui(self):
        ...
        self.main_paned.add(self.left_panel, minsize=250)
        self.main_paned.add(self.center_panel, minsize=760)
        self.main_paned.add(self.right_panel, minsize=340)
    ...
    def init_pane_sizes(self):
        try:
            total = max(self.main_paned.winfo_width(), 1200)
            left_w = max(280, int(total * 0.19))
            center_w = max(820, int(total * 0.49))
            self.main_paned.sashpos(0, left_w)
            self.main_paned.sashpos(1, left_w + center_w)
        except Exception:
            pass
    ...
    def build_center_panel(self, parent):
        ...
        tk.Label(
            top,
            text="stabilní kompaktní zobrazení • 1 zápas = 1 řádek • připraveno pro Pred / Forma / Tab / H2H",
            bg=CARD,
            fg=MUTED,
            font=FONT_SMALL
        ...
        self.header_canvas = tk.Canvas(self.grid_box, bg=CARD, height=28, highlightthickness=0, bd=0)
        self.header_canvas.grid(row=0, column=0, sticky="ew")
        self.match_header = tk.Frame(self.header_canvas, bg=CARD, width=self.grid_total_width, height=28)
        self.header_window = self.header_canvas.create_window((0, 0), window=self.match_header, anchor="nw")
        self.build_match_header(self.match_header)
        ...
        def on_canvas_resize(event):
            width = max(event.width, self.grid_total_width)
            self.match_canvas.itemconfigure(self.match_window, width=width)
            self.header_canvas.itemconfigure(self.header_window, width=width)
            self.header_canvas.configure(scrollregion=(0, 0, width, 28))
    ...
    def build_match_header(self, parent):
        for widget in parent.winfo_children():
            widget.destroy()
        parent.configure(height=28, width=self.grid_total_width)
        ...
        for title, width, anchor in self.grid_columns:
            tk.Label(
                ...
                font=FONT_HEADER_S,
                anchor=anchor
            ).place(x=x + 2, y=4, width=width - 4, height=18)
            x += width
    ...
    def _row_bg(self, idx: int) -> str:
        return ROW_BG_S_A if idx % 2 == 0 else ROW_BG_S_B

    def _pill_button(self, parent, text, x, y, width, height, command, enabled=True, subtle=False):
        bg = PILL_BG_S_ALT if subtle else PILL_BG_S
        fg = TEXT if enabled else MUTED
        btn = tk.Button(
            parent,
            text=text,
            bg=bg if enabled else parent["bg"],
            fg=fg,
            activebackground=PILL_BG_S_ALT,
            activeforeground=TEXT,
            font=FONT_PILL_S,
            relief="flat",
            bd=0,
    ...
    def _attach_hover(self, row_frame: tk.Frame, widgets: list[tk.Widget], idx: int):
        base_bg = self._row_bg(idx)
        ...
        def apply(bg):
            ...
            for w in widgets:
                try:
                    if isinstance(w, tk.Label) and str(w.cget("bg")) in {base_bg, ROW_BG_S_HOVER}:
                        w.configure(bg=bg)
                except Exception:
                    pass

        def on_enter(_e=None):
            apply(ROW_BG_S_HOVER)
    ...
    def show_team_hover_menu(self, event, row: dict, team_side: str):
        ...
        team_name = str(row.get("home_team" if team_side == "home" else "away_team", "?"))
        self.team_menu = tk.Menu(self.root, tearoff=0, bg=CARD_2, fg=TEXT, activebackground=PILL_BG_S, activeforeground=TEXT)
        self.team_menu.add_command(label=team_name)
        self.team_menu.add_separator()
        self.team_menu.add_command(label="Forma (připravit)")
        self.team_menu.add_command(label="Tabulka (připravit)")
        self.team_menu.add_command(label="H2H (připravit)")
        self.team_menu.add_command(label="Poslední zápasy (připravit)")
        try:
            self.team_menu.tk_popup(event.x_root + 6, event.y_root + 6)
        finally:
            try:
                self.team_menu.grab_release()
            except Exception:
    ...
    def render_match_row(self, row: dict, row_index: int):
        ...
        kickoff = tk.Label(wrap, text=self.fmt_kickoff(row.get("kickoff")), bg=bg_row, fg=TEXT_SOFT_S, font=FONT_GRID_S, anchor="w")
        kickoff.place(x=x + 4, y=5, width=82, height=18)
        widgets.append(kickoff)
        x += 86

        league = tk.Label(wrap, text=str(row.get("league_name", "?")), bg=bg_row, fg=MUTED, font=FONT_GRID_S, anchor="w")
        league.place(x=x + 4, y=5, width=146, height=18)
        widgets.append(league)
        x += 150

        home = tk.Label(wrap, text=str(row.get("home_team", "?")), bg=bg_row, fg=TEXT, font=FONT_GRID_TEAM_S, anchor="w", cursor="hand2")
        home.place(x=x + 4, y=5, width=140, height=18)
        home.bind("<Enter>", lambda e, r=row: self.show_team_hover_menu(e, r, "home"))
        widgets.append(home)
        x += 144

        away = tk.Label(wrap, text=str(row.get("away_team", "?")), bg=bg_row, fg=TEXT, font=FONT_GRID_TEAM_S, anchor="w", cursor="hand2")
        away.place(x=x + 4, y=5, width=140, height=18)
        away.bind("<Enter>", lambda e, r=row: self.show_team_hover_menu(e, r, "away"))
        widgets.append(away)
        x += 144

        for code, odd_value, width, market in [
            ("1", row.get("odd_1"), 50, "H2H"),
            ("X", row.get("odd_x"), 50, "H2H"),
            ("2", row.get("odd_2"), 50, "H2H"),
            ("1X", dc_odds["1X"], 54, "DC"),
            ("12", dc_odds["12"], 54, "DC"),
            ("X2", dc_odds["X2"], 54, "DC"),
        ]:
        ...
        for text, width in (("—", 58), ("—", 54), ("—", 48), ("—", 48)):
            lbl = tk.Label(wrap, text=text, bg=bg_row, fg=MUTED, font=FONT_GRID_S, anchor="center")
            lbl.place(x=x, y=5, width=width, height=18)
            widgets.append(lbl)
            x += width
        ...
        self._attach_hover(wrap, widgets, row_index)
