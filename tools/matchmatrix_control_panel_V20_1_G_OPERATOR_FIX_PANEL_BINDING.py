"""
MATCHMATRIX CONTROL PANEL V20.1.G OPERATOR FIX PANEL BINDING
========================================================
        ...
        try:
            ...
            if stop_error:
                ...
                if fix_recommendation:
                    fix_code = fix_recommendation.get("fix_code") or "-"
                    button = fix_recommendation.get("operator_button_cz") or "OPRAVIT"
                    confidence = fix_recommendation.get("confidence_pct") or "-"
                    auto_flag = "ANO" if fix_recommendation.get("auto_executable") else "NE"
                    fix = (
                        f"{fix_recommendation.get('fix_title_cz') or fix}\n"
                        f"Tlačítko: {button} | Fix: {fix_code} | Důvěra: {confidence}% | Auto: {auto_flag}"
                    )
