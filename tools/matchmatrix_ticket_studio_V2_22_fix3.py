...
class TicketStudioV223(TicketStudioV220):
    def __init__(self, root: tk.Tk):
        super().__init__(root)
        self.root.title("MatchMatrix Ticket Studio V2.22 fix3")
        if hasattr(self, "viewport_var"):
            self.viewport_var.set("desktop | selection fix first | metrics below | live selection | fix3")
        self.root.after(200, self.adjust_left_panel_width)

    def _sport_icon_for_item(self, item: dict) -> str:
        code = str(item.get("sport_code") or "").upper()
        sport_map = {
            "FB": "⚽",
            "HK": "🏒",
            "TN": "🎾",
        ...
        return sport_map.get(code, "⚽")

    def build_header(self):
        if not hasattr(self, "viewport_var"):
            self.viewport_var = tk.StringVar(value="desktop | selection fix first | metrics below | live selection | fix3")
        self.header = tk.Frame(self.root, bg=BG)
        self.header.pack(fill="x", padx=8, pady=(8, 6))
        self.header.grid_columnconfigure(0, weight=1)
        tk.Label(self.header, text="MatchMatrix Ticket Studio V2.22 fix3", bg=BG, fg=TEXT, font=("Segoe UI", 15, "bold"), anchor="w").grid(row=0, column=0, sticky="w")
