...
class TicketStudioV3:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("MatchMatrix Ticket Studio V3")
        ...
        self.viewport_var = tk.StringVar(value="desktop | unified V3")
    ...
    def build_ui(self):
        ...
        self.main_paned.add(self.left_panel, minsize=190)
        self.main_paned.add(self.center_panel, minsize=820)
        self.main_paned.add(self.right_panel, minsize=420)
    ...
    def init_pane_sizes(self):
        try:
            ...
            if total <= 1:
                return
            left_w = 210
            center_w = max(850, int(total * 0.60))
    ...
    def build_header(self):
        ...
        tk.Label(
            header,
            text="jednosouborová stabilní základna • výběr zápasů • template • runtime • přehled tiketů",
            bg=BG,
            fg=MUTED,
            font=FONT_SMALL,
            anchor="w",
    ...
    def build_center_panel(self, parent):
        ...
        tk.Label(top, text="klik na kurz = FIX • klik na A/B/C = přidání zápasu do bloku", bg=CARD, fg=MUTED, font=FONT_SMALL).pack(anchor="w", padx=10, pady=(0, 8))
    ...
    def build_controls_panel(self, parent):
        ...
        tk.Label(frame, textvariable=self.runtime_status_var, bg=TICKET_BG, fg=TEXT, font=FONT_SMALL, anchor="w", justify="left", wraplength=420).pack(fill="x", padx=10, pady=(0, 6))

        log_wrap = tk.Frame(frame, bg=TICKET_BG)
        log_wrap.pack(fill="both", expand=True, padx=10, pady=(0, 10))
        self.runtime_text = tk.Text(log_wrap, height=10, bg=CARD_2, fg=TEXT, insertbackground=TEXT, wrap="word", relief="flat", font=FONT_MONO)
    ...
    def load_leagues_and_matches(self, initial: bool):
        ...
        sql = """
            ...
            LEFT JOIN public.sports sp ON sp.id = m.sport_id OR sp.id = l.sport_id
        ...
        sql += " ORDER BY l.name, m.kickoff ASC NULLS LAST, ht.name, at.name LIMIT 1000"
    ...
    def render_match_row(self, row: dict, row_index: int):
        ...
        tk.Label(frame, text=match_text, bg=bg, fg=TEXT, font=FONT_SMALL, anchor="w").place(x=x + 8, y=0, width=410, height=46)
    ...
    def _render_fixed_item_card(self, item: PickItem, remove_cmd):
        ...
        top = tk.Frame(card, bg=TICKET_CARD)
        top.pack(fill="x", padx=8, pady=(8, 4))
        tk.Label(top, text=f"{self.sport_icon(item.sport_code)} {item.home_team} - {item.away_team}", bg=TICKET_CARD, fg=TEXT, font=FONT_BOLD, anchor="w").pack(side="left")
        tk.Button(top, text="✕", bg=TICKET_CARD, fg=RED, relief="flat", font=FONT_BOLD_XS, command=remove_cmd).pack(side="right")
        tk.Label(card, text=f"{item.league_name} • {self.fmt_kickoff(item.kickoff)}", bg=TICKET_CARD, fg=MUTED, font=FONT_XS, anchor="w").pack(fill="x", padx=8)
        meta = tk.Frame(card, bg=TICKET_CARD)
        meta.pack(fill="x", padx=8, pady=(6, 8))
        tk.Label(meta, text=f"FIX {item.outcome_code}", bg=ODD_SELECTED, fg=ODD_SELECTED_TEXT, font=FONT_BOLD_XS, padx=8, pady=3).pack(side="left")
        tk.Label(meta, text=f"@ {self.fmt_odds(item.odd_value)}", bg=CARD_2, fg=TEXT, font=FONT_XS, padx=8, pady=3).pack(side="left", padx=(6, 0))
    ...
    def _render_block_item_card(self, item: PickItem, block_index: int, remove_cmd):
        ...
        top.pack(fill="x", padx=8, pady=(8, 4))
        tk.Label(top, text=f"{self.sport_icon(item.sport_code)} {item.home_team} - {item.away_team}", bg=bg_card, fg=TEXT, font=FONT_BOLD, anchor="w").pack(side="left")
        tk.Button(top, text="✕", bg=bg_card, fg=RED, relief="flat", font=FONT_BOLD_XS, command=remove_cmd).pack(side="right")
        tk.Label(card, text=f"{item.league_name} • {self.fmt_kickoff(item.kickoff)}", bg=bg_card, fg=MUTED, font=FONT_XS, anchor="w").pack(fill="x", padx=8)
        badge_bg, badge_fg = BLOCK_COLORS[block_index]
        meta = tk.Frame(card, bg=bg_card)
        meta.pack(fill="x", padx=8, pady=(6, 8))
        tk.Label(meta, text=f"Blok {self.block_label(block_index)}", bg=badge_bg, fg=badge_fg, font=FONT_BOLD_XS, padx=8, pady=3).pack(side="left")
        tk.Label(meta, text="výstup 1/X/2 se určí až v kombinacích", bg=CARD_2, fg=TEXT, font=FONT_XS, padx=8, pady=3).pack(side="left", padx=(6, 0))
            ...
            for row in fixed_rows:
                self.fixed_items.append(
                    PickItem(
                        ...
                        market_outcome_id=int(row["market_outcome_id"]) if row.get("market_outcome_id") is not None else None,
    ...
    def describe_generated_row(self, row: dict) -> dict:
        ...
        fixed_parts = [f"{item.home_team} {item.outcome_code or ''} @{self.fmt_odds(item.odd_value)}" for item in self.fixed_items]
        blocks = {"A": [], "B": [], "C": []}
        probability = None

        # doplnění pravděpodobnosti z generated_tickets, pokud je dostupná
        run_id = self.safe_int(self.last_run_id_var.get(), 0)
        if run_id > 0:
            try:
                p_row = self.fetchone(
                    "SELECT probability FROM public.generated_tickets WHERE run_id = %s AND ticket_index = %s",
                    (run_id, int(row.get("ticket_index") or 0)),
                )
                if p_row:
                    probability = p_row.get("probability")
            except Exception:
                probability = None
        ...
        for bi in (1, 2, 3):
            for item in self.block_items[bi]:
                block_lookup[(bi, item.match_id)] = item

        for entry in items:
            bi = int(entry.get("block_index") or 0)
            label = self.block_label(bi)
            key = (bi, int(entry.get("match_id") or 0))
            local_item = block_lookup.get(key)
            if not local_item:
                continue
            outcome_code = self.lookup_outcome_code(int(entry.get("market_outcome_id") or 0)) or "?"
            odd = entry.get("odd")
            blocks[label].append(f"{outcome_code} @{self.fmt_odds(odd)}")

        return {
            "fixed": " | ".join(fixed_parts) if fixed_parts else "-",
            "A": " | ".join(blocks["A"]) if blocks["A"] else "-",
            "B": " | ".join(blocks["B"]) if blocks["B"] else "-",
            "C": " | ".join(blocks["C"]) if blocks["C"] else "-",
    ...
    def refresh_generated_summary(self):
        ...
        max_win = sum((self.safe_decimal(r.get("possible_win")) or Decimal("0")) for r in selected)
        probs = []
        run_id = self.safe_int(self.last_run_id_var.get(), 0)
        if run_id > 0:
            for row in selected:
                try:
                    ...
                    if p_row and p_row.get("probability") is not None:
                        probs.append(float(p_row["probability"]))
                except Exception:
                    pass
        agg_prob = sum(probs)
    ...
    def export_generated_csv(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            filetypes=[("CSV", "*.csv")],
            initialfile="matchmatrix_ticket_overview_v3.csv",
        )
        if not path:
            return
        try:
