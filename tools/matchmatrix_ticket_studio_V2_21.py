...
class TicketStudioV221(TicketStudioV220):
    def __init__(self, root: tk.Tk):
        super().__init__(root)
        self.root.title("MatchMatrix Ticket Studio V2.21")
        if hasattr(self, "viewport_var"):
            self.viewport_var.set("desktop | ticket grows first | metrics below | live selection")
        self.root.after(200, self.adjust_left_panel_width)

    def build_header(self):
        if not hasattr(self, "viewport_var"):
            self.viewport_var = tk.StringVar(value="desktop | ticket grows first | metrics below | live selection")
        self.header = tk.Frame(self.root, bg=BG)
        self.header.pack(fill="x", padx=8, pady=(8, 6))
        self.header.grid_columnconfigure(0, weight=1)
        tk.Label(self.header, text="MatchMatrix Ticket Studio V2.21", bg=BG, fg=TEXT, font=("Segoe UI", 15, "bold"), anchor="w").grid(row=0, column=0, sticky="w")
