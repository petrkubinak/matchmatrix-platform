...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V18.10 COMMAND CENTER CZ"
        )
    ...
    def build_ui(self):
        ...
        # =========================================================
        # V18.10 - MATCHMATRIX COMMAND CENTER
        # =========================================================
        # CO TO JE:
        # - Nová horní část panelu, která nahrazuje starý blok CORE / PEOPLE / MEDIA / ODDS / CELKEM.
        # - Dominantní ukazatel MATCHMATRIX CELKEM je vlevo.
        # - Vpravo jsou hlavní produktové vrstvy: SPORTY, PROVIDEŘI, PEOPLE, MEDIA, ODDS, WEB.
        #
        # K ČEMU TO JE:
        # - Panel má působit jako řídicí centrum MatchMatrix, ne jako DBeaver dashboard.
        # - Hned po spuštění je jasné, jak daleko je projekt a co brzdí další krok.
        #
        # KDE TO UVIDÍME:
        # - Přímo pod hlavní horní lištou panelu.
        #
        # JAK SE TO VYUŽIJE:
        # - Hodnoty se načítají z OPS/harvest/sport completion/readiness view.
        # - Pokud některé view chybí, panel použije bezpečný fallback a dál běží.
        ...
        command_frame.pack(fill="x", padx=8, pady=(3, 4))
        command_frame.columnconfigure(0, weight=2)
        command_frame.columnconfigure(1, weight=5)
        command_frame.columnconfigure(2, weight=3)
        ...
        self.command_total_frame.grid(row=0, column=0, sticky="nsew", padx=(8, 5), pady=6)
        ...
        # Střed: hlavní produktové vrstvy.
        command_metrics_panel = tk.Frame(command_frame, bg="#0d0716")
        command_metrics_panel.grid(row=0, column=1, sticky="nsew", padx=5, pady=6)

        tk.Label(
            command_metrics_panel,
            text="▣ COMMAND CENTER – STAV HLAVNÍCH OBLASTÍ",
            bg="#0d0716",
            fg=PINK,
            font=("Segoe UI", 9, "bold")
        ).pack(anchor="w", pady=(0, 3))

        command_grid = tk.Frame(command_metrics_panel, bg="#0d0716")
        command_grid.pack(fill="both", expand=True)

        for c in range(6):
            command_grid.columnconfigure(c, weight=1, uniform="command_center")

        self.create_command_center_metric_card(command_grid, 0, "SPORTY", "Sport readiness", 90, GREEN)
        self.create_command_center_metric_card(command_grid, 1, "PROVIDEŘI", "Coverage / health", 95, GREEN)
        self.create_command_center_metric_card(command_grid, 2, "PEOPLE", "Hráči / identity", 72, YELLOW)
        self.create_command_center_metric_card(command_grid, 3, "MEDIA", "Články / video", 61, PURPLE)
        self.create_command_center_metric_card(command_grid, 4, "ODDS", "Kurzy / trhy", 45, RED)
        self.create_command_center_metric_card(command_grid, 5, "WEB", "Aplikace", 12, RED)

        # Pravá část: projektová cesta místo starého čárového grafu.
        project_chart_panel = tk.Frame(command_frame, bg="#0d0716")
        project_chart_panel.grid(row=0, column=2, sticky="nsew", padx=(5, 8), pady=6)

        tk.Label(
            project_chart_panel,
            text="📍 PROJEKTOVÁ CESTA",
            bg="#0d0716",
            fg=PINK,
            font=("Segoe UI", 9, "bold")
        ...
        self.project_chart.pack(fill="both", expand=True, padx=0, pady=(0, 0))
        self.project_chart.bind("<Configure>", lambda event: self.draw_project_timeline_chart())
        ...
        self.kpi_bar.pack(fill="x", pady=(3, 2))
        ...
        # V18.8: zeštíhlená horní KPI lišta.
        # Zůstávají jen hlavní operační metriky; detailní AI/autonomní metriky
        # jsou dál v příslušných záložkách a loadery je ukládají do skrytých handle.
        self.kpi_stav = self.create_kpi("🛡 STAV", "PŘIPRAVEN", GREEN, "Systém")
        self.kpi_pending = self.create_kpi("⏳ FRONTY", "0", YELLOW, "Ke spuštění")
        self.kpi_alerty = self.create_kpi("🔔 ALERTY", "0", RED, "Kritické/var.")
        self.kpi_safe = self.create_kpi("✅ SAFE RUN", "0", PURPLE, "Bezpečné")
        self.kpi_conf = self.create_kpi("📈 AI", "0", PINK, "Důvěra")
        ...
        # Skryté KPI hodnoty – načítají se dál, ale nezabírají místo nahoře.
        ...
        # =========================================================
        # V18.5 UX - DNEŠNÍ PRIORITA + AI DOPORUČENÍ
        # =========================================================
        self.priority_bar = tk.Frame(self, bg="#0d0716")
        self.priority_bar.pack(fill="x", padx=8, pady=(4, 2))
        self.priority_bar.columnconfigure(0, weight=1)
        self.priority_bar.columnconfigure(1, weight=1)
    ...
    def create_command_center_metric_card(self, parent, column, title, subtitle, percent, color):
        """
        V18.10 - malá Command Center karta.

        CO TO JE:
        - Zobrazuje hlavní oblasti projektu: SPORTY, PROVIDEŘI, PEOPLE, MEDIA, ODDS, WEB.

        K ČEMU TO JE:
        - Místo technických KPI ukazuje produktové a projektové oblasti.
        """
        ...
        card = tk.Frame(
            parent,
            bg="#140b1e",
            highlightbackground="#2b1740",
            highlightthickness=1
        )
        card.grid(row=0, column=column, sticky="nsew", padx=3, pady=1)

        top = tk.Frame(card, bg="#140b1e")
        top.pack(fill="x", padx=7, pady=(5, 0))
        ...
        tk.Label(
            ...
            anchor="w"
        ).pack(fill="x", padx=7, pady=(0, 2))

        bar = tk.Canvas(card, height=8, bg="#140b1e", highlightthickness=0)
        bar.pack(fill="x", padx=7, pady=(1, 6))

        def redraw_bar(event=None):
            try:
                bar.delete("all")
                w = max(20, bar.winfo_width())
                h = max(6, bar.winfo_height())
                ...
                bar.create_rectangle(0, h - 4, w, h - 2, fill="#2c2038", outline="#2c2038")
                bar.create_rectangle(0, h - 4, max(2, fill_w), h - 2, fill=color, outline=color)
            except Exception:
                pass
        ...
        for widget in (card, top, value_label, bar):
