...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    """
        ...
        def create_fix_task():
            ...
            fix_text = (
                f"FIX TASK | provider={row.get('provider')} | "
                f"sport={row.get('sport_code')} | "
                f"entity={row.get('entity_type')} | "
                f"endpoint={row.get('endpoint_name')} | "
                f"status={row.get('parse_status')} | "
                f"message={row.get('parse_message')}"
            )
            ...
            messagebox.showinfo(
                "FIX ÚKOL VYTVOŘEN",
                "Fix úkol byl zapsán do LOG SPUŠTĚNÍ."
            )
