...
# =========================================================
        ...
        workflow_action_bar_2 = tk.Frame(documentation_workflow_frame, bg="#100918")
        workflow_action_bar_2.grid(row=2, column=0, columnspan=4, sticky="ew", padx=6, pady=(0, 5))
        self.make_button(workflow_action_bar_2, "📝 OTEVŘÍT KANDIDÁT", "#4c2c83", self.documentation_open_a20_candidate)
        self.make_button(workflow_action_bar_2, "🔎 FINÁLNÍ A17", "#6d45b8", self.documentation_run_final_a17)
        self.make_button(workflow_action_bar_2, "✅ SCHVÁLIT A ULOŽIT", "#0f6a42", self.documentation_approve_and_save_canonical)
        self.make_button(workflow_action_bar_2, "🛡 KANONICKÝ A17", "#0f5f63", self.documentation_run_canonical_a17)
        self.make_button(workflow_action_bar_2, "◆ GIT COMMIT", "#7b4ab8", self.documentation_git_commit)
        self.make_button(workflow_action_bar_2, "📄 OTEVŘÍT REPORT", "#355c8a", self.documentation_open_a17_report)
...
"""
    ...
    def documentation_approve_and_save_canonical(self):
        ...
        try:
            ...
            if any(str(item.get("result", "")).upper() in {"FAIL", "PARTIAL"} for item in findings):
                raise RuntimeError("Finální A17 obsahuje FAIL nebo PARTIAL. Schválení je zablokováno.")
            ...
            if not messagebox.askyesno("Schválení dokumentu", f"Potvrzuješ finální obsah a terminologii dokumentu {document_id}?\n\nDokument bude uložen jako APPROVED do kanonické složky."):
                return
            approved_text = re.sub(r"(?m)^\| Stav \| DRAFT[^|]*\|$", "| Stav | APPROVED |", text_value, count=1)
            ...
            if document_type == "CHAT_CONTINUATION":
                target_dir = os.path.join(DOCUMENTATION_ROOT, "docs", "09_HISTORY", "NAVÁZÁNÍ_NA_CHAT")
                suffix = "MATCHMATRIX_NAVAZANI_DO_CHATU.md"
            elif document_type == "DAILY_LOG":
                target_dir = os.path.join(DOCUMENTATION_ROOT, "docs", "09_HISTORY", "DENNÍ_ZÁPISY")
                suffix = "MATCHMATRIX_DENNI_ZAPIS.md"
            else:
                raise RuntimeError(f"Kanonické uložení typu {document_type!r} zatím panel nepodporuje.")
            filename = f"{document_id}_{suffix}"
            ...
            if os.path.exists(canonical_path) and Path(canonical_path).read_bytes() != encoded:
                raise RuntimeError(f"Kanonický soubor již existuje s jiným obsahem:\n{canonical_path}")
    ...
    def documentation_git_commit(self):
        ...
        try:
            ...
            powershell_script = (
                ...
                '        if ($LASTEXITCODE -eq 0) { throw "Pro vybrany dokument nejsou zadne zmeny ke commitu." }\n'
    ...
    def _documentation_finish_git_commit(self, success, output_text):
        self.documentation_workflow_running = False
        self.documentation_workflow_process = None
        match = re.search(r"__MM_GIT_COMMIT__=([0-9a-fA-F]+)", output_text or "")
        if success and match:
            commit_hash = match.group(1)
            self.documentation_workflow_git_commit = commit_hash
            self.documentation_workflow_step = "GIT COMMIT"
            self.documentation_workflow_last_status = f"COMMIT HOTOV: {commit_hash[:8]}"
            self.documentation_workflow_last_output = output_text
            self._documentation_manifest_update(workflow_status="GIT_COMMITTED", git_commit=commit_hash, canonical_document=self.documentation_workflow_canonical_document)
            self._documentation_update_workflow_ui()
            messagebox.showinfo("Git commit", f"Dokument byl commitnut.\n\nCommit: {commit_hash}\n\nPush nebyl spuštěn.")
        else:
            self.documentation_workflow_step = "GIT COMMIT"
