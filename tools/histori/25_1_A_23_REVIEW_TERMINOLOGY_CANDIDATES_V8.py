...
def _append_history_row(text: str, version: str, description: str) -> str:
    lines = text.splitlines()
    history_heading = _find_heading_index(lines, r"^#{1,3}\s*4\.\s*Historie verzí\s*$")
    if history_heading is None:
        history_heading = _find_heading_index(lines, r"^#{1,3}\s*Historie verzí\s*$")
    if history_heading is None:
        raise ValueError("Dokument neobsahuje sekci Historie verzí.")

    table_header = None
    for index in range(history_heading + 1, min(len(lines), history_heading + 20)):
        cells = split_markdown_row(lines[index])
        if len(cells) >= 3 and normalize_term(cells[0]) == "verze":
            table_header = index
            break
    if table_header is None:
        raise ValueError("Sekce Historie verzí neobsahuje očekávanou tabulku.")
    ...
    lines.insert(insert_at, f"| {version} | {today} | {_escape_md_cell(description)} |")
    return "\n".join(lines).rstrip() + "\n"
