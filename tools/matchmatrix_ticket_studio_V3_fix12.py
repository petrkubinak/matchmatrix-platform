...
class TicketStudioV3:
    ...
    def _render_form_boxes(self, parent, form_value: Optional[str], limit: int):
        ...
        if not tokens:
            tk.Label(wrap, text="-", bg=CARD, fg=MUTED, font=FONT_SMALL).pack(side="left")
            return wrap
        for token in tokens:
            label, bg_box, fg_box = mapping.get(token, (token, CARD_2, TEXT))
            tk.Label(wrap, text=label, bg=bg_box, fg=fg_box, font=FONT_BOLD_XS, width=2, pady=2).pack(side="left", padx=(0, 4))
        return wrap
    ...
    def _build_match_info_overview(self, parent, detail: dict):
        ...
        tk.Label(info, text="Rychlý panel", bg=CARD, fg=YELLOW, font=FONT_SECTION).pack(anchor="w", padx=10, pady=(8, 8))
        ...
        for col, txt in enumerate(headers):
            tk.Label(grid, text=txt, bg=CARD, fg=YELLOW if col else MUTED, font=FONT_BOLD_XS if col else FONT_XS).grid(row=0, column=col, sticky="w", padx=6, pady=(0, 6))
        ...
        for ridx, (label, home_val, away_val) in enumerate(summary_rows, start=1):
            tk.Label(grid, text=label, bg=CARD, fg=MUTED, font=FONT_SMALL).grid(row=ridx, column=0, sticky="w", padx=6, pady=4)
            tk.Label(grid, text=home_val, bg=CARD_2, fg=TEXT, font=FONT_SMALL, padx=8, pady=4).grid(row=ridx, column=1, sticky="ew", padx=6, pady=4)
            tk.Label(grid, text=away_val, bg=CARD_2, fg=TEXT, font=FONT_SMALL, padx=8, pady=4).grid(row=ridx, column=2, sticky="ew", padx=6, pady=4)

        h2h = tk.Frame(info, bg=CARD)
        h2h.pack(fill="x", padx=10, pady=(0, 8))
        tk.Label(h2h, text="H2H", bg=CARD, fg=MUTED, font=FONT_SMALL, width=12, anchor="w").pack(side="left")
        tk.Label(h2h, text=f"Domácí výhry: {detail.get('home_wins') if detail.get('home_wins') is not None else '-'} | Remízy: {detail.get('draws') if detail.get('draws') is not None else '-'} | Hosté výhry: {detail.get('away_wins') if detail.get('away_wins') is not None else '-'}", bg=CARD, fg=TEXT, font=FONT_SMALL, anchor="w").pack(side="left")
    ...
    def _build_match_info_form(self, parent, detail: dict):
        ...
        def render():
            ...
            tk.Label(content, text=f"Posledních {selected} zápasů", bg=CARD, fg=YELLOW, font=FONT_SECTION).pack(anchor="w", padx=10, pady=(8, 8))
            rows = [
            ...
            for label, form_value, pts in rows:
                ...
                tk.Label(line, text=label, bg=CARD, fg=MUTED, font=FONT_SMALL, width=10, anchor="w").pack(side="left")
                form_wrap = tk.Frame(line, bg=CARD)
                form_wrap.pack(side="left", padx=(0, 8))
                self._render_form_boxes(form_wrap, form_value, selected)
                tk.Label(line, text=f"{pts if pts is not None else '-'} b", bg=CARD_2, fg=TEXT, font=FONT_XS, padx=8, pady=3).pack(side="left")
        ...
        for count in (5, 10, 15):
            ...
            ).pack(side="left", padx=(8 if count == 5 else 4, 0), ipady=3)

        def refresh_buttons():
            for widget in controls.winfo_children()[1:]:
                txt = widget.cget("text")
                active = txt == form_var.get()
                widget.configure(bg=ACCENT if active else CARD_2, fg=BG if active else TEXT)
    ...
    def _build_match_info_table(self, parent, detail: dict):
        ...
        def render_table():
            for child in table_box.winfo_children():
                child.destroy()
            head = tk.Frame(table_box, bg=CARD_2)
            head.pack(fill="x", padx=8, pady=(8, 0))
            cols = [("#", 40), ("Tým", 210), ("Z", 40), ("V", 40), ("R", 40), ("P", 40), ("G", 70), ("RS", 50), ("B", 50), ("Forma", 220)]
            x = 0
            for title, width in cols:
                tk.Label(head, text=title, bg=CARD_2, fg=YELLOW, font=FONT_XS).place(x=x, y=0, width=width, height=30)
                x += width
            body_wrap, body_canvas, body_inner = self.create_scrollable_vertical(table_box, CARD)
            body_wrap.pack(fill="both", expand=True, padx=8, pady=(0, 8))
            for idx, r in enumerate(detail.get("table_rows") or []):
                bg_row = CARD if idx % 2 == 0 else CARD_3
                team_id = r.get("team_id")
                if team_id == detail.get("home_team_id") or team_id == detail.get("away_team_id"):
                    bg_row = "#0E355D"
                rowf = tk.Frame(body_inner, bg=bg_row, height=34)
                ...
                widths = [40, 210, 40, 40, 40, 40, 70, 50, 50]
                x = 0
                for val, width in zip(values, widths):
                    font = FONT_BOLD_XS if width == 210 and (team_id == detail.get("home_team_id") or team_id == detail.get("away_team_id")) else FONT_XS
                    tk.Label(rowf, text=val, bg=bg_row, fg=TEXT, font=font, anchor="w" if width == 210 else "center").place(x=x + (4 if width == 210 else 0), y=0, width=width - (4 if width == 210 else 0), height=34)
                    x += width
                form_host = tk.Frame(rowf, bg=bg_row)
                form_host.place(x=x + 6, y=5, width=210, height=24)
                self._render_form_boxes(form_host, form_for_row(r), int(form_var.get()))

        scope_buttons = []
        for label in ("CELKEM", "DOMA", "VENKU"):
            b = tk.Button(controls, text=label, bg=CARD_2, fg=TEXT, relief="flat", font=FONT_BOLD_XS, command=lambda s=label: [scope_var.set(s), refresh_scope(), render_table()])
            b.pack(side="left", padx=(8 if label == "CELKEM" else 4, 0), ipady=3)
            scope_buttons.append(b)
        form_buttons = []
        for count in (5, 10, 15):
            b = tk.Button(controls, text=str(count), bg=CARD_2, fg=TEXT, relief="flat", font=FONT_BOLD_XS, command=lambda c=count: [form_var.set(str(c)), refresh_form(), render_table()])
            b.pack(side="left", padx=(12 if count == 5 else 4, 0), ipady=3)
            form_buttons.append(b)

        def refresh_scope():
            for b in scope_buttons:
                active = b.cget("text") == scope_var.get()
                b.configure(bg=ACCENT if active else CARD_2, fg=BG if active else TEXT)

        def refresh_form():
            for b in form_buttons:
                active = b.cget("text") == form_var.get()
                b.configure(bg=ACCENT if active else CARD_2, fg=BG if active else TEXT)
    ...
    def _build_match_info_h2h(self, parent, detail: dict):
        ...
        tk.Label(card, text="Posledních 5 vzájemných zápasů na tomto hřišti", bg=CARD, fg=YELLOW, font=FONT_SECTION).pack(anchor="w", padx=10, pady=(8, 8))
        summary = tk.Frame(card, bg=CARD)
        summary.pack(fill="x", padx=10, pady=(0, 8))
        tk.Label(summary, text=f"Bilance: {detail.get('home_wins') if detail.get('home_wins') is not None else '-'} / {detail.get('draws') if detail.get('draws') is not None else '-'} / {detail.get('away_wins') if detail.get('away_wins') is not None else '-'}", bg=CARD_2, fg=TEXT, font=FONT_SMALL, padx=8, pady=5).pack(anchor="w")
        ...
        for idx, r in enumerate(rows):
            bg_row = CARD if idx % 2 == 0 else CARD_3
            line = tk.Frame(card, bg=bg_row, height=34)
    ...
    def show_match_info(self, row: dict):
        ...
        tk.Label(odds, text="Kurzy zápasu", bg=CARD, fg=YELLOW, font=FONT_SECTION).grid(row=0, column=0, columnspan=6, sticky="w", padx=10, pady=(8, 6))
        dc = self.compute_double_chance_odds(detail.get('odd_1'), detail.get('odd_x'), detail.get('odd_2'))
        odds_items = (("1", detail.get("odd_1")), ("X", detail.get("odd_x")), ("2", detail.get("odd_2")), ("1X", dc.get('1X')), ("12", dc.get('12')), ("X2", dc.get('X2')))
        for i, (lbl, val) in enumerate(odds_items):
            box = tk.Frame(odds, bg=CARD_2)
            box.grid(row=1 if i < 3 else 2, column=i % 3, sticky="ew", padx=8, pady=(0, 10))
            odds.grid_columnconfigure(i % 3, weight=1)
        ...
        action = tk.Frame(win, bg=BG)
        action.pack(fill='x', padx=12, pady=(0, 8))
        buttons = tk.Frame(action, bg=BG)
        buttons.pack(fill='x')
        for code, color in (("1", ODD_SELECTED), ("X", CARD_2), ("2", ODD_SELECTED)):
            tk.Button(buttons, text=f'Přidat FIX {code}', bg=color, fg=TEXT if code == 'X' else ODD_SELECTED_TEXT, relief='flat', font=FONT_XS,
                      command=lambda c=code: self.toggle_fixed_pick(row, c, detail.get(f'odd_{c.lower()}' if c != 'X' else 'odd_x'))).pack(side='left', padx=(0, 6), ipady=4)
        for bi in (1, 2, 3):
            bg_btn, fg_btn = BLOCK_COLORS[bi]
            tk.Button(buttons, text=f'Do bloku {self.block_label(bi)}', bg=bg_btn, fg=fg_btn, relief='flat', font=FONT_XS,
                      command=lambda b=bi: self.toggle_block_pick(row, b)).pack(side='left', padx=(0, 6), ipady=4)
        ...
        def render_view():
            for child in content.winfo_children():
                child.destroy()
            self._build_match_info_overview(content, detail)
            if view_var.get() == "FORMA":
                self._build_match_info_form(content, detail)
            elif view_var.get() == "TABULKA":
                self._build_match_info_table(content, detail)
            else:
                self._build_match_info_h2h(content, detail)
            for btn in nav_buttons:
                active = btn.cget("text") == view_var.get()
                btn.configure(bg=ACCENT if active else CARD_2, fg=BG if active else TEXT)

        for name in ("FORMA", "TABULKA", "H2H"):
            btn = tk.Button(nav, text=name, bg=CARD_2, fg=TEXT, relief="flat", font=FONT_BOLD_XS, command=lambda n=name: [view_var.set(n), render_view()])
        ...
        tk.Label(summary, text=" ".join(signal) if signal else "Detail panel je připravený, další grafika a barevné prvky doplníme v další verzi.", bg=CARD, fg=TEXT, font=FONT_SMALL, justify="left", wraplength=1040).pack(anchor="w", padx=10, pady=(0, 10))
    ...
    def show_generated_overview(self):
        ...
        for idx, row in enumerate(self.generated_rows):
            ...
            x = 0
            widths = [40, 70, OVERVIEW_FIX_W, OVERVIEW_BLOCK_W, OVERVIEW_BLOCK_W, OVERVIEW_BLOCK_W, OVERVIEW_ODD_W, OVERVIEW_WIN_W, OVERVIEW_PRED_W]
