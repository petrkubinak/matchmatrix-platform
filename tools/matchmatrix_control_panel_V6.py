...
# ============================================================
# MATCHMATRIX CONTROL PANEL V6
# Mission Control style + batch launcher + OPS/DB mini dashboard
# ============================================================
...
class MatchMatrixPanelV6:
    def __init__(self, root: tk.Tk):
        ...
        self.log_write("V6 načten: Mission Control styl + OPS/DB mini dashboard.")
    ...
    def build_header(self, parent) -> None:
        ...
        tk.Label(
            header,
            text="Multi-sport + Multi-entity batch launcher | styl Mission Control | OPS / DB mini dashboard",
            bg=BG,
            fg=MUTED,
            font=("Segoe UI", 10),
    ...
    def build_ops_dashboard(self, parent) -> None:
        ...
        ops_wrap.pack(fill="x", pady=(0, 10))

        row1 = tk.Frame(ops_wrap, bg=BG)
        row1.pack(fill="x", pady=(0, 8))

        self.card_matches = MetricCard(row1, "Zápasy", "-", "public.matches", ACCENT)
        self.card_matches.pack(side="left", fill="both", expand=True, padx=4)

        self.card_leagues = MetricCard(row1, "Ligy", "-", "public.leagues", ACCENT_2)
        self.card_leagues.pack(side="left", fill="both", expand=True, padx=4)

        self.card_teams = MetricCard(row1, "Týmy", "-", "public.teams", ACCENT)
        self.card_teams.pack(side="left", fill="both", expand=True, padx=4)

        self.card_players = MetricCard(row1, "Hráči", "-", "public.players", ACCENT_2)
        self.card_players.pack(side="left", fill="both", expand=True, padx=4)

        row2 = tk.Frame(ops_wrap, bg=BG)
        row2.pack(fill="x", pady=(0, 8))

        self.card_planner_pending = MetricCard(row2, "Planner pending", "-", "ops.ingest_planner", WARN)
        self.card_planner_pending.pack(side="left", fill="both", expand=True, padx=4)

        self.card_planner_running = MetricCard(row2, "Planner running", "-", "ops.ingest_planner", WARN)
        self.card_planner_running.pack(side="left", fill="both", expand=True, padx=4)

        self.card_last_job = MetricCard(row2, "Poslední job", "-", "ops.job_runs", GOOD)
        self.card_last_job.pack(side="left", fill="both", expand=True, padx=4)

        row3 = tk.Frame(ops_wrap, bg=BG)
        row3.pack(fill="x")

        self.bar_db_health = ProgressBarCard(row3, "DB HEALTH", GOOD)
        self.bar_db_health.pack(side="left", fill="both", expand=True, padx=4)

        self.bar_ops_health = ProgressBarCard(row3, "OPS HEALTH", WARN)
        self.bar_ops_health.pack(side="left", fill="both", expand=True, padx=4)
