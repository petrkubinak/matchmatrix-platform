...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V17.11.01 CZ"
        )
        ...
        # V17.11.02
        # Denní limit autonomních akcí na sport

        self.daily_sport_limit = 5

        self.sport_daily_counter = {
            "FB": 0,
            "BK": 0,
            "HK": 0,
            "HB": 0,
    ...
    def build_ui(self):
        ...
        self.kpi_sport_limit = self.create_kpi("🏆 SPORTY DNES", "0", PURPLE, "Denní limit")
...
""",
    ...
    def update_auto_result_counters(self, return_code):
        if (
            return_code == 0
            and self.last_worker_name
        ):

            sport = self.get_sport_from_worker(
                self.last_worker_name
            )

            if sport in self.sport_daily_counter:

                self.sport_daily_counter[sport] += 1
        if return_code == 0:
    ...
    def get_sport_from_worker(self, worker_name):

        worker = str(worker_name).upper()
    ...
    def get_recommended_worker_row(self):
        ...
        for row in rows:
            ...
            score = base_score
            sport_code = self.get_sport_from_worker(worker)

            if sport_code in self.sport_daily_counter:

                used_today = self.sport_daily_counter[sport_code]

                if used_today >= self.daily_sport_limit:

                    score -= 5000

                    reason_parts.append(
                        f"denní limit sportu {sport_code} již vyčerpán"
                    )
            reason_parts = []
