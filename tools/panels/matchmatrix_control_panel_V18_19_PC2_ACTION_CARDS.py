"""
MATCHMATRIX CONTROL PANEL V18.18 PC2 ACTION BUTTONS
========================================================
...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V18.19 PC2 ACTION CARDS CZ"
        )
    ...
    def build_ui(self):
        ...
        command_frame.pack(fill="x", padx=8, pady=(2, 4))
        ...
        action_panel = tk.Frame(command_middle_panel, bg="#0a0610")
        action_panel.grid(row=1, column=0, sticky="nsew")
        ...
        self.content_outer.pack(fill="both", expand=True, padx=8, pady=(4, 2))
        ...
        self.bottom_tabs = tk.Frame(self, bg="#0d0716", height=92)
        self.bottom_tabs.pack(fill="x", side="bottom", padx=8, pady=(2, 6))
        ...
        # =========================================================
        # V18.16: PC2 COMMAND CENTER
        # =========================================================
        tab_pc2_command.columnconfigure(0, weight=1)
        tab_pc2_command.columnconfigure(1, weight=1)
        tab_pc2_command.rowconfigure(1, weight=1)
        tab_pc2_command.rowconfigure(2, weight=1)
        tab_pc2_command.rowconfigure(3, weight=1)

        pc2_button_bar = tk.Frame(tab_pc2_command, bg=BG)
        pc2_button_bar.grid(row=0, column=0, columnspan=2, sticky="ew", padx=4, pady=4)

        self.pc2_run_button = self.make_button(
            pc2_button_bar,
        ...
        self.pc2_button_tree = self.create_section(
            tab_pc2_command, "🚀 PC2 DALŠÍ AKCE / RUN BUTTON", 1, 0
        )

        self.pc2_kpi_tree = self.create_section(
            tab_pc2_command, "📊 PC2 KPI", 1, 1
        )

        self.pc2_queue_tree = self.create_section(
            tab_pc2_command, "🧭 PC2 SPOUŠTĚCÍ FRONTA", 2, 0, 2
        )

        self.pc2_roadmap_tree = self.create_section(
            tab_pc2_command, "🗺 PC2 MASTER ROADMAP", 3, 0
        )

        self.pc2_actions_tree = self.create_section(
            tab_pc2_command, "✅ PC2 DALŠÍ AKCE", 3, 1
        )
...
Další termín: {h.get('next_target_date') or '-'}"""
    ...
    def load_pc2_command_center(self):
        ...
        self.populate_pc2_action_cards(self.pc2_button_tree, db_query(button_sql))
        self.populate_tree(self.pc2_kpi_tree, db_query(kpi_sql))
        self.populate_tree(self.pc2_queue_tree, db_query(queue_sql))
        self.populate_tree(self.pc2_roadmap_tree, db_query(roadmap_sql))
        self.populate_tree(self.pc2_actions_tree, db_query(actions_sql))

        # Aktualizace textu hlavního PC2 tlačítka podle DB.
        try:
            rows = db_query(button_sql)
    ...
    def on_pc2_action_card_select(self, event=None):
        """
        V18.19 - klik na akční kartu synchronizuje výběr s hlavní PC2 frontou.
        """
        try:
    ...
    def get_selected_pc2_command_id(self):
        # V18.19: nejdřív zkus výběr v akčních kartách, potom hlavní spouštěcí frontu.
    ...
    def pc2_set_selected_status(self, new_status):
        """
        V18.18 - ruční přepnutí stavu PC2 příkazu přímo z panelu.
        ...
        """, (
            new_status,
            f"Manual status change from OPS Panel V18.18: {old_status} -> {new_status}",
            command_id,
        ))
    ...
    def pc2_continue_selected_command(self):
        ...
        ok, err = db_execute("""
            ...
            SET
                ...
                last_result = 'Continue pending planner jobs from OPS Panel V18.18.',
                updated_at = now()
            WHERE id = %s;
        """, (command_id,))
    ...
    def pc2_retry_selected_command(self):
        ...
        ok, err = db_execute("""
            ...
            SET
                ...
                last_result = 'Retry requested from OPS Panel V18.18.',
                updated_at = now()
            WHERE id = %s;
        """, (command_id,))
    ...
    def extract_run_group_from_command_text(self, command_text):
        """
        V18.18 - vytáhne --run-group z command_text.
        """
        try:
    ...
    def pc2_reset_selected_planner_pending(self):
        """
        V18.18 - vrátí planner joby vybraného PC2 commandu na pending.
        ...
        ok2, err2 = db_execute("""
            ...
            WHERE id = %s;
        """, (
            f"Planner jobs reset to pending from OPS Panel V18.18 | run_group={run_group}",
            command_id,
        ))
    ...
    def start_pc2_command(self, row, allow_non_ready=False, test_mode=False):
        ...
        """, (
            "TEST started from OPS Panel V18.18" if test_mode else "Started from OPS Panel V18.18",
            command_id,
        ))
...
""".strip()
    ...
    def reflow_command_center(self):
        """
        V18.13:
        - Široké okno: oblasti | KPI+akce | doporučení.
        - Užší okno: oblasti | KPI+akce a doporučení se přesune pod ně.
        """
        if not hasattr(self, "command_frame"):
            return

        try:
            width = self.command_frame.winfo_width()
        except Exception:
            return
        ...
        for child in (getattr(self, "command_metrics_panel", None), getattr(self, "command_middle_panel", None), getattr(self, "priority_bar", None)):
        ...
        for r in range(3):
            self.command_frame.rowconfigure(r, weight=0)
        for c in range(4):
            self.command_frame.columnconfigure(c, weight=0, minsize=0)

        if compact:
            self.command_frame.columnconfigure(0, weight=5, minsize=360)
            self.command_frame.columnconfigure(1, weight=7, minsize=420)
            self.command_frame.rowconfigure(0, weight=0)
            self.command_frame.rowconfigure(1, weight=0)
            self.command_metrics_panel.grid(row=0, column=0, sticky="nsew", padx=(8, 5), pady=5)
            self.command_middle_panel.grid(row=0, column=1, sticky="nsew", padx=(5, 8), pady=5)
            self.priority_bar.grid(row=1, column=0, columnspan=2, sticky="ew", padx=8, pady=(0, 5))
        else:
            self.command_frame.columnconfigure(0, weight=5, minsize=520)
            self.command_frame.columnconfigure(1, weight=6, minsize=620)
            self.command_frame.columnconfigure(2, weight=4, minsize=420)
            self.command_metrics_panel.grid(row=0, column=0, sticky="nsew", padx=(8, 5), pady=5)
            self.command_middle_panel.grid(row=0, column=1, sticky="nsew", padx=5, pady=5)
            self.priority_bar.grid(row=0, column=2, sticky="nsew", padx=(5, 8), pady=5)
    ...
    def reflow_bottom_tabs(self):
        """Spodní záložky se při menším okně zalomí do dvou řad."""
        if not hasattr(self, "bottom_tab_buttons"):
            return

        try:
            width = max(1, self.bottom_tabs.winfo_width())
            count = max(1, len(self.bottom_tab_buttons))
            ...
            per_row = max(4, min(count, per_row))
            ...
            for c in range(per_row):
                self.bottom_tabs.columnconfigure(c, weight=1, uniform="bottom_tabs")

            for index, btn in enumerate(self.bottom_tab_buttons):
                btn.grid(
                    row=index // per_row,
                    column=index % per_row,
                    sticky="nsew",
            ...
            needed_rows = (count + per_row - 1) // per_row
            self.bottom_tabs.configure(height=max(58, needed_rows * 42 + 10))
