...
class TicketStudioV3:
    ...
    def show_match_info(self, row: dict):
        ...
        tk.Label(action, text='Rychlá akce', bg=BG, fg=YELLOW, font=FONT_XS).pack(anchor='w', pady=(0, 4))
        ...
        rows = [
            ...
            ("Body / zápasy", f"Domácí: {detail.get('home_points') if detail.get('home_points') is not None else '-'} b / {detail.get('home_played') if detail.get('home_played') is not None else '-'} z. | Hosté: {detail.get('away_points') if detail.get('away_points') is not None else '-'} b / {detail.get('away_played') if detail.get('away_played') is not None else '-'} z."),
            ("Skóre", f"Domácí: {detail.get('home_goals_for') if detail.get('home_goals_for') is not None else '-'}:{detail.get('home_goals_against') if detail.get('home_goals_against') is not None else '-'} | Hosté: {detail.get('away_goals_for') if detail.get('away_goals_for') is not None else '-'}:{detail.get('away_goals_against') if detail.get('away_goals_against') is not None else '-'}"),
            ("Forma 5", f"Domácí: {detail.get('home_form') or '-'} ({detail.get('home_points_last_5') if detail.get('home_points_last_5') is not None else '-'} b) | Hosté: {detail.get('away_form') or '-'} ({detail.get('away_points_last_5') if detail.get('away_points_last_5') is not None else '-'} b)"),
            ("Forma 10", f"Domácí: {detail.get('home_form_10') or '-'} ({detail.get('home_points_last_10') if detail.get('home_points_last_10') is not None else '-'} b) | Hosté: {detail.get('away_form_10') or '-'} ({detail.get('away_points_last_10') if detail.get('away_points_last_10') is not None else '-'} b)"),
            ("Forma 15", f"Domácí: {detail.get('home_form_15') or '-'} ({detail.get('home_points_last_15') if detail.get('home_points_last_15') is not None else '-'} b) | Hosté: {detail.get('away_form_15') or '-'} ({detail.get('away_points_last_15') if detail.get('away_points_last_15') is not None else '-'} b)"),
        ...
        tk.Label(summary, text=" ".join(signal) if signal else "Detail panel je připravený, další grafika a barevné prvky doplníme v další verzi.", bg=CARD, fg=TEXT, font=FONT_SMALL, justify="left", wraplength=700).pack(anchor="w", padx=10, pady=(0, 10))
