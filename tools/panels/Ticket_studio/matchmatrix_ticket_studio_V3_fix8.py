...
# ============================================================
# MATCHMATRIX TICKET STUDIO V3 FIX8
# ------------------------------------------------------------
...
class TicketStudioV3:
    ...
    def show_generated_overview(self):
        ...
        tk.Label(
            top,
            text="rozpis bloků: součin kurzů včetně detailu výpočtu",
            bg=BG,
            fg=MUTED,
            font=FONT_XS
        ...
        row_height = 54
        widths = [40, 70, 320, 320, 320, 320, 90, 110, 90]
        for idx, row in enumerate(self.generated_rows):
            bg = CARD if idx % 2 == 0 else CARD_3
            line = tk.Frame(inner, bg=bg, height=row_height)
            ...
            for col_idx, width in enumerate(widths):
                if col_idx == 0:
                    tk.Checkbutton(
                        ...
                        activebackground=bg,
                        selectcolor=CARD_2
                    ).place(x=x + 8, y=17, width=24, height=20)
                else:
                    justify = "left" if col_idx in (2, 3, 4, 5) else "center"
                    anchor = "w" if col_idx in (1, 2, 3, 4, 5) else "center"
                    tk.Label(
                        line,
                        ...
                        justify=justify,
                        wraplength=width - 10,
                    ).place(x=x + 4, y=0, width=width - 8, height=row_height)
                x += width
    ...
    def _build_product_detail(self, odds_list: list[Decimal], outcome_code: str | None = None) -> tuple[str, Decimal | None]:
        ...
        for odd in valid:
            product *= odd
            parts.append(self.fmt_odds(odd))
        prefix = f"{outcome_code}: " if outcome_code else ""
        if len(parts) == 1:
            detail = f"{prefix}{parts[0]}"
        else:
            detail = f"{prefix}{' × '.join(parts)} = {self.fmt_odds(product)}"
    ...
    def describe_generated_row(self, row: dict) -> dict:
        ...
        fixed_odds: list[Decimal] = []
        for item in self.fixed_items:
            odd_val = self.safe_decimal(getattr(item, "odd_value", None))
            if odd_val is None or odd_val <= 0:
                continue
            fixed_odds.append(odd_val)

        fixed_detail, fixed_product = self._build_product_detail(fixed_odds, None)

        block_odds = {"A": [], "B": [], "C": []}
        block_codes = {"A": None, "B": None, "C": None}

        for entry in items:
            bi = int(entry.get("block_index") or 0)
            label = self.block_label(bi)
            if label not in block_odds:
                continue
            odd_val = self.safe_decimal(entry.get("odd"))
            if odd_val is None or odd_val <= 0:
                continue
            block_odds[label].append(odd_val)
            mo_id = self.safe_int(entry.get("market_outcome_id"), 0)
            code = self.lookup_outcome_code(mo_id) if mo_id > 0 else None
            if code:
                block_codes[label] = code

        a_detail, a_product = self._build_product_detail(block_odds["A"], block_codes["A"])
        b_detail, b_product = self._build_product_detail(block_odds["B"], block_codes["B"])
        c_detail, c_product = self._build_product_detail(block_odds["C"], block_codes["C"])

        total_product = Decimal("1")
        used = False
        for part in (fixed_product, a_product, b_product, c_product):
        ...
        return {
            "fixed": self.fmt_odds(fixed_product) if fixed_product is not None else "-",
            "A": self.fmt_odds(a_product) if a_product is not None else "-",
            "B": self.fmt_odds(b_product) if b_product is not None else "-",
            "C": self.fmt_odds(c_product) if c_product is not None else "-",
    ...
    def export_generated_csv(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            filetypes=[("CSV", "*.csv")],
            initialfile="matchmatrix_ticket_overview_v3_fix8.csv",
        )
        if not path:
            return
        try:
            ...
            messagebox.showinfo("Export CSV", f"Export hotový.{path}")
        except Exception as e:
