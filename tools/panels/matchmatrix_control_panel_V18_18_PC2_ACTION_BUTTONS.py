...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V18.18 PC2 ACTION BUTTONS CZ"
        )
...
Další termín: {h.get('next_target_date') or '-'}"""
    ...
    def load_pc2_command_center(self):
        ...
        self.populate_tree(self.pc2_button_tree, db_query(button_sql))
        ...
        try:
            ...
            if rows and "CHYBA" not in rows[0]:
                ...
                enabled = bool(row.get("button_enabled"))
    ...
    def get_selected_pc2_command_id(self):
        selected = self.pc2_queue_tree.selection()
        if not selected:
            return None

        item = self.pc2_queue_tree.item(selected[0])
        values = item.get("values", [])
        if not values:
            return None
    ...
    def run_pc2_selected_command(self):
        ...
        if not command_id:
            messagebox.showinfo(
                "PC2 VYBRANÁ AKCE",
                "Nejdřív klikni na řádek v tabulce PC2 SPOUŠTĚCÍ FRONTA."
            )
            return
