...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V18.8 SLIM KPI + PEOPLE GOVERNANCE CZ"
        )
    ...
    def draw_project_timeline_chart(self):
        """
        CO TO JE:
        - Klidnější projektový graf V18.9.
        - Místo pěti překřížených křivek ukazuje hlavně CELKEM trend.
        - Ostatní vrstvy jsou zobrazené jako malé aktuální hodnoty vpravo.

        K ČEMU TO JE:
        - Graf je čitelnější a méně rušivý.
        - Hned ukáže, jestli se projekt celkově posouvá nahoru nebo dolů.

        KDE TO UVIDÍME:
        - Horní pruh pod titulkem panelu.

        JAK SE TO VYUŽIJE:
        - Historie se drží v paměti panelu.
        - Poslední hodnoty se aktualizují z DB při refreshi.
        """
        ...
        if width < 300 or height < 55:
            return

        # V18.9: více prostoru pro hlavní trend, méně popisků.
        left = 42
        right = 150
        top = 12
        bottom = 20

        chart_w = max(180, width - left - right)
        chart_h = max(38, height - top - bottom)
        x0 = left
        x1 = left + chart_w
        y0 = top
        y1 = top + chart_h
        ...
        # Aktuální hodnoty z DB/progress cells, fallback na poslední historii.
        current_values = dict(getattr(self, "project_progress_values", {}) or {})
        last_row = history[-1]
        fallback_values = {
            "CORE": last_row[1],
            "PEOPLE": last_row[2],
        ...
        for key, value in fallback_values.items():
            current_values.setdefault(key, value)
        ...
        def point(index, percent):
            x = x0 + (index / (len(history) - 1)) * chart_w
            y = y1 - (clamp_pct(percent) / 100) * chart_h
            return x, y

        # Měkké pozadí a cílová zóna 80+.
        canvas.create_rectangle(
            x0, y0, x1, y1,
            fill="#100819",
            outline="#2a1a39"
        ...
        target_y = y1 - (80 / 100) * chart_h
        canvas.create_rectangle(
            x0 + 1, y0 + 1, x1 - 1, target_y,
            fill="#112118",
            outline=""
        )
        canvas.create_text(
        ...
        # Jednoduchá osa Y: 0 / 50 / 100.
        for pct in (0, 50, 100):
            y = y1 - (pct / 100) * chart_h
            canvas.create_line(x0, y, x1, y, fill="#2b1c38")
            canvas.create_text(
                x0 - 7, y,
                text=f"{pct}%",
                fill="#a995bd",
        ...
        # Osa X pouze první / prostřední / poslední bod.
        label_indexes = sorted(set([0, len(history) // 2, len(history) - 1]))
        for i in label_indexes:
            x, _ = point(i, 0)
            canvas.create_line(x, y0, x, y1, fill="#24162f")
            canvas.create_text(
                x, height - 8,
                text=str(history[i][0]),
                fill="#bca0d4",
                font=("Segoe UI", 7, "bold")
            )

        # V18.9: hlavní trend je CELKEM. Ostatní vrstvy neruší graf.
        coords = []
        ...
        if len(coords) >= 4:
            # Jemná spodní linka pro hloubku.
            shadow = []
            for i in range(0, len(coords), 2):
                shadow.extend([coords[i], coords[i + 1] + 2])
            canvas.create_line(*shadow, fill="#4c1d57", width=4, smooth=True)
            canvas.create_line(*coords, fill=PINK, width=3, smooth=True)

        last_x, last_y = point(len(history) - 1, history[-1][5])
        last_total = int(round(clamp_pct(current_values.get("CELKEM", history[-1][5]))))
        canvas.create_oval(
            last_x - 5, last_y - 5,
            last_x + 5, last_y + 5,
            fill=PINK,
        ...
        # Vpravo: aktuální hodnoty vrstev jako čistý seznam s mini pruhem.
        legend_x = x1 + 16
        legend_y = y0 + 2
        rows = [
            ("CORE", GREEN),
        ...
        for row_index, (name, color) in enumerate(rows):
            y = legend_y + row_index * 13
            value = int(round(clamp_pct(current_values.get(name, 0))))
            bar_w = 48
            ...
            canvas.create_rectangle(
                legend_x + 63, y - 3,
                legend_x + 63 + bar_w, y - 1,
                fill="#2c2038",
                outline="#2c2038"
            )
            canvas.create_rectangle(
                legend_x + 63, y - 3,
                legend_x + 63 + fill_w, y - 1,
                fill=color,
                outline=color
            )
