"""
MATCHMATRIX CONTROL PANEL V19 NAVIGACE / DENNÍ PRÁCE
========================================================
...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V19.3 - ALERT DETAIL + KRÁTKÉ STAVY CZ"
        )
    ...
    def build_ui(self):
        ...
        tk.Label(
            header,
            text="▰ MATCHMATRIX ŘÍDICÍ CENTRUM V19.5",
            bg="#07040d",
            fg=PINK,
        ...
        tab_pc2_command.columnconfigure(0, weight=3)
        tab_pc2_command.columnconfigure(1, weight=4)
        tab_pc2_command.columnconfigure(2, weight=0, minsize=260)
        ...
        self.pc2_button_tree = self.create_section(
            tab_pc2_command, "🏠 DENNÍ PRÁCE – CO SPUSTIT TEĎ", 0, 0
        )

        self.pc2_kpi_tree = self.create_section(
            tab_pc2_command, "📊 STRUČNÝ STAV PC2", 0, 1
        )
        ...
        self.pc2_roadmap_tree = self.create_section(
            tab_pc2_command, "🗺 PC2 MASTER ROADMAP", 2, 0
        )
...
Další termín: {h.get('next_target_date') or '-'}"""
    ...
    def on_pc2_action_card_select(self, event=None):
        """
        V19.4 - klik na akční kartu / frontu synchronizuje výběr a ukáže vpravo, nad čím tlačítka pracují.
        """
        try:
            ...
            if row and hasattr(self, "pc2_selected_action_label"):
                self.pc2_selected_action_label.config(
                    text=(
                        ...
                        f"Akce: {row.get('command_title') or '-'}"
                    )
                )
        except Exception:
            pass
...
""".strip()
    ...
    def populate_tree(self, tree, rows):
        ...
        for row in rows:
            ...
            # V19.3: do Treeview ukládáme plné hodnoty.
            # Viditelné sloupce se oříznou šířkou sloupce, ale dvojklik na řádek ukáže celý detail.
...
""".strip()
    ...
    def reflow_command_center(self):
        ...
        for child in (getattr(self, "command_metrics_panel", None), getattr(self, "command_middle_panel", None), getattr(self, "priority_bar", None), getattr(self, "command_actions_panel", None)):
        ...
        if compact:
            self.command_frame.columnconfigure(0, weight=5, minsize=330)
            self.command_frame.columnconfigure(1, weight=6, minsize=380)
            self.command_frame.columnconfigure(2, weight=2, minsize=230)
            ...
            self.command_actions_panel.grid(row=0, column=2, sticky="nsew", padx=(5, 8), pady=5)
            self.priority_bar.grid(row=1, column=0, columnspan=3, sticky="ew", padx=8, pady=(0, 5))
        else:
            # V19.5: PŘEHLED má vyrovnaný horní layout:
            # hlavní oblasti | KPI | doporučení | rychlé akce vpravo.
            self.command_frame.columnconfigure(0, weight=5, minsize=500)
            self.command_frame.columnconfigure(1, weight=5, minsize=500)
            self.command_frame.columnconfigure(2, weight=4, minsize=380)
            self.command_frame.columnconfigure(3, weight=2, minsize=250)
            self.command_metrics_panel.grid(row=0, column=0, sticky="nsew", padx=(8, 5), pady=5)
            self.command_middle_panel.grid(row=0, column=1, sticky="nsew", padx=5, pady=5)
            self.priority_bar.grid(row=0, column=2, sticky="nsew", padx=5, pady=5)
            self.command_actions_panel.grid(row=0, column=3, sticky="nsew", padx=(5, 8), pady=5)
