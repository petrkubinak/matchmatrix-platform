"""
MATCHMATRIX CONTROL PANEL V18.15 PC2 COMMAND CENTER
========================================================
...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V18.15 PC2 COMMAND CENTER CZ"
        )
...
Další termín: {h.get('next_target_date') or '-'}"""
    ...
    def load_pc2_command_center(self):
        ...
        SELECT
            ...
            priority_label_cs,
            pc2_next_action_cs
        FROM ops.v_pc2_command_center_next_actions_v1
        ORDER BY
            next_harvest_layer,
            sport_code;
        """
    ...
    def start_pc2_command(self, row):
        """
        CO TO JE:
        - Spustí command_text z ops.pc2_run_command_queue.

        K ČEMU TO JE:
        - PC2 Command Center přechází z dashboardu na reálné spouštění.
        """
        ...
        ok, err = db_execute("""
            ...
            SET
                ...
                last_result = 'Started from OPS Panel V18.15',
                updated_at = now()
            WHERE id = %s;
        """, (command_id,))
        ...
        self.log(f"PC2 PŘÍKAZ: {command_text}")
        self.start_worker_activity(f"PC2 {row.get('sport_code')}/{row.get('target_layer')}")
        self.load_pc2_command_center()

        thread = threading.Thread(
            target=self.run_pc2_command_thread,
            args=(int(command_id), command_text),
            daemon=True
        )
        thread.start()

    def run_pc2_command_thread(self, command_id, command_text):
        ...
        try:
            ...
            self.log_queue.put("PŘÍKAZ: " + str(command_text))
