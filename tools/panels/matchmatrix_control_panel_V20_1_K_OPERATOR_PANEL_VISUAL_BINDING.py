"""
MATCHMATRIX CONTROL PANEL V20.1.K OPERATOR PANEL VISUAL BINDING
========================================================
...
V20.1.G:
- DENNÍ PRÁCE dostává první funkční tlačítko OPRAVIT.
- Tlačítko čte doporučenou opravu z ops.v_operator_fix_recommendations_v1.
- Auto-fix se provede přes ops.fn_operator_execute_fix_v1(monitor_id, 'PANEL_OPERATOR').
- Výsledek se zapisuje do ops.operator_fix_execution_log.
- Cíl: chyba → doporučení → jedno kliknutí → READY → pokračovat.

V20.1.K:
- DENNÍ PRÁCE čte horní grafické karty z ops.v_operator_dashboard_summary_v1.
- Panel ukazuje READY / BĚŽÍ / HOTOVO / CHYBA a doporučenou další akci.
- Červený stav má prioritu: nejdříve OPRAVIT, teprve potom SPUSTIT další READY akci.
- Cíl: akční operátorský panel, kde je okamžitě vidět co udělat dál.
"""
...
# V20.1.G - OPERATOR FIX PANEL BINDING
COLUMN_LABELS.update({
    "monitor_id": "Monitor ID",
    "run_key": "Run key",
    "last_error_code": "Kód chyby",
    "last_error_message": "Popis chyby",
    "fix_id": "Fix ID",
    "fix_code": "Kód opravy",
    "fix_title_cz": "Název opravy",
    "fix_description_cz": "Popis opravy",
    "operator_button_cz": "Tlačítko opravy",
    "confidence_pct": "Důvěra %",
    "auto_executable": "Auto oprava",
    "requires_operator_confirm": "Vyžaduje potvrzení",
    "target_action": "Akce opravy",
    "execution_mode_cz": "Režim opravy",
    "confidence_color": "Barva důvěry",
})
...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V20.1.G - OPERATOR FIX PANEL BINDING CZ"
        )
    ...
    def build_ui(self):
        ...
        tk.Label(
            header,
            text="▰ MATCHMATRIX ŘÍDICÍ CENTRUM V20.1.G",
            bg="#07040d",
            fg=PINK,
        ...
        global_pc2_button(12, "🛠 OPRAVIT STOP CHYBU", "#b91c1c", self.operator_execute_recommended_fix)
        global_pc2_button(13, "↻ RETRY / READY", "#6d45b8", self.pc2_retry_selected_command)
        global_pc2_button(14, "⏳ PLANNER PENDING", "#4c2c83", self.pc2_reset_selected_planner_pending)

        global_pc2_group(15, "4) KONTROLA")
        global_pc2_button(16, "🔍 TEST", "#0f5f63", self.pc2_test_selected_command)
        global_pc2_button(17, "↻ OBNOVIT PC2", "#3b2555", self.load_pc2_command_center)
...
# =========================================================
        ...
        pc2_side_button(
            ...
            "🛠 OPRAVIT STOP CHYBU",
            "#b91c1c",
            self.operator_execute_recommended_fix
        )
        pc2_side_button(
            pc2_action_side,
            13,
        ...
        pc2_group(pc2_action_side, 15, "4) KONTROLA")
        pc2_side_button(
            pc2_action_side,
            16,
            "🔍 TEST VYBRANÉ AKCE",
...
Další termín: {h.get('next_target_date') or '-'}"""
    ...
    def update_pc2_visual_operator_cards(self, rows):
        """
        MATCHMATRIX 20_1_K – OPERATOR PANEL VISUAL BINDING

        CO TO JE:
        - Napojení horních grafických karet v záložce DENNÍ PRÁCE
          na jednotné view ops.v_operator_dashboard_summary_v1.

        K ČEMU TO JE:
        - Panel už neskládá horní stav z více oddělených dotazů.
        - Jeden DB zdroj řekne: READY / BĚŽÍ / HOTOVO / CHYBA,
          poslední výsledek a doporučenou další akci.

        KDE TO UVIDÍME:
        - OPS Panel -> DENNÍ PRÁCE -> horní karty:
          DNEŠNÍ POSTUP / AKTUÁLNÍ-DALŠÍ BĚH / POSLEDNÍ VÝSLEDEK / CHYBY-STOP.

        JAK SE TO VYUŽIJE:
        - Pokud je něco červené, karta ukáže OPRAVIT.
        - Pokud není chyba, karta ukáže další READY akci ke spuštění.
        - Operátor nemusí číst tabulky, jen podle semaforu pokračuje.
        ...
        try:
            summary = first_ok("""
                SELECT *
                FROM ops.v_operator_dashboard_summary_v1;
            """)

            if not summary:
                # Bezpečný fallback na starší výpočet z rows, kdyby view ještě nebylo nasazené.
                rows = rows or []
                total = len(rows)
                ready = sum(1 for r in rows if str(r.get("run_status", "")).upper() in ("READY", "READY_TO_RUN", "PŘIPRAVENO"))
                running = sum(1 for r in rows if str(r.get("run_status", "")).upper() in ("RUNNING", "BĚŽÍ"))
                done = sum(1 for r in rows if str(r.get("run_status", "")).upper() in ("DONE", "HOTOVO"))
                error = sum(1 for r in rows if str(r.get("run_status", "")).upper() in ("ERROR", "FAILED", "CHYBA"))
                pct = int(round(100 * done / total)) if total else 0
                accent = RED if error else (YELLOW if running else GREEN)

                self.set_pc2_visual_card(
                    "day",
                    f"{pct} %",
                    f"READY: {ready}   Běží: {running}\nHotovo: {done}   Chyba: {error}",
                    pct,
                    accent
                )
                self.set_pc2_visual_card("current", "Bez summary", "Chybí ops.v_operator_dashboard_summary_v1.", 0, YELLOW)
                self.set_pc2_visual_card("result", "Detail níže", "Používám fallback z PC2 fronty.", pct, accent)
                self.set_pc2_visual_card("error", f"{error} CHYBA", "Nasadit 20_1_J_operator_dashboard_summary.sql.", 100, RED if error else GREEN)
                return

            total = as_int(summary.get("total_commands"))
            ready = as_int(summary.get("ready_commands"))
            running = as_int(summary.get("running_commands"))
            done = as_int(summary.get("done_commands"))
            errors = as_int(summary.get("error_commands"))
            blocked = as_int(summary.get("blocked_commands"))
            queue_pct = as_pct(summary.get("queue_done_pct"))
            operator_color = color_from_light(summary.get("operator_traffic_light"), GREEN)

            # 1) DNEŠNÍ POSTUP / STAV FRONTY
            self.set_pc2_visual_card(
                "day",
                f"{queue_pct} %",
                (
                    f"READY: {ready}   Běží: {running}   Hotovo: {done}\n"
                    f"Chyba: {errors}   Blokováno: {blocked}   Celkem: {total}\n"
                    f"{clean(summary.get('operator_main_message'), '')}"
                ),
                queue_pct,
                operator_color
            )
            ...
            next_action = clean(summary.get("next_operator_action"), "ZKONTROLOVAT")
            next_sport = clean(summary.get("next_sport_code"))
            next_layer = clean(summary.get("next_target_layer"))
            next_title = clean(summary.get("next_command_title"))
            next_status = clean(summary.get("next_run_status"))
            next_priority = clean(summary.get("next_priority_score"))
            next_reason = clean(summary.get("next_recommendation_reason"), "")
            next_accent = color_from_light(summary.get("operator_traffic_light"), PURPLE)
            if next_action == "SPUSTIT":
                next_accent = GREEN
            elif next_action == "SLEDOVAT":
                next_accent = YELLOW
            elif next_action == "OPRAVIT":
                next_accent = RED
            ...
            self.set_pc2_visual_card(
                ...
                f"{next_action}",
                (
                    f"{next_sport} / {next_layer} | priorita {next_priority}\n"
                    f"{next_title}\n"
                    f"Stav: {next_status} | {next_reason}"
                ),
                100 if next_action in ("SPUSTIT", "OPRAVIT") else 50,
                next_accent
            )

            # 3) POSLEDNÍ VÝSLEDEK
            last_pct = as_pct(summary.get("last_result_pct"), 0)
            last_sport = clean(summary.get("last_sport_code"))
            last_layer = clean(summary.get("last_target_layer"))
            last_status = clean(summary.get("last_run_status"))
            last_msg = clean(summary.get("last_result_message"), "Bez posledního výsledku.")
            last_operator_msg = clean(summary.get("last_operator_message"), "")
            last_errors = as_int(summary.get("last_error_count"))
            last_accent = RED if last_errors > 0 else GREEN

            self.set_pc2_visual_card(
                "result",
                f"{last_pct} %",
                (
                    f"{last_sport} / {last_layer} | {last_status}\n"
                    f"Chyby: {last_errors}\n"
                    f"{last_msg}\n{last_operator_msg}"
                ),
                last_pct,
            ...
            # 4) CHYBY / STOP
            stop_errors = as_int(summary.get("active_stop_errors"))
            total_problem_count = errors + stop_errors
            if total_problem_count > 0:
                self.set_pc2_visual_card(
                    "error",
                    f"{total_problem_count} STOP",
                    (
                        f"Fronta chyby: {errors}   Monitor STOP: {stop_errors}\n"
                        f"Doporučení: {clean(summary.get('operator_main_message'), 'Nejdříve vyřeš chyby.')}\n"
                        f"První akce: {next_sport} / {next_layer} → {next_action}"
                    ),
                    100,
                    RED
                )
                ...
                self.set_pc2_visual_card(
                    ...
                    (
                        "Bez červených chyb.\n"
                        f"Další krok: {next_sport} / {next_layer} → {next_action}"
                    ),
                    100,
                    GREEN
                )
            ...
            try:
                self.set_pc2_visual_card("error", "Chyba panelu", str(exc)[:180], 100, RED)
            except Exception:
                pass
    ...
    def operator_execute_recommended_fix(self):
        """
        MATCHMATRIX 20_1_G – OPERATOR FIX PANEL BINDING

        CO TO JE:
        - Akční tlačítko OPRAVIT v záložce DENNÍ PRÁCE.

        K ČEMU TO JE:
        - Operátor nemusí ručně hledat SQL opravu.
        - Panel vezme nejvyšší doporučenou opravu z ops.v_operator_fix_recommendations_v1.
        - Potom zavolá ops.fn_operator_execute_fix_v1(monitor_id, 'PANEL_OPERATOR').

        KDE TO UVIDÍME:
        - OPS Panel -> DENNÍ PRÁCE -> pravá lišta -> OPRAVA -> OPRAVIT STOP CHYBU.

        JAK SE TO VYUŽIJE:
        - TIMEOUT / ROUTING_ERROR s auto_executable = true se vrátí do READY.
        - Oprava se zapíše do ops.operator_fix_execution_log.
        - Panel se po akci obnoví.
        ...
        rows = db_query("""
            SELECT
                ...
                auto_executable,
                execution_mode_cz
            FROM ops.v_operator_fix_recommendations_v1
            ORDER BY
                CASE WHEN auto_executable THEN 1 ELSE 2 END,
                confidence_pct DESC
            LIMIT 1;
        """)

        if not rows or "CHYBA" in rows[0]:
            error_text = rows[0].get("CHYBA") if rows and isinstance(rows[0], dict) else "Není aktivní STOP chyba s doporučenou opravou."
            messagebox.showinfo(
                "Operator Fix",
                f"Není co opravit.\n\n{error_text}"
            )
        ...
        auto_executable = bool(rec.get("auto_executable"))

        confirm_text = (
            "Provést doporučenou opravu?\n\n"
            f"Sport: {rec.get('sport_code')} / {rec.get('sport_name')}\n"
            f"Provider: {rec.get('provider')}\n"
            f"Entita: {rec.get('entity_type')} / {rec.get('target_layer')}\n\n"
            f"Chyba: {rec.get('last_error_code')}\n"
            f"Popis: {rec.get('last_error_message')}\n\n"
            f"Oprava: {rec.get('fix_title_cz')} ({fix_code})\n"
            f"Důvěra: {rec.get('confidence_pct')} %\n"
            f"Auto oprava: {'ANO' if auto_executable else 'NE'}\n\n"
            f"{rec.get('fix_description_cz') or ''}"
        )

        if not messagebox.askyesno("Potvrdit opravu", confirm_text):
            return

        result = db_query(f"""
            SELECT *
            FROM ops.fn_operator_execute_fix_v1(
                {int(monitor_id)}::BIGINT,
                'PANEL_OPERATOR'::TEXT
            );
        """)

        if not result or "CHYBA" in result[0]:
            error_text = result[0].get("CHYBA") if result and isinstance(result[0], dict) else "Neznámá chyba při auto-fixu."
            messagebox.showerror(
                "Operator Fix selhal",
                f"Oprava nebyla provedena.\n\n{error_text}"
            )
            return
        ...
        if ok:
            messagebox.showinfo(
                "Operator Fix hotovo",
                (
                    "Oprava byla úspěšně provedena.\n\n"
                    f"Monitor ID: {out.get('out_monitor_id')}\n"
                    f"Fix: {out.get('out_fix_code')}\n"
                    f"Stav: {out.get('execution_status')}\n"
                    f"Zpráva: {out.get('execution_message')}"
                )
            )
        else:
            messagebox.showwarning(
                "Operator Fix vyžaduje kontrolu",
                (
                    "Oprava nebyla automaticky provedena.\n\n"
                    f"Monitor ID: {out.get('out_monitor_id')}\n"
                    f"Fix: {out.get('out_fix_code')}\n"
                    f"Stav: {out.get('execution_status')}\n"
                    f"Zpráva: {out.get('execution_message')}"
    ...
    def load_pc2_command_center(self):
        ...
        SELECT
            ...
            run_status,
            safety_mode,
            panel_action_enabled AS button_enabled,
            safety_mode AS safety_note_cs,
            action_description,
            purpose_description,
            target_tables,
            panel_usage,
        ...
        ORDER BY
            CASE
                WHEN run_status = 'READY_TO_RUN' THEN 1
                WHEN run_status = 'DONE' THEN 2
                WHEN run_status = 'FAILED' THEN 3
                WHEN run_status = 'BLOCKED' THEN 4
                ELSE 9
            END,
            priority_score,
            sport_code,
    ...
    def populate_pc2_action_cards(self, tree, rows):
        """
        V18.20 - PC2 ACTION CARDS

        CO TO JE:
        - Místo useknutých DB sloupců ukazuje dlouhý název akce jako hlavní kartu.

        K ČEMU TO JE:
        - V PC2 Command Center je hned vidět celý příkaz typu:
          ▶ SPUSTIT PEOPLE HARVEST PRO BASKETBALL.
        """
        try:
            ...
            tree.column("action_card", width=760, minwidth=520, stretch=True, anchor="w")
            tree.column("status", width=135, minwidth=105, stretch=False, anchor="center")
            tree.column("priority", width=85, minwidth=70, stretch=False, anchor="center")
            tree.column("sport", width=150, minwidth=110, stretch=False, anchor="w")
            tree.column("layer", width=110, minwidth=90, stretch=False, anchor="center")
