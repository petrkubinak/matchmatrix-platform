# MATCHMATRIX CONTROL PANEL V17.1
#
# CO TO JE:
...
# C:\Python314\python.exe tools\matchmatrix_control_panel_V17_1.py
...
SELECT
    ...
    execution_allowed,
    COALESCE(execution_state, '') AS active_execution_state,
    COALESCE(pid::text, '') AS active_pid,
    COALESCE(to_char(started_at, 'YYYY-MM-DD HH24:MI:SS'), '') AS active_started_at
FROM ops.v_execution_lock_guard_v1
WHERE execution_allowed = true
ORDER BY sport_code, entity, candidate_provider
LIMIT 100;
"""
...
class MatchMatrixPanelV170(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title("MatchMatrix Control Panel V17.1 SAFE MODE")
    ...
    def render_scheduler(self):
        ...
        for row in self.rows_scheduler:
            ...
            if row.get("worker_already_running"):
                tag = "BLOCKED"
    ...
    def run_next_scheduler_job(self):
        ...
        if worker_already_running or not execution_allowed:
            messagebox.showwarning(
                "RUN NEXT BLOCKED",
                ...
                "Důvod: worker už běží nebo není execution_allowed."
            )
            return
        ...
        confirm = messagebox.askyesno(
            ...
            f"SAFE TO EXECUTE: {row.get('safe_to_execute')}\n"
