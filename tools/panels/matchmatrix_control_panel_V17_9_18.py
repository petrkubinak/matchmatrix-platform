...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def build_ui(self):
        ...
        self.ai_critical = self.create_kpi("🚨 AI CRITICAL", "0", RED)
        self.ai_safe_retry = self.create_kpi("♻ SAFE RETRY", "0", GREEN)
        self.ai_auto_fix = self.create_kpi("🛠 AUTO FIX", "0", PURPLE)
        self.ai_blocking = self.create_kpi("⛔ BLOCKING", "0", RED)
        self.ai_score = self.create_kpi("🤖 AI SCORE", "0", PINK)
        ...
        self.orchestration_summary_tree = self.create_section(
            dashboard_row_1, "⚙ ORCHESTRATION SUMMARY", 0, 0
        )
        ...
        self.dashboard_tree = self.create_section(
            dashboard_row_1, "📊 STAV SCHEDULERU", 0, 2
        )
        ...
        self.worker_health_tree = self.create_section(
            dashboard_row_2, "🧩 WORKER HEALTH", 0, 2
        )
        ...
        self.cooldown_tree = self.create_section(
            dashboard_row_3, "❄ PLANNER COOLDOWN", 0, 1
        )
        ...
        self.logs_detail_tree = self.create_section(
            tab_logs, "▤ POSLEDNÍ JOB LOGY", 0, 0
        )
        ...
        ai_ops_health_frame = tk.Frame(ai_ops_paned, bg=BG)
        ai_ops_alert_frame = tk.Frame(ai_ops_paned, bg=BG)
        scheduler_autopilot_frame = tk.Frame(ai_ops_paned, bg=BG)
        ai_action_queue_frame = tk.Frame(ai_ops_paned, bg=BG)
        ai_action_history_frame = tk.Frame(ai_ops_paned, bg=BG)

        for f in [
            ai_ops_health_frame,
            ai_ops_alert_frame,
            scheduler_autopilot_frame,
            ai_action_queue_frame,
            ai_action_history_frame
        ]:
            f.columnconfigure(0, weight=1)
            f.rowconfigure(0, weight=1)
            ai_ops_paned.add(f, minsize=120)

        self.ai_ops_health_tree = self.create_section(
            ai_ops_health_frame, "🤖 AI OPS HEALTH", 0, 0
        )

        self.ai_ops_alert_tree = self.create_section(
            ai_ops_alert_frame, "🚨 AI OPS ALERT CENTER", 0, 0
        )

        self.scheduler_autopilot_tree = self.create_section(
            scheduler_autopilot_frame, "🤖 SCHEDULER AUTOPILOT", 0, 0
        )

        self.ai_action_queue_tree = self.create_section(
            ai_action_queue_frame, "⚙ AI ACTION QUEUE", 0, 0
        )

        self.ai_action_history_tree = self.create_section(
            ai_action_history_frame, "📜 AI ACTION HISTORY", 0, 0
        )

        self.fix_tasks_tree = self.create_section(
            tab_fix_tasks, "🛠 FIX TASKS", 1, 0
        )
    ...
    def open_payload_group_detail(self, event):
        ...
        detail_window = tk.Toplevel(self)
        detail_window.title(
            f"PAYLOAD DETAIL :: {provider} / {sport_code} / {entity_type} / {parse_status}"
        )
        detail_window.geometry("1500x850")
        detail_window.configure(bg=BG)
        ...
        tk.Label(
            detail_window,
            text=f"📦 PAYLOAD DETAIL :: {provider} / {sport_code} / {entity_type} / {parse_status}",
            bg=BG,
            fg=PINK,
            font=("Segoe UI", 20, "bold")
        ).pack(anchor="w", padx=15, pady=10)
    ...
    """
        ...
        win.title(f"PAYLOAD JSON DETAIL :: {payload_id}")
        win.geometry("1500x850")
        win.configure(bg=BG)

        tk.Label(
            win,
            text=f"📦 PAYLOAD JSON DETAIL :: {payload_id}",
            bg=BG,
            fg=PINK,
            font=("Segoe UI", 20, "bold")
        ).pack(anchor="w", padx=15, pady=10)
    ...
    """
        ...
        def create_fix_task():
            ...
            self.log(
                f"FIX TASK SAVED | {provider} | {sport} | {entity} | {suggested_fix}"
            )

            messagebox.showinfo(
                "FIX TASK ULOŽEN",
                "Fix task byl uložen do ops.fix_tasks"
            )

        def show_full_json():

            json_win = tk.Toplevel(self)
            json_win.title(f"FULL PAYLOAD JSON :: {payload_id}")
            json_win.geometry("1500x850")
            json_win.configure(bg=BG)

            tk.Label(
                json_win,
                text=f"👁 FULL PAYLOAD JSON :: {payload_id}",
                bg=BG,
                fg=PINK,
                font=("Segoe UI", 20, "bold")
            ).pack(anchor="w", padx=15, pady=10)
    ...
    def open_parse_message_detail(self, message_text):
        ...
        win.title("PARSE MESSAGE DETAIL")
    ...
    def load_fix_tasks(self):
        ...
        self.fix_btn_open.config(
            text=f"OPEN ({counts['open']})"
        )

        self.fix_btn_fixed.config(
            text=f"FIXED ({counts['fixed']})"
        )

        self.fix_btn_ignored.config(
            text=f"IGNORED ({counts['ignored']})"
        )
    ...
    def open_fix_task_detail(self, event):
        ...
        win = tk.Toplevel(self)
        win.title(f"FIX TASK DETAIL :: {fix_task_id}")
        win.geometry("1400x850")
        win.configure(bg=BG)

        tk.Label(
            win,
            text=f"🛠 FIX TASK DETAIL :: {fix_task_id}",
            bg=BG,
            fg=PINK,
    ...
    """
        ...
        def mark_fixed():
            ...
            self.log(f"FIX TASK FIXED | id={fix_task_id}")
            messagebox.showinfo("FIX TASK", "Úkol označen jako FIXED.")
            self.load_fix_tasks()
            win.destroy()
        ...
        def mark_ignored():
            ...
            self.log(f"FIX TASK IGNORED | id={fix_task_id}")
            messagebox.showinfo("FIX TASK", "Úkol označen jako IGNORED.")
            self.load_fix_tasks()
            win.destroy()
