...
# ============================================================
# MATCHMATRIX CONTROL PANEL V13 FIX2
# - compact / responsive layout
# - global scroll for whole window
# - adaptive bottom split
# - snapshot before/after/diff
# - live run status
# - layer profiles: CORE / PEOPLE / MEDIA / ODDS / FULL SPORT
# - safe quick buttons for BK unified cycle, media pipeline, odds refresh
# ============================================================
...
BK_CORE_PARSERS_RUNNER = str(PROJECT_ROOT / "workers" / "parsers" / "105_Z_run_bk_core_parsers_v1.py")
BK_UNIFIED_CYCLE_RUNNER = str(PROJECT_ROOT / "workers" / "106_B_run_bk_unified_cycle_v1.py")
MEDIA_PIPELINE_RUNNER = str(PROJECT_ROOT / "workers" / "run_media_pipeline_v1.py")
TICKET_STUDIO_RUNNER = str(PROJECT_ROOT / "tools" / "matchmatrix_ticket_studio_V3_fix13.py")
...
# Entity profily jsou záměrně oddělené podle vrstev.
# Panel jen vybírá entity; dávkování potom bezpečně řeší batch runner podle limitu.
ENTITY_PROFILE_MAP = {
    "custom": [],
    "core": ["leagues", "teams", "fixtures"],
    "people": ["players", "coaches", "player_profiles", "player_stats", "player_season_stats", "team_squad"],
    "media": ["articles", "highlights", "videos", "comments"],
    "odds": ["odds"],
    "full_sport": [
        "leagues",
        "teams",
    ...
    # zpětná kompatibilita se staršími názvy ve V11/V12
    "core_ingest": ["leagues", "teams", "fixtures"],
    "full_ingest": ["leagues", "teams", "fixtures", "players", "player_profiles", "player_season_stats", "player_stats", "odds"],
    "players_only": ["players", "player_profiles", "player_season_stats", "player_stats"],
    "fixtures_only": ["fixtures"],
}

PROFILE_HELP_TEXT = {
    "custom": "ruční výběr entit",
    "core": "leagues + teams + fixtures",
    "people": "players + coaches + profily/statistiky",
    "media": "articles + highlights + videos + comments",
    "odds": "kurzy / odds",
    "full_sport": "core -> people -> media -> odds podle vybraných entit",
}

# Některé pull skripty API-Sport vyžadují sezonu povinně.
# Bez tohoto parametru PowerShell čeká na ruční zadání a batch skončí timeoutem.
DEFAULT_SEASON = "2024"
SEASON_REQUIRED_ENTITIES = {"teams"}

# Některé entity zatím nejsou bezpečné pro paralelní běh.
# Důvod: některé PowerShell pull skripty používají dočasné SQL soubory,
# které mohou při paralelním běhu kolidovat.
# Panel proto u těchto entit automaticky sníží max-workers na 1.
ENTITY_SAFE_MAX_WORKERS = {
    "teams": 1,
    "players": 1,
    "player_profiles": 1,
    "player_stats": 1,
...
# V14: centrální konfigurace layerů podle sportu.
# Důvod: jeden sport nemusí používat stejného providera pro CORE, PEOPLE, MEDIA a ODDS.
# Například BK CORE běží přes api_sport, ale BK PEOPLE máme ověřené přes sportsdataio.
# Panel proto nesmí pro PEOPLE automaticky přebírat CORE provider/run_group.
SPORT_LAYER_CONFIG = {
    "FB": {
        "core": {"provider": "api_football", "run_group": "EU_top,EU_exact_v1"},
        "people": {"provider": "api_football", "run_group": "FB_PEOPLE_SCALE_04", "entity": "players"},
        "media": {"provider": "api_football", "run_group": "FB_MEDIA"},
        "odds": {"provider": "theodds", "run_group": "FB_ODDS_CORE"},
    },
    "BK": {
        "core": {"provider": "api_sport", "run_group": "BK_CORE"},
        "people": {"provider": "sportsdataio", "run_group": "PEOPLE_AUTO_SPORTSDATAIO_2024", "entity": "players"},
        "media": {"provider": "api_sport", "run_group": "BK_MEDIA"},
        "odds": {"provider": "theodds", "run_group": "BK_ODDS_CORE"},
    },
    "AFB": {
        "core": {"provider": "api_american_football", "run_group": "AFB_CORE"},
        "people": {"provider": "api_american_football", "run_group": "AFB_PEOPLE_V2", "entity": "players"},
        "media": {"provider": "api_american_football", "run_group": "AFB_MEDIA"},
        "odds": {"provider": "theodds", "run_group": "AFB_ODDS_CORE"},
    },
    "HK": {
        "core": {"provider": "api_hockey", "run_group": "HK_CORE"},
        "people": {"provider": "api_hockey", "run_group": "HK_PEOPLE", "entity": "players"},
        "media": {"provider": "api_hockey", "run_group": "HK_MEDIA"},
        "odds": {"provider": "theodds", "run_group": "HK_ODDS_CORE"},
    },
    "HB": {
        "core": {"provider": "api_handball", "run_group": "HB_CORE"},
        "people": {"provider": "api_handball", "run_group": "HB_PEOPLE", "entity": "players"},
        "media": {"provider": "api_handball", "run_group": "HB_MEDIA"},
        "odds": {"provider": "theodds", "run_group": "HB_ODDS_CORE"},
    },
    "CK": {
        "core": {"provider": "api_cricket", "run_group": "CK_CORE"},
        "people": {"provider": "api_cricket", "run_group": "CK_PEOPLE", "entity": "players"},
        "media": {"provider": "api_cricket", "run_group": "CK_MEDIA"},
        "odds": {"provider": "theodds", "run_group": "CK_ODDS_CORE"},
    },
    "BSB": {
        "core": {"provider": "api_baseball", "run_group": "BSB_CORE"},
        "people": {"provider": "sportsdataio", "run_group": "PEOPLE_AUTO_SPORTSDATAIO_2024", "entity": "players"},
        "media": {"provider": "api_baseball", "run_group": "BSB_MEDIA"},
        "odds": {"provider": "theodds", "run_group": "BSB_ODDS_CORE"},
    },
    "VB": {
        "core": {"provider": "api_volleyball", "run_group": "VB_CORE"},
        "people": {"provider": "api_volleyball", "run_group": "VB_PEOPLE", "entity": "players"},
        "media": {"provider": "api_volleyball", "run_group": "VB_MEDIA"},
        "odds": {"provider": "theodds", "run_group": "VB_ODDS_CORE"},
    },
}
...
# Dynamické škálování tlačítek podle velikosti okna.
# Cíl: čitelné ovládání na větším monitoru, ale bez rozbití layoutu na menším okně.
BUTTON_FONT_BASE = 10
BUTTON_FONT_MIN = 9
BUTTON_FONT_MAX = 13
...
class MatchMatrixPanelV15:
    def apply_dynamic_button_scale(self, force=False):
        """
        Bezpečný fallback pro dynamické škálování tlačítek.
        """
        try:
            width = self.root.winfo_width()

            if width >= 1600:
                btn_font = (FONT_FAMILY, 11, "bold")
            elif width >= 1300:
                btn_font = (FONT_FAMILY, 10, "bold")
            else:
                btn_font = (FONT_FAMILY, 9, "bold")

            style = ttk.Style()
            style.configure("Accent.TButton", font=btn_font, padding=4)

        except Exception:
            pass
    def on_root_configure(self, event=None):
        """
        Dynamické škálování UI podle šířky okna.
        Bezpečný fallback, aby panel nikdy nespadl při změně velikosti okna.
        """
        try:
            width = self.root.winfo_width()
            ...
            if width >= 1600:
                ...
                self.root.tk.call("tk", "scaling", 1.00)
            elif width >= 1000:
                self.root.tk.call("tk", "scaling", 0.95)
            else:
                self.root.tk.call("tk", "scaling", 0.90)
        except Exception:
            pass
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("MatchMatrix Mission Control V15")
        ...
        self._last_button_font_size: int | None = None

        self._setup_style()
        self._build_ui()
        self.root.bind("<Configure>", self.on_root_configure, add="+")
        self.apply_dynamic_button_scale(force=True)
        ...
        self.log_write("V15 načten: SIMPLE orchestrace + SPORT_LAYER_CONFIG + safe workers + dynamické písmo tlačítek.")

        def on_root_configure(self, event=None):
            """
            Dynamické škálování UI podle šířky okna.
            Zatím bezpečný fallback, aby panel nespadl při startu.
            """
            try:
                width = self.root.winfo_width()
    ...
    def build_header(self, parent) -> None:
        ...
        tk.Label(
            self.header_frame,
            text="TICKETMATRIXPLATFORM MISSION CONTROL V12",
            bg=BG,
            fg=FG,
            font=FONT_TITLE,
    ...
    def build_settings_frame(self, settings_frame) -> None:
        ...
        tk.Label(settings_frame, text="Layer / Entity profil", bg=BG, fg=FG, font=FONT_TEXT).grid(row=2, column=0, sticky="w", pady=2)
        ...
        self.profile_help_lbl = tk.Label(settings_frame, text=PROFILE_HELP_TEXT["custom"], bg=BG, fg=MUTED, font=FONT_SMALL)
        self.profile_help_lbl.grid(row=3, column=1, sticky="w", pady=(0, 4))

        tk.Label(settings_frame, text="Provider mode", bg=BG, fg=FG, font=FONT_TEXT).grid(row=4, column=0, sticky="w", pady=2)
        ...
        provider_mode_combo.grid(row=4, column=1, sticky="ew", pady=2)
        provider_mode_combo.bind("<<ComboboxSelected>>", self.on_provider_mode_changed)

        tk.Label(settings_frame, text="Manual provider", bg=BG, fg=FG, font=FONT_TEXT).grid(row=5, column=0, sticky="w", pady=2)
        ...
        self.manual_provider_entry.grid(row=5, column=1, sticky="ew", pady=2)

        tk.Label(settings_frame, text="Limit", bg=BG, fg=FG, font=FONT_TEXT).grid(row=6, column=0, sticky="w", pady=2)
        self.limit_var = tk.StringVar(value="5")
        ttk.Entry(settings_frame, textvariable=self.limit_var).grid(row=6, column=1, sticky="ew", pady=2)

        tk.Label(settings_frame, text="Max workers", bg=BG, fg=FG, font=FONT_TEXT).grid(row=7, column=0, sticky="w", pady=2)
        self.max_workers_var = tk.StringVar(value="3")
        ttk.Entry(settings_frame, textvariable=self.max_workers_var).grid(row=7, column=1, sticky="ew", pady=2)

        tk.Label(settings_frame, text="Timeout sec", bg=BG, fg=FG, font=FONT_TEXT).grid(row=8, column=0, sticky="w", pady=2)
        self.timeout_sec_var = tk.StringVar(value="300")
        ttk.Entry(settings_frame, textvariable=self.timeout_sec_var).grid(row=8, column=1, sticky="ew", pady=2)
    ...
    def build_action_area(self, parent) -> None:
        ...
        ttk.Button(
            row1,
            text="Spustit vybraný layer / batch",
        ...
        ttk.Button(
            row1,
            text="BK UNIFIED CYCLE",
            style="Accent.TButton",
            command=self.run_bk_unified_cycle_thread,
        ).pack(side="left", padx=4, pady=4)
        ...
        ttk.Button(
            ...
            text="MEDIA PIPELINE",
            style="Ghost.TButton",
            command=self.run_media_pipeline_thread,
        ).pack(side="left", padx=4, pady=4)

        ttk.Button(
            row2,
            text="ODDS REFRESH / THEODDS",
            style="Ghost.TButton",
    ...
    def on_profile_changed(self, event=None) -> None:
        ...
        if hasattr(self, "profile_help_lbl"):
            self.profile_help_lbl.config(text=PROFILE_HELP_TEXT.get(profile, "profil entit"))

        if profile == "custom":
            self.log_write("Použit profil entit: custom / ruční výběr")
            self.update_selection_dashboard()
            return
        ...
        missing = []
        for wanted_entity in wanted:
            if wanted_entity not in self.db_entity_options:
                missing.append(wanted_entity)

        for idx, entity in enumerate(self.db_entity_options):
            if entity in wanted:
                self.entities_listbox.selection_set(idx)

        self.log_write(f"Použit profil entit: {profile} -> {', '.join(wanted) if wanted else '-'}")
        if missing:
            self.log_write("Poznámka: tyto entity nejsou pro aktuální sport/DB výběr dostupné: " + ", ".join(missing))
        self.update_selection_dashboard()
    ...
    def get_selected_entities(self) -> list[str]:
        selected = [self.entities_listbox.get(i) for i in self.entities_listbox.curselection()]

        # Pokud je aktivní profil, vracíme entity v pořadí profilu, ne v abecedním/DB pořadí listboxu.
        # To je důležité hlavně pro CORE: leagues -> teams -> fixtures.
        profile = self.profile_var.get() if hasattr(self, "profile_var") else "custom"
        profile_order = ENTITY_PROFILE_MAP.get(profile, [])

        if profile != "custom" and profile_order:
            ordered = [entity for entity in profile_order if entity in selected]
            rest = [entity for entity in selected if entity not in ordered]
            return ordered + rest
    ...
    def run_bk_core_parsers_thread(self) -> None:
        thread = threading.Thread(target=self.run_bk_core_parsers, daemon=True)
        thread.start()

    def run_bk_unified_cycle_thread(self) -> None:
        thread = threading.Thread(target=self.run_bk_unified_cycle, daemon=True)
        thread.start()

    def run_media_pipeline_thread(self) -> None:
        thread = threading.Thread(target=self.run_media_pipeline, daemon=True)
        thread.start()

    def run_bk_unified_cycle(self) -> None:
        script_path = Path(BK_UNIFIED_CYCLE_RUNNER)

        if not script_path.exists():
            self.log_write(f"CHYBA: BK unified cycle runner neexistuje: {script_path}")
            messagebox.showerror(
                "BK unified cycle",
                "Nejdřív ulož runner:\n"
                f"{script_path}\n\n"
                "Soubor jsme navrhli jako 106_B_run_bk_unified_cycle_v1.py."
            )
            return

        cmd = [PYTHON_EXE, str(script_path)]
        self.log_write("Spouštím BK UNIFIED CYCLE:")
        self.log_write(" ".join(cmd))

        self.run_command_stream(
            cmd=cmd,
            runner_name="BK unified cycle",
            total_steps=2,
            step_label="BK parsers -> unified merge",
            sports="BK",
            entities="leagues, teams, fixtures, players",
            run_group="BK_CORE",
        )

    def run_media_pipeline(self) -> None:
        script_path = Path(MEDIA_PIPELINE_RUNNER)

        if not script_path.exists():
            self.log_write(f"CHYBA: MEDIA pipeline runner neexistuje: {script_path}")
            messagebox.showerror(
                "MEDIA pipeline",
                "Nenalezl jsem media runner:\n"
                f"{script_path}\n\n"
                "Podle scanu by měl být v C:\\MatchMatrix-platform\\workers\\run_media_pipeline_v1.py."
            )
            return
        ...
        # Media runner historicky běžel i bez parametrů. Parametry proto nepřidáváme natvrdo,
        # aby panel nerozbil existující worker, pokud je zatím nepodporuje.
        self.log_write("Spouštím MEDIA PIPELINE:")
        self.log_write(" ".join(cmd))
        self.log_write(f"Kontext panelu: sports={selected_sports or '-'} | entities={selected_entities or '-'} | limit={limit} | timeout={timeout_sec}")

        self.run_command_stream(
            cmd=cmd,
            runner_name="media pipeline",
            total_steps=1,
            step_label="media pull/parse/merge/match/score podle workeru",
            sports=", ".join(selected_sports) or "MULTI",
            entities=", ".join(selected_entities) or "articles, highlights, videos, comments",
            run_group="MEDIA",
        )

    def run_ticket_studio_thread(self) -> None:
        thread = threading.Thread(target=self.run_ticket_studio, daemon=True)
        thread.start()

    def run_bk_core_parsers(self) -> None:
        script_path = Path(BK_CORE_PARSERS_RUNNER)

        if not script_path.exists():
            self.log_write(f"CHYBA: BK CORE parsers runner neexistuje: {script_path}")
            messagebox.showerror(
                "BK CORE parsers",
                "Nejdřív ulož runner:\n"
                f"{script_path}\n\n"
                "Soubor jsme navrhli jako 105_Z_run_bk_core_parsers_v1.py."
            )
            return

        cmd = [PYTHON_EXE, str(script_path)]

        self.log_write("Spouštím BK CORE parsers runner:")
        self.log_write(" ".join(cmd))

        self.run_command_stream(
            cmd=cmd,
            runner_name="BK CORE parsers runner",
            total_steps=1,
            step_label="BK RAW -> Python parsers -> staging",
            sports="BK",
            entities="fixtures, leagues, teams, players",
            run_group="BK_CORE",
        )

    def run_ticket_studio(self) -> None:
        script_path = Path(TICKET_STUDIO_RUNNER)

        if not script_path.exists():
            fallback_path = PROJECT_ROOT / "matchmatrix_ticket_studio_V3_fix13.py"
            if fallback_path.exists():
                script_path = fallback_path
            else:
                self.log_write(f"CHYBA: Ticket Studio soubor neexistuje: {script_path}")
                messagebox.showerror(
                    "Ticket Studio",
                    "Nenalezl jsem Ticket Studio. Doporučené umístění:\n"
                    f"{script_path}\n\n"
                    "Alternativně může být v rootu projektu jako matchmatrix_ticket_studio_V3_fix13.py."
                )
                return

        cmd = [PYTHON_EXE, str(script_path)]

        self.log_write("Spouštím Ticket Studio:")
        self.log_write(" ".join(cmd))

        # Ticket Studio je samostatná GUI aplikace. Použijeme subprocess.Popen,
        # aby se hlavní Mission Control panel nezablokoval.
        try:
            subprocess.Popen(
                cmd,
            ...
            self.log_write("Ticket Studio spuštěno jako samostatné okno.")
        except Exception as e:
            self.log_write(f"CHYBA při spuštění Ticket Studio: {e}")
            messagebox.showerror("Ticket Studio", str(e))
    ...
    def resolve_layer_config(self, sport: str, layer: str) -> dict:
        """Vrátí konfiguraci pro konkrétní sport/layer.

        V14 princip:
        - CORE může používat jiného providera než PEOPLE.
        - PEOPLE nesmí přebírat CORE run_group.
        - Pokud sport/layer není v mapě, panel použije bezpečný fallback.
        """
        sport_code = (sport or "").strip().upper()
        layer_code = (layer or "").strip().lower()
        return dict(SPORT_LAYER_CONFIG.get(sport_code, {}).get(layer_code, {}))
    ...
    def run_people_v22(self) -> None:
        ...
        config = self.resolve_layer_config(sport, "people")

        provider = config.get("provider") or self.resolve_provider_for_sport(sport)
        configured_run_group = config.get("run_group") or f"{sport}_PEOPLE"
        entity = config.get("entity") or "players"

        # PEOPLE V2.2 zatím podporuje hlavně players.
        # Pokud uživatel vybere player_profiles/player_stats apod., panel bezpečně přepne na players.
        if selected_entities and selected_entities[0] != entity:
            self.log_write(
                f"PEOPLE V2.2 pro {sport} používá entity={entity}; vybráno '{selected_entities[0]}', přepínám na {entity}."
            )

        # V15 SIMPLE MODE: PEOPLE nikdy nepřebírá run_group z globálního dropdownu.
        # Důvod: PEOPLE může používat jiného providera a jiný run_group než CORE.
        run_group = configured_run_group
        self.log_write(
            f"PEOPLE config locked: používám SPORT_LAYER_CONFIG pro {sport}, ignoruji globální run_group dropdown."
        )

        self.log_write(
            f"V14 layer config: sport={sport} | layer=people | provider={provider} | run_group={run_group} | entity={entity}"
        )
    ...
    def run_batch_combinations(self) -> None:
        ...
        profile = self.profile_var.get()
        self.log_write(f"Layer profil: {profile} | dávkování: sport x entity, sekvenčně podle limitu")

        combos = []
        for sport in selected_sports:
            layer_config = self.resolve_layer_config(sport, profile)
            provider = layer_config.get("provider") or self.resolve_provider_for_sport(sport)
            effective_run_group_for_sport = run_group or layer_config.get("run_group") or f"{sport}_{profile.upper()}"
            if layer_config:
                self.log_write(
                    f"V14 layer config: sport={sport} | layer={profile} | provider={provider} | run_group={effective_run_group_for_sport}"
                )
            for entity in selected_entities:
                requested_workers = str(max_workers)
                effective_workers = requested_workers

                # FIX2: rizikové entity spouštíme sekvenčně.
                # Chráníme tím PowerShell pull skripty před kolizemi dočasných SQL souborů.
                if entity in ENTITY_SAFE_MAX_WORKERS:
                    safe_workers = ENTITY_SAFE_MAX_WORKERS[entity]
                    try:
                        requested_workers_int = int(requested_workers)
                    except ValueError:
                        requested_workers_int = safe_workers

                    if requested_workers_int > safe_workers:
                        effective_workers = str(safe_workers)
                        self.log_write(
                            f"SAFE WORKERS: entity={entity} | max-workers {requested_workers} -> {effective_workers}"
                        )
                ...
                cmd = [
                    ...
                    "--entity", entity,
                    "--limit", limit,
                    "--max-workers", effective_workers,
                    "--timeout-sec", timeout_sec,
                ]

                # FIX1: API-Sport teams pull vyžaduje Season.
                # Bez sezony PowerShell prompt čeká na vstup a panel doběhne do timeoutu.
                if entity in SEASON_REQUIRED_ENTITIES:
                    cmd.extend(["--season", DEFAULT_SEASON])

                if effective_run_group_for_sport:
                    cmd.extend(["--run-group", effective_run_group_for_sport])
