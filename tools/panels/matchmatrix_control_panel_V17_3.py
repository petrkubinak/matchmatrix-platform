# MATCHMATRIX CONTROL PANEL V17.3
#
# CO TO JE:
...
# - SMART RUN NEXT přes ops.v_orchestration_priority_queue_v3
#
...
# C:\Python314\python.exe tools\matchmatrix_control_panel_V17_3.py
...
FROM ops.v_orchestration_priority_queue_v3
WHERE final_ready_for_run_next = true
...
class MatchMatrixPanelV170(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title("MatchMatrix Control Panel V17.3 SMART RUN NEXT")
    ...
    def setup_scheduler_tree(self):
        ...
        for col in columns:
            ...
            if col in ("ready_for_scheduler", "worker_already_running", "execution_allowed", "has_pending_planner_job", "final_ready_for_run_next"):
                width = 140
    ...
    def render_scheduler(self):
        ...
        for row in self.rows_scheduler:
            ...
            if row.get("worker_already_running") or not row.get("final_ready_for_run_next"):
                tag = "BLOCKED"
            ...
            self.tree_scheduler.insert(
                ...
                values=(
                    ...
                    row["planner_job_id"],
                    row["planner_job_status"],
                    row["planner_priority"],
                    row["has_pending_planner_job"],
                    row["planner_guard_state"],
                    row["final_ready_for_run_next"],
                    row["run_next_state"],
    ...
    def run_next_scheduler_job(self):
        ...
        final_ready_for_run_next = row.get("final_ready_for_run_next")
        has_pending_planner_job = row.get("has_pending_planner_job")
        planner_job_id = row.get("planner_job_id")
        planner_job_status = row.get("planner_job_status")
        planner_guard_state = row.get("planner_guard_state")
        run_next_state = row.get("run_next_state")
        orchestration_state = row.get("orchestration_state")
        timeout_sec = int(row.get("timeout_sec") or 600)

        if worker_already_running or not execution_allowed or not ready_for_scheduler or not final_ready_for_run_next:
            messagebox.showwarning(
                "RUN NEXT BLOCKED",
                ...
                "Důvod: worker už běží, není execution_allowed, není ready_for_scheduler nebo chybí pending planner job."
            )
            return
