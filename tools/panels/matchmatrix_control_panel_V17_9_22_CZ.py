"""
MATCHMATRIX CONTROL PANEL V17.9.22
========================================================
...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V17.9.22 CZ"
        )
    ...
    def build_ui(self):
        ...
        self.kpi_stav = self.create_kpi("🛡 STAV", "PŘIPRAVEN", GREEN)
        self.kpi_pending = self.create_kpi("⏳ ČEKAJÍCÍ", "0", YELLOW)
        self.kpi_alerty = self.create_kpi("🔔 ALERTY", "0", RED)
        self.kpi_safe = self.create_kpi("✅ BEZPEČNÉ", "0", PURPLE)
        self.kpi_conf = self.create_kpi("📈 DŮVĚRA AI", "0", PINK)

        self.ai_critical = self.create_kpi("🚨 AI KRITICKÉ", "0", RED)
        self.ai_safe_retry = self.create_kpi("♻ BEZPEČNÝ RETRY", "0", GREEN)
        self.ai_auto_fix = self.create_kpi("🛠 AUTO OPRAVA", "0", PURPLE)
        self.ai_blocking = self.create_kpi("⛔ BLOKOVÁNO", "0", RED)
        self.ai_score = self.create_kpi("🤖 AI SKÓRE", "0", PINK)

        self.coverage_ready = self.create_kpi("📦 READY", "0", GREEN)
        self.coverage_missing = self.create_kpi("🚧 CHYBÍ", "0", YELLOW)
        self.coverage_paid = self.create_kpi("💰 ČEKÁ NA PRO", "0", PURPLE)
        self.dev_backlog = self.create_kpi("📋 BACKLOG", "0", PINK)

        self.autonomous_ready = self.create_kpi("🚀 AUTO READY", "0", GREEN)
        self.autonomous_running = self.create_kpi("▶ AUTO BĚŽÍ", "0", YELLOW)
        self.autonomous_success = self.create_kpi("✅ AUTO OK", "0", PURPLE)
        self.autonomous_failed = self.create_kpi("❌ AUTO CHYBA", "0", RED)
    ...
    def run_next_safe(self):
        ...
        if worker not in WORKER_COMMANDS:
            ...
            messagebox.showerror(
                "CHYBA",
                f"Není definovaný příkaz pro {worker}"
            )
