"""
MATCHMATRIX CONTROL PANEL V20.1.P OPERATOR PANEL RECOMMENDATION BINDING
========================================================
...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V20.1.P - OPERATOR RECOMMENDATION BINDING CZ"
        )
...
Další termín: {h.get('next_target_date') or '-'}"""
    ...
    def load_pc2_command_center(self):
        ...
        SELECT
            ...
            q.execution_bucket,
            q.priority_score,
            COALESCE(ab.effective_status, q.run_status) AS run_status,
            q.safety_mode,
        ...
        ORDER BY
            CASE
                WHEN COALESCE(ab.effective_status, q.run_status) IN ('FAILED','ERROR') THEN 1
                WHEN COALESCE(ab.effective_status, q.run_status) = 'READY_TO_RUN' THEN 2
                WHEN COALESCE(ab.effective_status, q.run_status) = 'RUNNING' THEN 3
                WHEN COALESCE(ab.effective_status, q.run_status) = 'DONE' THEN 4
                WHEN COALESCE(ab.effective_status, q.run_status) = 'BLOCKED' THEN 5
                ELSE 9
            END,
            q.priority_score,
            q.sport_code,
        ...
        SELECT
            ...
            COALESCE(ab.effective_status, q.run_status) AS effective_status,
            ab.classification_reason,
            ab.operator_recommendation,
            ab.button_label_cz,
            ab.button_color,
    ...
    def populate_pc2_action_cards(self, tree, rows):
        """
        V20.1.P - PC2 ACTION CARDS + OPERATOR BUTTONS
        ...
        try:
            ...
            tree.heading("operator_button", text="DOPORUČENÁ AKCE")
            ...
            tree.column("operator_button", width=245, minwidth=190, stretch=False, anchor="w")
            ...
            for row in rows:
                ...
                button_label = str(row.get("button_label_cz") or row.get("operator_recommendation") or "🔎 DETAIL").strip()
        ...
        try:
            rows = db_query(f"""
                SELECT
                    ...
                    button_color,
                    button_help_cz
                FROM ops.v_operator_action_buttons_v1
    ...
    def update_operator_recommendation_for_selection(self, command_id=None):
        ...
        try:
            ...
            label = row.get("button_label_cz") or "🔎 DETAIL"
            color = self.operator_button_color_to_hex(row.get("button_color"))
            enabled = bool(row.get("button_enabled", True))
            help_text = row.get("button_help_cz") or row.get("operator_recommendation") or "Otevři detail řádku."
            ...
            if hasattr(self, "global_operator_action_help"):
                self.global_operator_action_help.config(
                    text=f"{row.get('sport_code') or '-'} / {row.get('target_layer') or '-'}\n{reason}\n{help_text}"
                )
        except Exception as exc:
            try:
