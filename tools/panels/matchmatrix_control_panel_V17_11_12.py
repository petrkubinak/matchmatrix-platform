...
class KpiValueHandle:
    ...
    def redraw(self):
        try:
            ...
            canvas.create_rectangle(
                1, h - 6, max(2, fill_w), h - 3,
                fill=self.accent_color,
                outline=self.accent_color
            )

            # Stavová tečka vlevo nahoře – barva jen jako indikátor, ne celý box.
            canvas.create_oval(
                2, 1, 9, 8,
                fill=self.accent_color,
                outline=self.accent_color
...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V17.11.12 CZ"
        )
    ...
    def create_kpi(
        ...
        """
        V17.11.12 - grafická KPI karta.

        CO TO JE:
        - KPI už není velká barevná cedule.
        - Barva je jen tenký akcent, tečka a malý indikátor dole.

        K ČEMU TO JE:
        - Horní panel je klidnější.
        - Důležité stavy jsou čitelné, ale nebijí do očí.
        """
        ...
        border_color = color if is_important else CARD_BORDER
        title_color = color if is_important else MUTED
        ...
        for widget in (frame, inner, title_lbl, lbl, subtitle_lbl, graph):
            widget.bind(
                "<Button-1>",
