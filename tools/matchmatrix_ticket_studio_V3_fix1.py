...
class TicketStudioV3:
    ...
    def load_leagues_and_matches(self, initial: bool):
        ...
        sql = """
            ...
            limited_matches AS (
                ...
                WHERE (%s = 'ALL' OR fm.sport_code = %s)
                ORDER BY fm.league_name, fm.kickoff ASC NULLS LAST, fm.home_team, fm.away_team
                LIMIT 400
            ),
            odds_agg AS (
                SELECT
                ...
                JOIN public.markets mk ON mk.id = mo.market_id
                JOIN limited_matches lm ON lm.match_id = o.match_id
            ...
            LEFT JOIN odds_agg oa ON oa.match_id = lm.match_id
            WHERE (%s = FALSE OR (oa.odd_1 IS NOT NULL AND oa.odd_x IS NOT NULL AND oa.odd_2 IS NOT NULL))
            ORDER BY lm.league_name, lm.kickoff ASC NULLS LAST, lm.home_team, lm.away_team
        """
        params: list = [time_interval, sport_code, sport_code, only_odds]
    ...
    def describe_generated_row(self, row: dict) -> dict:
        ...
        fixed_parts = [self.fmt_odds(item.odd_value) for item in self.fixed_items]
        blocks = {"A": [], "B": [], "C": []}
    ...
    def export_generated_csv(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            filetypes=[("CSV", "*.csv")],
            initialfile="matchmatrix_ticket_overview_v3_fix1.csv",
        )
        if not path:
            return
        try:
