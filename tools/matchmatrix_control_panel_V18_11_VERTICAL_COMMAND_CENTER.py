"""
MATCHMATRIX CONTROL PANEL V18.7 PEOPLE GOVERNANCE
========================================================
...
V18.1:
- doplněné V18 command center záložky: Provider Matrix, Media, Architecture, Governance,
- napojení na 118_A až 118_E foundation views,
...
TAB_HELP_TEXTS = {
    ...
    "GOVERNANCE": ("GOVERNANCE", "Detail databázové governance: ACTIVE_MASTER, ACTIVE_PANEL, REVIEW, LEGACY a DROP kandidáti."),
}
...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V18.11 VERTICAL COMMAND CENTER CZ"
        )
    ...
    def build_ui(self):
        ...
        # =========================================================
        # V18.11 - MATCHMATRIX VERTICAL COMMAND CENTER
        # =========================================================
        # CO TO JE:
        # - Horní část panelu je nově čtená zleva doprava:
        #   MATCHMATRIX CELKEM -> hlavní oblasti -> OPS KPI -> akční tlačítka -> doporučení.
        # - Hlavní oblasti už nejsou jako malé karty vedle sebe, ale jako řádky s grafickým pruhem.
        # - KPI jsou také pod sebou, aby nepůsobila jako přeplněná technická lišta.
        # - Tlačítka jsou seřazená do skupin podle účelu.
        # - Dnešní priorita a AI doporučení jsou úplně vpravo pod sebou.
        ...
        command_frame.pack(fill="x", padx=8, pady=(3, 5))
        command_frame.columnconfigure(0, weight=2, minsize=190)
        command_frame.columnconfigure(1, weight=5, minsize=470)
        command_frame.columnconfigure(2, weight=2, minsize=210)
        command_frame.columnconfigure(3, weight=3, minsize=330)
        command_frame.columnconfigure(4, weight=4, minsize=430)
        command_frame.rowconfigure(0, weight=1)

        # Levý dominantní blok: MatchMatrix celkem.
        self.command_total_frame = tk.Frame(
            command_frame,
            bg="#11091a",
            highlightbackground="#5b3475",
            highlightthickness=1
        )
        self.command_total_frame.grid(row=0, column=0, sticky="nsew", padx=(8, 5), pady=7)

        tk.Label(
            self.command_total_frame,
            text="MATCHMATRIX",
            bg="#11091a",
            fg="#d8b4fe",
        ...
        self.command_total_value = tk.Label(
            self.command_total_frame,
            text="36 %",
            bg="#11091a",
            fg="white",
        ...
        self.command_total_value.pack(anchor="w", padx=12, pady=(0, 0))

        self.command_total_status = tk.Label(
            self.command_total_frame,
            text="CELKOVÁ PŘIPRAVENOST PLATFORMY",
            bg="#11091a",
            fg="#bca0d4",
        ...
        self.command_total_status.pack(anchor="w", padx=12, pady=(0, 7))

        self.command_total_bar = tk.Canvas(
            self.command_total_frame,
            height=14,
            bg="#11091a",
            highlightthickness=0
        )
        self.command_total_bar.pack(fill="x", padx=12, pady=(0, 10))
        self.command_total_bar.bind("<Configure>", lambda event: self.draw_command_total_bar())

        for widget in (self.command_total_frame, self.command_total_value, self.command_total_status, self.command_total_bar):
            widget.bind("<Button-1>", lambda event: self.open_project_layer_help("PROJEKT"))
            widget.configure(cursor="hand2")

        # Hlavní oblasti: řádky pod sebou s procentem vpravo.
        command_metrics_panel = tk.Frame(command_frame, bg="#0a0610")
        command_metrics_panel.grid(row=0, column=1, sticky="nsew", padx=5, pady=7)
        ...
        self.create_command_center_metric_card(command_metrics_panel, 6, "WEB", "Aplikace", 12, RED)

        # OPS KPI: pod sebou, ne jako dlouhá přeplněná lišta.
        command_kpi_panel = tk.Frame(command_frame, bg="#0a0610")
        command_kpi_panel.grid(row=0, column=2, sticky="nsew", padx=5, pady=7)
        ...
        self.kpi_stav = self.create_command_kpi_row(command_kpi_panel, 1, "🛡 STAV", "PŘIPRAVEN", GREEN, "Systém")
        self.kpi_pending = self.create_command_kpi_row(command_kpi_panel, 2, "⏳ FRONTY", "0", YELLOW, "Ke spuštění")
        self.kpi_alerty = self.create_command_kpi_row(command_kpi_panel, 3, "🔔 ALERTY", "0", RED, "Kritické/var.")
        self.kpi_safe = self.create_command_kpi_row(command_kpi_panel, 4, "✅ SAFE RUN", "0", PURPLE, "Bezpečné")
        self.kpi_conf = self.create_command_kpi_row(command_kpi_panel, 5, "📈 AI", "0", PINK, "Důvěra")

        # Detailní technické KPI zůstávají načítané, ale nejsou v horní liště.
        ...
        # Akční tlačítka: skupiny podle účelu.
        action_panel = tk.Frame(command_frame, bg="#0a0610")
        action_panel.grid(row=0, column=3, sticky="nsew", padx=5, pady=7)
        ...
        action_group_system = tk.Frame(action_panel, bg="#100918")
        action_group_system.grid(row=1, column=0, sticky="ew", pady=(0, 3))
        tk.Label(action_group_system, text="SYSTÉM", bg="#100918", fg="#8f7ca3", font=("Segoe UI", 7, "bold")).pack(side="left", padx=(6, 4))
        self.make_button(action_group_system, "↻ OBNOVIT", "#3b2555", self.refresh_all)
        self.auto_mode_button = self.make_button(action_group_system, "● REŽIM: MANUÁL", "#3b2555", self.toggle_auto_mode)

        action_group_run = tk.Frame(action_panel, bg="#100918")
        action_group_run.grid(row=2, column=0, sticky="ew", pady=(0, 3))
        tk.Label(action_group_run, text="SPUŠTĚNÍ", bg="#100918", fg="#8f7ca3", font=("Segoe UI", 7, "bold")).pack(side="left", padx=(6, 4))
        self.make_button(action_group_run, "⭐ DOPORUČENÁ", "#6d45b8", self.run_recommended_worker)
        self.make_button(action_group_run, "▶ DALŠÍ", "#0f6a42", self.run_next_safe)
        self.make_button(action_group_run, "▶ VYBRANÝ", "#0f5f63", self.run_selected_worker)

        action_group_ai = tk.Frame(action_panel, bg="#100918")
        action_group_ai.grid(row=3, column=0, sticky="ew", pady=(0, 3))
        tk.Label(action_group_ai, text="AI / LOG", bg="#100918", fg="#8f7ca3", font=("Segoe UI", 7, "bold")).pack(side="left", padx=(6, 4))
        self.make_button(action_group_ai, "ℹ PROČ", "#4c2c83", self.explain_recommended_worker)
        self.make_button(action_group_ai, "🤖 AUTO", "#0f5f63", self.run_autonomous_dispatch)
        self.make_button(action_group_ai, "🗑 LOG", "#6f1d4d", self.clear_log)

        runtime_row = tk.Frame(action_panel, bg="#0a0610")
        runtime_row.grid(row=4, column=0, sticky="ew", pady=(4, 0))
        ...
        self.auto_status_label.grid(row=1, column=0, sticky="ew", pady=(2, 0))
        ...
        self.worker_progress.grid(row=2, column=0, sticky="ew", pady=(5, 0))

        # Pravý sloupec: Dnešní priorita + AI doporučení.
        self.priority_bar = tk.Frame(command_frame, bg="#0a0610")
        self.priority_bar.grid(row=0, column=4, sticky="nsew", padx=(5, 8), pady=7)
        ...
        self.today_priority_text = self.create_info_card(
            self.priority_bar,
            "🎯 DNEŠNÍ PRIORITA",
            1,
            0
        ...
        self.ai_recommendation_text = self.create_info_card(
            self.priority_bar,
            "🤖 AI DOPORUČENÍ",
            2,
            0
    ...
    def create_command_center_metric_card(self, parent, row, title, subtitle, percent, color):
        ...
        card.grid(row=row, column=0, sticky="ew", padx=0, pady=2)
    ...
    def create_command_kpi_row(self, parent, row, title, value, color, subtitle=""):
        ...
        tk.Label(
            ...
            font=("Segoe UI", 6, "bold"),
            anchor="e"
        ).grid(row=0, column=2, rowspan=2, sticky="e", padx=(4, 7))
...
Další termín: {h.get('next_target_date') or '-'}"""
    ...
    def load_media_dashboard(self):
        ...
        CO TO JE:
        - Přehled článků, zdrojů, refresh fronty a linkování media vrstvy.
        """
    ...
    def load_governance_dashboard(self):
        ...
        CO TO JE:
        - Detail nad ops.database_object_governance podle stavů ACTIVE_MASTER, ACTIVE_PANEL,
          ACTIVE_REVIEW, LEGACY_KEEP a DROP_CANDIDATE.
        """
        ...
        SELECT
            ...
            updated_at
        FROM ops.database_object_governance
        WHERE governance_status = 'ACTIVE_REVIEW'
        ORDER BY
            schema_name,
            object_name
        LIMIT 300;
        ...
        SELECT
            ...
            updated_at
        FROM ops.database_object_governance
        WHERE governance_status IN ('LEGACY_KEEP', 'DROP_CANDIDATE')
        ORDER BY
            governance_status DESC,
            schema_name,
            object_name
        LIMIT 300;
        """
        ...
        self.populate_tree(self.governance_master_tree, db_query(master_sql))
        self.populate_tree(self.governance_review_tree, db_query(review_sql))
        self.populate_tree(self.governance_legacy_tree, db_query(legacy_sql))
        self.populate_tree(self.governance_detail_tree, db_query(detail_sql))
