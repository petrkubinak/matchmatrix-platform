"""
MATCHMATRIX CONTROL PANEL V20.C.2 VISUAL OPERATOR DASHBOARD
========================================================
...
class MatchMatrixAdminPanel(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(
            "MATCHMATRIX OPS PANEL V20.C.2 - VISUAL OPERATOR DASHBOARD CZ"
        )
    ...
    def build_ui(self):
        ...
        tk.Label(
            header,
            text="▰ MATCHMATRIX ŘÍDICÍ CENTRUM V20.C.2",
            bg="#07040d",
            fg=PINK,
...
# =========================================================
        ...
        # V20.C.2 - VISUAL OPERATOR DASHBOARD
        # CO TO JE:
...
Další termín: {h.get('next_target_date') or '-'}"""
    ...
    def update_pc2_visual_operator_cards(self, rows):
        """
        Přepočet grafických karet z PC2 command queue.
        """
        try:
            rows = [r for r in (rows or []) if "CHYBA" not in r]
            total = len(rows)
            done = len([r for r in rows if str(r.get("run_status") or "").upper() == "DONE"])
            running = len([r for r in rows if str(r.get("run_status") or "").upper() == "RUNNING"])
            ready = len([r for r in rows if str(r.get("run_status") or "").upper() == "READY_TO_RUN"])
            failed = len([r for r in rows if str(r.get("run_status") or "").upper() in ("FAILED", "ERROR")])
            blocked = len([r for r in rows if str(r.get("run_status") or "").upper() == "BLOCKED"])

            day_pct = int(round((done / total) * 100)) if total else 0
            self.set_pc2_visual_card(
                "day",
                f"{day_pct} %",
                f"Hotovo: {done}   Běží: {running}   Čeká: {ready}   Chyba: {failed}   Blokováno: {blocked}",
                day_pct,
                GREEN if failed == 0 else RED
            )

            current = None
            for wanted in ("RUNNING", "READY_TO_RUN", "FAILED", "BLOCKED"):
                found = [r for r in rows if str(r.get("run_status") or "").upper() == wanted]
                if found:
                    current = found[0]
                    break

            if current:
                title = str(current.get("command_title") or "Další akce")
                sport = current.get("sport_code") or "-"
                layer = current.get("target_layer") or "-"
                status = cz_status(current.get("run_status"))
                priority = current.get("priority_score") or "-"
                pct = 100 if str(current.get("run_status") or "").upper() == "DONE" else (50 if str(current.get("run_status") or "").upper() == "RUNNING" else 0)
                self.set_pc2_visual_card(
                    "current",
                    f"{sport} / {layer}",
                    f"{status} | Priorita: {priority}\n{title[:95]}",
                    pct,
                    PURPLE if str(current.get("run_status") or "").upper() != "FAILED" else RED
                )
            else:
                self.set_pc2_visual_card("current", "Žádná akce", "Fronta je prázdná nebo není načtená.", 0, YELLOW)

            last_done = None
            done_rows = [r for r in rows if str(r.get("run_status") or "").upper() == "DONE"]
            if done_rows:
                last_done = done_rows[0]
            if last_done:
                result = str(last_done.get("last_result") or "Dokončeno")
                self.set_pc2_visual_card(
                    "result",
                    "100 % OK",
                    f"{last_done.get('sport_code') or '-'} / {last_done.get('target_layer') or '-'}\n{result[:100]}",
                    100,
                    GREEN
                )
            else:
                self.set_pc2_visual_card("result", "Zatím nic", "Čeká se na první dokončený běh.", 0, YELLOW)

            if failed > 0:
                first_error = [r for r in rows if str(r.get("run_status") or "").upper() in ("FAILED", "ERROR")][0]
                self.set_pc2_visual_card(
                    "error",
                    f"{failed} chyba",
                    f"{first_error.get('sport_code') or '-'} / {first_error.get('target_layer') or '-'}\nDoporučení: klikni na řádek a použij RETRY / READY nebo TEST.",
                    100,
                    RED
                )
            else:
                self.set_pc2_visual_card(
                    "error",
                    "0 STOP",
                    "Bez chyb. Můžeš pokračovat další READY akcí.",
                    100,
                    GREEN
                )
        except Exception as exc:
            try:
                self.set_pc2_visual_card("error", "Chyba panelu", str(exc)[:120], 100, RED)
            except Exception:
                pass
