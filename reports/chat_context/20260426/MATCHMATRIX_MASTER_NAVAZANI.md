...
Zítra začít tímto příkazem ve VS terminálu:

cd C:\MatchMatrix-platform
python ingest\API-Handball\inspect_api_handball_leagues_db.py
...
Podle výstupu připravit přesný parser:
C:\MatchMatrix-platform\ingest\API-Handball\parse_api_handball_leagues_v1.py

Cíl HB:
raw → parse → staging.stg_provider_leagues → public.leagues → runtime audit CONFIRMED

Pokračovat po jedné akci.