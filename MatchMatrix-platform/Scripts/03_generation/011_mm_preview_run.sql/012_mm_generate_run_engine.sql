...
begin
  -- 1) Preview (odhad + validace) před jakýmkoli generováním
  select pr.estimated_tickets, pr.preview_warnings
    into v_estimated_tickets, v_warnings
  from public.mm_preview_run(p_template_id) pr;

  v_estimated_tickets := coalesce(v_estimated_tickets, 0);

  -- 2) Pokud preview hlásí validační problémy (kromě LIMIT), stop
  ...
  if array_length(v_nonlimit_warnings, 1) is not null then
    raise exception using
      errcode = 'P0001',
      message = format(
        'MatchMatrix: template validation failed for template_id=%s. Warnings: %s',
        p_template_id,
        array_to_string(v_nonlimit_warnings, ' | ')
      );
  end if;

  -- 3) Hard limit na počet tiketů
  if v_estimated_tickets > v_max then
    raise exception using
      errcode = 'P0001',
      message = format(
        'MatchMatrix: LIMIT exceeded. estimated_tickets=%s > max_tickets=%s for template_id=%s',
        v_estimated_tickets, v_max, p_template_id
      );
  end if;

  -- 4) Teprve teď založíme run (uložíme efektivní limit)
  insert into public.generated_runs(template_id, created_at, max_tickets, min_probability)
  values (p_template_id, now(), v_max, p_min_probability)
  returning id into v_run_id;

  -- 5) Generování tiketů (posíláme efektivní limit)
