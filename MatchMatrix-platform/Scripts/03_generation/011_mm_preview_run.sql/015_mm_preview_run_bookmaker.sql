create or replace function public.mm_preview_run(
  p_template_id bigint,
  p_bookmaker_id int
)
returns table(
  template_id bigint,
  variable_blocks integer,
  fixed_picks integer,
...
begin
  ...
  -- 🔥 TADY JE ZMĚNA → filtr na bookmaker_id
  best_odds as (
    select
      tbm.block_index,
      tbm.match_id,
    ...
    where tbm.template_id = p_template_id
      and o.bookmaker_id = p_bookmaker_id   -- ⭐ hlavní filtr
    ...
    (
      ...
      || case
           when c.estimated_tickets > v_max_tickets then
             array[format(
               'LIMIT: estimated_tickets=%s exceeds max_tickets=%s.',
               c.estimated_tickets, v_max_tickets
             )]
           else array[]::text[]
         end
    ) as preview_warnings
