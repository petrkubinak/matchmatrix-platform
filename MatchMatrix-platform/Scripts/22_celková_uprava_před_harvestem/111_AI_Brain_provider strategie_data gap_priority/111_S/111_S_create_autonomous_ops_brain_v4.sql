...
AUTONOMOUS OPS BRAIN V4

CO TO JE:
- Přidává Focus Alignment Score.
- Brain už nevybírá jen podle AI retry.
- Kontroluje, jestli entita odpovídá doporučené vrstvě sportu.

K ČEMU TO JE:
- Pokud je sport ve stavu MEDIA_LAYER, nesmí nahoře doporučovat CORE fixtures/teams.
- Pokud je sport ve stavu PEOPLE_LAYER, má preferovat players/coaches.
- Pokud je sport ve stavu CORE_HARVEST, má preferovat leagues/teams/fixtures.

KDE TO UVIDÍME:
- ops.v_autonomous_ops_brain_v4

JAK SE TO VYUŽIJE:
- Budoucí 111_S launcher bude spouštět pouze akce s brain_decision = RUN.
*/

CREATE OR REPLACE VIEW ops.v_autonomous_ops_brain_v4 AS
WITH normalized AS (
    SELECT
        CASE
    ...
    SELECT
        ...
        CASE
            WHEN s.recommended_focus = 'PEOPLE_LAYER'
             AND n.entity IN ('players', 'coaches', 'coach', 'people', 'player_profiles', 'player_stats')
                THEN 40

            WHEN s.recommended_focus = 'MEDIA_LAYER'
             AND n.entity IN ('media', 'articles', 'article', 'highlights', 'videos', 'video', 'media_refresh', 'media_assets')
                THEN 40

            WHEN s.recommended_focus = 'ODDS_LAYER'
             AND n.entity IN ('odds', 'bookmakers', 'markets')
                THEN 40

            WHEN s.recommended_focus = 'CORE_HARVEST'
             AND n.entity IN ('leagues', 'teams', 'fixtures', 'matches')
                THEN 40
            ...
            WHEN s.recommended_focus = 'PEOPLE_LAYER'
             AND n.entity IN ('leagues', 'teams', 'fixtures', 'matches', 'odds')
                THEN -25

            WHEN s.recommended_focus = 'ODDS_LAYER'
             AND n.entity IN ('leagues', 'teams', 'fixtures', 'matches', 'players', 'coaches')
                THEN -25

            ELSE 0
        END AS focus_alignment_score,
...
scored AS (
    SELECT
        *,
        ...
        - COALESCE(empty_runs, 0) * 10
        -
        CASE
            WHEN is_not_implemented THEN 100
            ELSE 0
...
SELECT
    ...
    CASE
        ...
        WHEN GREATEST(raw_brain_score, 0) >= 90 THEN 'RUN'
        WHEN GREATEST(raw_brain_score, 0) >= 55 THEN 'RUN_WITH_CAUTION'
    ...
    CASE
        ...
        WHEN focus_alignment_score >= 40
            THEN 'Entita odpovídá doporučené vrstvě sportu.'
        WHEN focus_alignment_score < 0
            THEN 'Entita neodpovídá doporučené vrstvě sportu.'
        WHEN GREATEST(raw_brain_score, 0) >= 90
            THEN 'Vysoké skóre, vhodné pro autonomní spuštění.'
        WHEN GREATEST(raw_brain_score, 0) >= 55
