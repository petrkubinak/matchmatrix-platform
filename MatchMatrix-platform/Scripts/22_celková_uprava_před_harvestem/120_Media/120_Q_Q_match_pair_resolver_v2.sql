/*
===============================================================================
MATCHMATRIX SQL 120_Q_Q
MATCH PAIR RESOLVER V2
===============================================================================

CO TO JE:
- Vylepšená funkce pro hledání zápasů podle dvojice týmů.
- Přidává volitelný filtr sportu.

K ČEMU TO JE:
- Aby dotaz typu "Barcelona vs Real Madrid" mohl vracet:
  - všechny sporty
  - nebo pouze fotbal / basket / hokej podle zvoleného sport_id.
...
JAK SE TO VYUŽIJE:
- SELECT * FROM ops.fn_context_match_pair_search_v2('barcelona', 'real madrid', NULL, 30);
- SELECT * FROM ops.fn_context_match_pair_search_v2('barcelona', 'real madrid', 1, 30);
===============================================================================
*/

CREATE OR REPLACE FUNCTION ops.fn_context_match_pair_search_v2(
    p_team_a TEXT,
    p_team_b TEXT,
...
WITH team_a AS (
    SELECT
        ...
        sport_id,
        final_score
    FROM ops.fn_context_search_v3(p_team_a, 30)
...
team_b AS (
    SELECT
        ...
        sport_id,
        final_score
    FROM ops.fn_context_search_v3(p_team_b, 30)
    WHERE entity_type = 'TEAM'
      AND (p_sport_id IS NULL OR sport_id = p_sport_id)
),
...
WHERE p_sport_id IS NULL
   OR m.sport_id = p_sport_id
