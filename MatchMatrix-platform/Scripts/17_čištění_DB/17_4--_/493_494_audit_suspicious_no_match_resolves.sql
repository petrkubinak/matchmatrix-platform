...
--   unmatched_theodds. Pokud je tvoje importni tabulka pojmenovana jinak,
...
-- ============================================================================
-- 494_audit_suspicious_no_match_resolves.sql
-- Audit podezřelých resolve výsledků po 493
--
-- ULOZENI:
--   C:\MatchMatrix-platform\db\audit\494_audit_suspicious_no_match_resolves.sql
--
-- SPUSTENI:
--   Spust po 493 ve stejne DBeaver session
-- ============================================================================

begin;

-- 1) Podezrele resolve podle nazvu / vekovych vetvi / geografickych konfliktu
select
    src_row_id,
    league_key,
    league_name_raw,
    ...
    audit_group
from tmp_no_match_audit
where audit_group = 'missing_fixture'
  and (
       coalesce(home_team_name, '') ilike '%U18%'
    or coalesce(home_team_name, '') ilike '%U19%'
    or coalesce(home_team_name, '') ilike '%U20%'
    or coalesce(home_team_name, '') ilike '%U21%'
    or coalesce(home_team_name, '') ilike '%Women%'
    or coalesce(home_team_name, '') ilike '%W%'
    or coalesce(away_team_name, '') ilike '%U18%'
    or coalesce(away_team_name, '') ilike '%U19%'
    or coalesce(away_team_name, '') ilike '%U20%'
    or coalesce(away_team_name, '') ilike '%U21%'
    or coalesce(away_team_name, '') ilike '%Women%'
    or coalesce(away_team_name, '') ilike '%W%'
    or lower(coalesce(home_name_raw, '')) <> lower(coalesce(home_team_name, ''))
    or lower(coalesce(away_name_raw, '')) <> lower(coalesce(away_team_name, ''))
  )
order by league_key, home_name_raw, away_name_raw;

-- 2) Souhrn podezrelych resolve podle ligy
select
    coalesce(league_key, '(null)') as league_key,
...
group by coalesce(league_key, '(null)')
order by suspicious_rows desc, league_key;
