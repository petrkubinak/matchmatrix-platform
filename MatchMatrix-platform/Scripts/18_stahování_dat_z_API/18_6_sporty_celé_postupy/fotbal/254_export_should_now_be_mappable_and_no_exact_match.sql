-- =====================================================================
-- 254_export_should_now_be_mappable_and_no_exact_match.sql
-- Finalni export zbytku po dokonceni MULTI_MATCH cleanup
-- =====================================================================
...
SELECT
    CASE
        WHEN mp.provider_team_id IS NOT NULL THEN 'ALREADY_MAPPED'
        WHEN a.candidate_count = 1 THEN 'SHOULD_NOW_BE_MAPPABLE'
        WHEN a.candidate_count = 0 THEN 'NO_EXACT_MATCH'
        ELSE 'MULTI_MATCH'
...
WHERE mp.provider_team_id IS NULL
  AND (
        a.candidate_count = 1
     OR a.candidate_count = 0
  )
ORDER BY
    CASE
        WHEN a.candidate_count = 1 THEN 1
        ELSE 2
    END,
    a.team_name;