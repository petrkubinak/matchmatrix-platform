-- 2026-03-02 EU leagues import (generated from Evropa do MatchMatrix.xlsx)
-- Save as: C:\MATCHMATRIX-PLATFORM\db\imports\2026-03-02_eu_leagues_api_football_apply.sql
-- Or run directly via psql stream.

BEGIN;

WITH src(bucket,country,country_code,provider_league_id,league_name,league_type) AS (
    VALUES
    ...
    ('EU_national_1_2','Crimea','UA','870','Premier League','League'),
    ...
    ('EU_national_1_2','Iceland','IS','166','1. Deild','League'),
...
-- 0) sanity check: leagues missing from public.leagues
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM canon WHERE canonical_league_id IS NULL) THEN
    RAISE EXCEPTION 'Missing canonical leagues in public.leagues for some provider_league_id';
  END IF;
END $$;

-- 1) whitelist (ops.league_import_plan)
...
-- 2) provider mapping (public.league_provider_map)
INSERT INTO public.league_provider_map(league_id, provider, provider_league_id, updated_at)
SELECT
...
-- 3) runtime targets (ops.ingest_targets)
...
-- Post-checks:
-- SELECT COUNT(*) FROM ops.ingest_targets WHERE provider='api_football' AND run_group LIKE 'EU_%';
