# MATCHMATRIX OFFICIAL SITE MEDIA INGEST V1.2
...
def is_article_url(source_name: str, href: str) -> bool:
    h = href.lower()
    name = source_name.upper().strip()

    if name in ("NBA", "NHL", "FIFA"):
        return "/news" in h

    if name == "UEFA":
        return "/news" in h or "/stories" in h

    if name == "PREMIER LEAGUE":
        return "/news/" in h or "/en/news/" in h

    if name == "LALIGA":
        return "/news" in h or "/en-gb/news" in h

    if name == "BUNDESLIGA":
        return "/news" in h or "/bundesliga/news" in h

    if name == "SERIE A":
        return "/media/" in h or "/news" in h or "/serie-a" in h
...
def main():
    ...
    print("=" * 80)
    print("MATCHMATRIX OFFICIAL SITE MEDIA INGEST V1.2")
    print("=" * 80)
    ...
    for source in sources:
        ...
        try:
            ...
            for link in links:
                ...
                if not is_article_url(source_name, href):
                    continue
            ...
            if health_status == "OK":
                ...
                health_note = "HTTP OK, ale scraper nenašel žádné použitelné odkazy."
            else:
