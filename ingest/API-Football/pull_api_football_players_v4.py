...
# ==========================================================
# LOAD .ENV - vzdy ze stejne slozky jako je skript
...
def get_db_connection():
    ...
    if not pg_password:
        raise RuntimeError(
            f"Chybí PGPASSWORD v .env. Načtený .env: {ENV_PATH}"
        )
...
def _count_filled_stats(stat_row: dict) -> int:
    ...
    fields_to_check = [
        ("games", "appearences"),      # API-Football často používá právě tento překlep
